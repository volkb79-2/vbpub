# Shared game releases and tmpfs lifecycle — v3 proposal

- Status: MVP-oriented revised architecture, 2026-07-23
...
All Wings additions are strictly opt-in. A patched Wings with no lifecycle
providers configured, or a server that selects no provider, follows the stock
container lifecycle and remains fully usable without the release manager. No
socket, manager process, release store, special egg, or systemd slice is a boot
or start prerequisite for ordinary servers. Compatibility is an acceptance
oracle, not merely an implementation preference.
...
Implementation should start with a smaller manager MVP: background creation of
immutable persistent releases, one verified active tmpfs generation shared
read-only by multiple consumers, explicit offline activation/rollback, and a
detailed operation log. Channels, independent rolling generations, automated
RCON maintenance, scoped APIs, and richer scheduling remain the intended
evolution but are not prerequisites for proving the core storage lifecycle.

## Changes from the previous draft (v2)

V3 retains v2's three-layer architecture and makes the following contracts
explicit.

### 1. patched Wings remains stock-usable without the manager

The provider feature is default-off and selected per server. With no provider
configured or selected, the new branches are no-ops and vanilla create, start,
restart, install, SFTP, backup, and file behavior remains unchanged. A provider
failure blocks only a server whose selected policy says that provider is
required; it never turns an unrelated server or the Wings daemon into a
manager dependency.

### 2. provider activation is explicit

Wings does not infer that a server needs shared content merely because the
socket exists or because content-looking variables are present. The node
registers a provider ID, for example `shared-release`, and an admin-only
server/egg variable selects it:

```text
WINGS_CONTENT_PROVIDER=shared-release
```

At every real start Wings first performs its normal Panel sync, resolves the
provider ID under node policy, and calls only that provider. An absent selector
means `none`. Node-side UUID/egg mappings may force or deny selection for
hardened deployments.

### 3. the Unix socket is IPC, not the shared game resource

`/run/wings-providers/shared-release.sock` is one local Unix-domain socket on
which the manager accepts short prepare/lease calls from Wings. It is analogous
to a local API endpoint. The resources shared by game containers are the
immutable release generation and its read-only tmpfs-backed mount; the socket
contains no game data and each start still receives its own lease.

### 4. RCON is local-only

The Soulmask RCON adapter assumes no RCON port is published on a host/public
interface. It reaches loopback inside the selected container network namespace,
as the existing helper does, or uses another explicitly private local path.
RCON therefore remains a manager-to-container maintenance channel, never a
remote user API. The adapter must not weaken that network boundary.

### 5. manager work receives its own aggregate resource boundary

The manager installs a parent slice with separate control, staging, and
publication children. The parent carries aggregate ceilings so multiple
download/update jobs cannot evade the node budget; the staging child carries
the lowest CPU/IO priority and hard device limits. This host-side hierarchy is
useful with vanilla Wings and does not depend on the proposed Wings per-server
cgroup support.

### 6. deliver an MVP before the full manager

V2 described the desired mature product in one pass. V3 defines a shippable
MVP and labels later capabilities explicitly. The MVP proves background release
creation, verified tmpfs publication, multiple read-only consumers, crash-safe
state, rollback to the previous content release, and high-quality logs. It does
not initially promise rolling mixed generations, automatic player-aware
maintenance, online save-schema rollback, or a general scheduler.

### 7. Wings cgroups are a separate proposal

The accompanying
[`shared-ramdisk-update-lifecycle-cgroups-1-codex.md`](shared-ramdisk-update-lifecycle-cgroups-1-codex.md)
redesigns placement, slices, phase policy, and online reconciliation from
vanilla Wings. Release jobs benefit from cgroups, but neither feature requires
the other and they should not share an upstream PR or enablement switch.

## Earlier changes retained from the first draft (v1 to v2)
...
Here “v2” names the fresh Wings patch branch proposed after the current local
stack; it is not the revision number of this document.
...
The hook starts with an activation check. After Wings' normal pre-start Panel
sync has refreshed the server environment, it resolves an optional provider ID
using node policy. Resolution returns one of:

- `none`: take the untouched stock path and perform no socket operation;
- a registered optional provider: call it, but follow explicitly configured
  failure behavior; or
- a registered required provider: call it and refuse this server's start on
  failure.

An unknown or unauthorized non-empty provider selector is an actionable start
error, not a fallback to stale shared files. This affects only the selecting
server. Wings startup itself never connects to all configured sockets merely
...
docker:
  lifecycle_providers:
    shared-release:
      ...
      timeout: 20m
      activation_variable: WINGS_CONTENT_PROVIDER
      allowed_eggs: [soulmask, valheim, generic-steam]
      selector_variables:
...
There is intentionally no implicit default provider in the first patch. An
operator who later wants a node/egg default can configure it explicitly; the
zero value remains `none`. A lifecycle-provider block in node config registers
the ID and socket but does not by itself enroll every server.
...
The first usable release should prove the storage and lifecycle claims with the
fewest moving parts.

MVP capabilities:

1. One node and one or more packaged application profiles; Soulmask is the
   first profile, while the release/manifest engine is game-neutral.
2. A source driver can build a new transaction on persistent disk while the
   current tmpfs generation remains mounted and servers keep running.
3. A transaction becomes an immutable release only after a manifest, required
   probes, hashes, size checks, and `COMPLETE` marker succeed.
4. One group has one active tmpfs generation at a time. Any number of Wings
   consumers receive read-only mounts and individual leases to it.
5. Activation is an explicit offline cohort operation: refuse while a lease or
   container still uses G, populate and verify H, atomically assign H, then let
   users or the CLI start consumers.
6. Retain at least active and previous persistent releases. `rollback` is the
   same offline activation mechanism selecting the previous content manifest.
7. Reconcile tmpfs, releases, leases, and Docker labels after manager/Wings
   restart. Rebuild tmpfs after host reboot.
8. A local CLI exposes `stage`, `status`, `activate`, `rollback`, `gc`, and
   `doctor`, with machine-readable output.
9. Every operation has a durable ID and detailed structured log.

Illustrative MVP workflow:

```text
shared-release stage soulmask --channel stable
shared-release status soulmask-prod --json
shared-release activate soulmask-prod --release <generation>
shared-release rollback soulmask-prod
```

The MVP deliberately excludes:

- two simultaneous tmpfs generations and rolling per-server cutover;
- automatic Panel/Wings stop/start orchestration;
- RCON player-aware scheduling and unattended rollout;
- a network API or multi-node coordinator;
- user-defined source executables or arbitrary host paths;
- automatic mutable-save restoration;
- online install/reinstall integration beyond refusing unsafe operations.

For the two Soulmask servers, an operator can initially perform the existing
save and stop sequence, run `activate`, then start MAIN followed by CLIENT. The
provider still makes an ordinary later Panel restart safe with respect to the
selected content generation. Dependency automation and the rollout controller
are manager/Wings v2 work; the MVP must state that it does not yet guarantee
MAIN readiness ordering for arbitrary manual child starts.

### MVP operation journal

Human logs alone are insufficient for a component that changes mounts and
release assignments. Each command/job writes an append-only structured event
stream to journald and a durable operation record under manager state. Records
include:

- operation/transaction ID, actor, command, profile, group, and server UUIDs;
- start/end timestamps and state transitions;
- source build/manifest identity and sanitized source arguments;
- bytes downloaded/copied, file count, manifest/hash result, and duration;
- selected persistent release and tmpfs generation;
- capacity preflight and relevant cgroup/slice identity;
- lease creation/release and Docker container/generation labels;
- every promotion, rollback, quarantine, cleanup, and recovery decision;
- errors with the failed phase and whether state changed; and
- RCON action names and results when that later adapter is enabled, never its
  password or full secret-bearing environment.

`status --json` and `operation show <id> --json` expose the same state without
requiring log scraping. A dry-run prints the intended source, paths, capacity,
affected leases, and release transition but performs no mount or channel
mutation.

### Evolution after the MVP

The mature capabilities described in the rest of this proposal remain the
roadmap. A sensible order is:

1. multiple persistent channels and exact pins;
2. automated Panel/Wings power operations with operation IDs;
3. local RCON notification/save adapter and scheduled cohort rollout;
4. dual tmpfs generations and rolling/independent policies;
5. scoped service credentials and richer dependency/readiness integration;
6. multi-node distribution only after single-node reconciliation is proven.

The persistent release format, manifests, operation journal, provider protocol,
and Docker labels must be versioned from the MVP so these additions do not
require discarding deployed state.
...
### Release-manager slice hierarchy

Yes: put the entire manager workload below one aggregate parent, then split
control, staging, and publication into children. The parent is the important
part—without it, starting several individually limited downloaders can multiply
the total resource use beyond the intended node budget.

One generic systemd layout is:

```text
-.slice
├─wings.slice                         existing Wings/game tier
│ ├─wings-mgmt.slice
│ └─wings-<server>.slice …
└─game-releases.slice                 aggregate manager boundary
  ├─game-releases-control.slice       daemon/socket/CLI coordination
  │ └─shared-release-manager.service
  ├─game-releases-stage.slice         network/download/unpack/hash jobs
  │ └─shared-release-stage@<op>.service …
  └─game-releases-publish.slice       tmpfs populate/verify during cutover
    └─shared-release-publish@<op>.service
```

The exact root names are operator policy. On a host designed fresh, both tiers
may instead sit under a `game-host.slice`; they must not be forced under the
game server tier if that tier's `memory.min` would accidentally protect
downloaders as though they were live games.

Policy by level:

- `game-releases.slice` carries the aggregate `MemoryHigh`/`MemoryMax`, optional
  `CPUQuota`, and device-specific IO bandwidth/IOPS ceilings.
- `control.slice` retains enough memory/CPU weight for status, cancellation,
  lease service, and cleanup even while a job is throttled.
- `stage.slice` has low CPU and IO weights, no memory protection, bounded
  concurrency, and the strictest device limits.
- `publish.slice` may receive a temporary higher weight during an offline
  cutover, but remains below the aggregate parent ceilings.
- one transient service/scope per operation makes cancellation, accounting,
  timeout, and post-mortem attribution precise.

Weights alone are not hard guarantees: they matter only under sibling
contention and are work-conserving. `CPUQuota`, `MemoryHigh`/`MemoryMax`, and
`IOReadBandwidthMax`/`IOWriteBandwidthMax` or equivalent `io.max` device limits
provide the actual ceilings. `MemoryHigh` throttles but does not cap; use
`MemoryMax` only with enough headroom to avoid killing an unpack/publish job in
an unrecoverable phase. Every child also receives a job-local limit, but the
parent enforces the total.

The manager should launch workers through transient systemd services/scopes or
fixed template units rather than merely moving an already-forked process after
it starts. This makes accounting correct from the first allocation and lets
systemd own/reapply the properties. The socket daemon can run even when no
Wings cgroup patch is installed, so this hierarchy is not coupled to the
separate per-server cgroup proposal.
...
2. An egg defines admin-facing variables for provider ID, profile, group,
   channel, release intent, and optional dependency UUID. App-specific defaults
   come with the egg/profile. `WINGS_CONTENT_PROVIDER=shared-release` is the
   explicit opt-in; leaving it empty is stock behavior.
...
5. On first start, Wings sees the explicit provider selector and calls that
   provider with the server UUID and validated selectors. The provider
   auto-enrols the UUID in the group, resolves the release, creates a tmpfs
   lease, and returns read-only mounts.
...
RCON is explicitly local-only in this design. Soulmask containers do not
publish the RCON port, the release manager does not add a host port, and no
remote manager API proxies arbitrary RCON commands. The adapter chooses a
known server UUID and reaches `127.0.0.1` inside that container's network
namespace. Firewall exposure is therefore not part of setup and a remotely
reachable RCON listener is a failed security oracle.
...
For the MVP, keep RCON out of the long-running downloader/provider process.
Use a root-only local helper with an allow-list of fixed operations such as
player count, broadcast, `SaveWorld 0`, and `BackupDataBase world`. If it needs
the Docker socket to join a container network namespace, that privilege stays
in this narrow helper; the network-facing Steam downloader never receives it.
...
Wings -> release provider
  mandatory on every selected-server start; enforces mounts/generation despite
  user actions
...
1. Core immutable release store, manifest, single active/previous assignment,
   operation journal, fixture source, and kill-at-every-phase tests.
2. Manager parent/control/stage/publish slices plus a SteamCMD driver with
   low-impact staging and fake-source tests.
3. Single-generation tmpfs publisher with verification, capacity policy, and
   crash/reboot recovery.
4. Explicitly selected Wings provider protocol, read-only mounts, Docker
   labels, and lease reconciliation; non-selected servers exercise the stock
   compatibility suite.
5. Soulmask profile, one cohort, generation guard, manual save/stop/start
   runbook, and CLI `stage/status/activate/rollback/doctor`.

Post-MVP:

6. Channels, exact pins, richer selector/group policies, and auto-enrolment.
7. Rollout CLI using a temporary root-only Wings-token or dedicated Panel
   adapter.
8. Local-only RCON notification/save plugin and scheduled rollout.
9. Dual generations, rolling policy, scoped control, and general dependency
   readiness.
...
1. With no providers configured, patched Wings produces the same container
   configuration and start/install behavior as vanilla Wings and never attempts
   a provider socket connection.
2. With a provider registered but no server selector, that server follows the
   same stock path and provider downtime has no effect.
3. Every actual start source for a selected server calls its provider exactly
   once; creating an offline environment does not.
4. Required-provider timeout/failure leaves no container and returns an
   actionable error to the initiating surface.
5. Wings rejects writable, out-of-prefix, denied-target, duplicate, and
   path-escape mounts from a test provider.
6. Create failure calls `abort-start`; successful create labels and commits the
   lease; removal releases it.
7. Provider or Wings crash reconciles leases from Docker labels without
   deleting an in-use generation.
8. A user restart still works through the Panel and receives the selected
   release automatically.
9. Post-MVP dependency handling covers Panel, API, crash, and reboot starts;
   cycles and timeouts cannot deadlock Wings.
10. With that post-MVP feature enabled, CLIENT never transitions to container
    start before MAIN's chosen readiness event and matching-generation check.

### Release manager/tmpfs (MVP plus roadmap)

1. Kill acquisition at arbitrary points; G keeps serving and no incomplete
   transaction becomes `latest`.
2. Saturate staging I/O under a representative game workload; configured
    cgroup/I/O limits prevent the severe latency regression being designed
    against.
3. Stage H while G runs and prove G's files, mounts, hashes, and tmpfs usage do
    not change.
4. In single-generation mode, H publication refuses while a G lease exists.
5. In post-MVP dual-generation mode, capacity is checked before allocation and
   both generations remain immutable until their final lease releases.
6. A write/unlink/rename to every managed root fails in every consumer;
    per-instance `WS/Saved` writes remain isolated and persistent.
7. Reboot loses tmpfs and reconstructs the assigned generation solely from
    its complete persistent release.
8. Unknown source paths, bad hashes, ENOSPC, foreign mounts, and stale
    sentinels are detected before start.
9. Two concurrent staging jobs cannot exceed the aggregate manager parent
    slice's CPU, memory, or configured block-device IO ceilings.
10. Starving the staging child does not prevent the control child from serving
    `status`, cancellation, lease release, or recovery.
11. Every state-changing MVP operation has a durable operation ID whose record
    explains source identity, validation, release assignment, leases, resource
    slice, result, and whether state changed; no credential appears in it.

### Soulmask rollout (MVP plus mature automation)

1. Content-manager operations while stopped leave every `world.db` and
    `GameXishu.json` hash unchanged.
2. The rollout stops CLIENT before MAIN and starts MAIN before CLIENT.
3. Post-MVP RCON save failure aborts unattended rollout without stopping the
   server.
4. Wings-initiated stop does not trigger crash restart; raw RCON `SaveAndExit`
    does restart under vanilla defaults unless protected by maintenance intent.
5. An ordinary MAIN restart while CLIENT runs G receives G even if H is staged.
6. Explicit cohort rollout moves both to H; an explicit rollback reconstructs
    G without using hidden private copies.
7. `tmpfs-required` blocks an unsafe automatic disk fallback. A manual
    degraded override is conspicuous and serves the same verified generation.
8. Docker build/image pressure while Soulmask runs no longer causes the pak
    refault/responsiveness regression at the calibrated tmpfs/cgroup settings.
9. No Soulmask RCON port is published or reachable from an external test
    network; the local namespace adapter still performs its allow-listed calls.
...
| Decision | Soulmask MVP |
|---|---|
| Persistent releases | Active + previous, immutable and verified |
| Live publication | One `tmpfs-required` generation |
| Consumers | Multiple read-only leases, one cohort |
| Acquisition | Manual/background `stage`, resource-limited |
| Activation/rollback | Explicit CLI, all cohort consumers confirmed offline |
| Wings integration | Explicit `WINGS_CONTENT_PROVIDER=shared-release` selection |
| Power/dependency orchestration | Existing manual runbook: save, CLIENT stop, MAIN stop, activate, MAIN start/readiness, CLIENT start |
| RCON | Optional root-only local helper; no published port |
| External API control | None required |
| Audit | Durable operation IDs plus structured journal |

Mature target defaults:
...
- Wings calls the release provider on every selected-server start, so
  user-initiated restarts cannot bypass content preparation, selection, or
  read-only mounts.
- In the mature design Wings guarantees dependency readiness and normal power
  semantics. The MVP keeps the current explicit MAIN-then-CLIENT runbook and
  does not claim that dependency feature yet.
