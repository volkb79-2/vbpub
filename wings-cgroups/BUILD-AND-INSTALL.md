...
```bash
patchstack/scripts/build-image.sh pterodactyl cgroup.8    # -> wings-local:1.13.1-cgroup.8
...
Bump the suffix (`cgroup.8` → `cgroup.9`) for each deployable change so compose
`--force-recreate` and rollback stay unambiguous.

## 4. Verify the image before deploying

```bash
docker run --rm wings-local:1.13.1-cgroup.8 version      # prints the version
...
patchstack/scripts/build-image.sh pterodactyl cgroup.8
```
