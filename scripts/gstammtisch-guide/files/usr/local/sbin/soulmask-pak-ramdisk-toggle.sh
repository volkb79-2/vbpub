...
_show_status() {
  if _is_active; then
    ...
    [ -n "${PAK_DIR:-}" ] && log "  bind: /mnt/soulmask-paks → $PAK_DIR"
    log "  (current server session uses disk-backed pak until next restart)"
  else
...
_activate() {
  ...
  _show_status
  echo ""
  log "ACTION REQUIRED: stop the Soulmask server via Wings panel, then start it again."
  log "  The running session still uses disk-backed pak until the next container start."
...
_deactivate() {
  ...
  log "INACTIVE — ramdisk unmounted, pak directory reverted to disk"
  echo ""
  log "ACTION REQUIRED: stop the Soulmask server via Wings panel, then start it again."
  log "  The running session still uses ramdisk-backed pak until the next container start."
  log "  Wings → server → power → stop → start"
}
