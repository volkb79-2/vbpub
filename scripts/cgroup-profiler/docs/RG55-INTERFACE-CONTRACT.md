...
| `--token <str>` | no | the value run-gate exported as `RUN_GATE_PROFILE_SESSION` into the lane process (`[A-Za-z0-9._-]{8,64}`). With a token, the daemon finds the exact token among numeric PIDs in the explicitly mounted host `/proc`, then includes their descendants; it repeats discovery every interval. Without a token, it samples positive PIDs directly in the target cgroup, using host `/proc/<pid>/cgroup` path resolution when private-PID namespace translation makes `cgroup.procs` report zero. |
