...
- Both tests additionally require an explicit
  `VBPUB_ALLOW_VM_GLOBAL_SWAP_TEST=1` environment variable and a
  `systemd-detect-virt --vm` QEMU/KVM result on top of the pre-existing
  `root + losetup available` skip gate — that old gate
...
- The old privileged container runner and its `--cgroupns=host` launch mode
  were removed. Sharing a container's cgroup namespace with the real host's
  systemd would have been a separate live-state contention hazard even
  without the loop/swap incident.

Those fixes make a guest test failure clean up its guest kernel state. They
are still defense-in-depth: a `docker rm -f`/SIGKILL of the *runner* can
interrupt QEMU, but it cannot turn the guest's swap into host swap. The old
privileged container runner is removed; this VM harness is the only route
for that class of test — and for the much larger full install-then-reboot
flow this project needs.
...
sibling container") was to add QEMU into a privileged systemd container,
since that container already had loop-device access for image
customization. That plan was implemented once, then reverted, because of a realization made while
...
real devtmpfs, real systemd-as-PID1 — was irrelevant to running QEMU.
...
unprivileged image** (`testing/vm/Dockerfile`) and the only container used
by the harness. It is decoupled from the retired privileged test image and
works without host device passthrough.

### 2.3 The architecture

```
run-vm-harness.sh (wrapper, on the devcontainer host)
  └── docker exec → debian-install-vm-harness-<worktree-hash> (persistent, unprivileged container)
...
+ starting persistent VM-harness runner (worktree-specific name)
