...
# Generated by vbpub debian-install v2; release: {release}
#
# Archive-class (`a=`) pinning, not codename-keyed -- resolves the same
# way across a release upgrade (the sources file's {release} substitution
# is the only thing that needs to change; this file never does).
...
# Point-release updates ({release}-updates) -- same priority as stable
Package: *
Pin: release a=stable-updates
Pin-Priority: 500

# Security updates -- version numbering alone already resolves security
# over stable, but pin above it (550) to go straight to security even
# when installing a package fresh, where both look equally "available".
Package: *
...
# Oldstable / oldstable-backports / testing -- visible but never
# auto-selected
Package: *
Pin: release a=oldstable, a=oldstable-backports, a=testing
Pin-Priority: 100

# Unstable -- visible but never auto-selected
Package: *
