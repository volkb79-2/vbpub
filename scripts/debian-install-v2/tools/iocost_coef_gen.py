...
#     symlink; basename()-ing it and re-prepending a bare /dev/ (as
#     upstream does) looks for /dev/<name> directly, which does not exist
#     — only /dev/mapper/<name> does. Resolved via realpath before the
#     partition-glob step.
...
def dir_to_dev(path):
    ...
    # LOCAL PATCH (not upstream): resolve down to a device that actually has
    # its own /sys/block/<dev>/queue/scheduler, alternating the two upstream
    # steps (partition -> whole device) with an LVM/dm-mapper equivalent
    # (logical volume -> real underlying device via /sys/block/<dm>/slaves/)
    # until one applies, since an LVM setup needs BOTH in sequence: findmnt's
    # SOURCE for a logical volume is the /dev/mapper/<name> symlink (only
    # /dev/mapper/<name> exists, not a bare /dev/<name> — resolved via
    # realpath first), the dm-N node it resolves to has no scheduler file of
    # its own (/sys/block/<dm>/slaves/ points at what does), and THAT may
    # itself be a partition needing the ordinary partition -> whole-device
    # step once more (confirmed live, 2026-08-28: gstammtisch--vg-root ->
    # dm-0 -> vda5 -> vda). A plain (non-LVM) partition or whole disk
    # resolves in the loop's first iteration exactly as upstream did.
    if not os.path.exists(f'/dev/{devname}') and os.path.exists(f'/dev/mapper/{devname}'):
        devname = os.path.basename(os.path.realpath(f'/dev/mapper/{devname}'))
    for _ in range(5):
        if os.path.exists(f'/sys/block/{devname}/queue/scheduler'):
            break
        parents = glob.glob('/sys/block/*/' + devname)
        if len(parents):
            devname = os.path.basename(os.path.dirname(parents[0]))
            continue
        slaves = glob.glob(f'/sys/block/{devname}/slaves/*')
        if len(slaves) == 1:
            devname = os.path.basename(slaves[0])
            continue
        break
