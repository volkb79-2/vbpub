...
**Version:** 8.0.0-draft.4 · **Status:** DRAFT (ground-up specification for CIU v8; companion to `CIU-V8-TESTING-GATE-PROPOSAL.md` revision 3.x; worked examples `v8-dstdns-demo/`) · **Date:** 2026-09-03
**Supersedes:** 8.0.0-draft.3 (2026-09-02) and draft.2 (2026-08-30). draft.3 was a fresh text after the first adversarial review (`CIU-V8-ADVERSARIAL-REVIEW-2026-09-02.md`, R-01..R-78): declaration files are plain TOML, consumers declare *bindings* instead of reading *routes*, secrets are structured tables, the contract of a LogicalService is derived, the instance lock is the checkout directory, and the gate has a zero-instance mode. draft.4 is draft.3 corrected in place after the independent third-party review (`CIU-V8-THIRD-PARTY-REVIEW-2026-09-02.md`, T-01..T-35; dispositions with reasoning in `CIU-V8-THIRD-PARTY-REVIEW-RESPONSE-2026-09-03.md`); every changed rule is listed in Appendix D with the finding that caused it.
...
### S4.5 Labels
S4.5.1 CIU stamps every container, network and named volume it creates with the fixed labels `ciu.project`, `ciu.instance` (the 6-hex id), `ciu.checkout` (the physical checkout path the id was derived from), `ciu.realization`, `ciu.service`, `ciu.replica` (when replicated), `ciu.managed-by=ciu`. `ciu clean`, `ciu instance reap`, `ciu diagnose` and the gate enumerate resources by these labels only, never by name pattern; the label namespace does not depend on any consumer setting, so a consumer change can never orphan a resource. S4.5.2 A template label whose key starts with `ciu.` is an ERROR (`[S4.5] label namespace ciu.* is reserved`). Consumer labels are authored in templates from `ciu_stack.<svc>.identity.*`. S4.5.3 **Ownership before deletion.** A mutating verb that removes or recreates a resource MUST verify that its `ciu.project`, `ciu.instance` **and** `ciu.checkout` labels equal this instance's; a resource carrying this instance's id but another checkout path is a 24-bit hash collision or a moved checkout and is refused with both paths (`[S4.5] container dstdns-98535c-vault is owned by /home/vb/other/dstdns, not this checkout; ciu instance init --move if the checkout moved`). `ciu instance init` in a checkout whose derived id matches running resources with the same `ciu.checkout` adopts them (the S2.3.4 recovery); with a different `ciu.checkout` it refuses unless `--move` is given.
...
|---|---|---|---|
| `image` | string `name[:tag][@digest]` | required | the one image declaration; without `build` the image is **pulled** (vendor-owned) |
...
- `[hooks]`: `pre_secrets`, `pre_compose`, `post_compose` — lists whose entries are either a script path (string, equivalent to `{ run = "<path>" }`) or a table with exactly these keys: `run` (path, required), `service` (a service key of this stack, optional; default = the primary service; the service in whose container the entry's facts are probed and which `hookkit.docker_exec` targets by default), `provides` (list of TypedFacts the script creates, default `[]`), `secrets` (list of this stack's secret keys the script may read, default `[]`, S12.2). Any other key is an ERROR (S3.8.1). `service` MUST name an enabled service of the stack (`[S6.10] hook entry names service 'minoi'; the stack has minio`).
...
4. `host(C) == host(R)` and the providing service is not `host_network`: `network = "instance"`, `host = identity(R, svc).container_name` (or service `compose_key` for replicated services), `port = e.port`. 4a. `host(C) == host(R)` and the providing service has `host_network = true`: it listens on the host's own interfaces, not on the instance network; `network = "host"`, `host = "ciu-host"` (CIU injects `extra_hosts: ["ciu-host:host-gateway"]` into the consumer's compose block, S11.4), `port = e.port`; a consumer that is itself `host_network` gets `host = "127.0.0.1"`. Cross-host consumers of a `host_network` provider continue with step 5 using `port = e.port` (nothing is published: the process already listens on the host's addresses; `host_port` and `host_bind` are refused on such an endpoint, S7.4.7).
...
### S8.5 Gates
S8.5.1 After bringing up wave `k`, CIU evaluates, per provider service `p` on this host, the **strongest predicate** requested by the edges into `p` from later waves: an edge from a `wait = "started"` binding requires `Running`; an edge from a `wait = "healthy"` binding, a `requires` entry, a `depends_on`, or a derived edge (S8.3) requires **healthy** (S8.6.3), which for a `one_shot` provider means **completed**; a binding's `facts` add the fact probes of S8.5.2. Each predicate must hold within `p.health.gate_timeout` (S6.6.3); timeout → ERROR naming `p`, the predicate, its last observed state, and the budget. The predicates evaluated are written as `[resolved.gates.<k>]` (S8.5.4). S8.5.2 Before starting wave `k+1`, every TypedFact required by an edge into `k+1` (binding `facts`) whose provider is on THIS host is probed (S5.6) with a bounded poll of the same budget; `starting` and `unreachable` are retried within the budget, `absent` is retried within the budget and reported as absent when it expires. S8.5.3 For a provider on ANOTHER host CIU probes reachability (TCP connect / TLS handshake / HTTP GET per endpoint protocol against the derived resolution) and takes the provider's facts and completions from that host's **receipt** (S17.4.3–S17.5); reachability alone never stands for a fact — without a valid receipt the fact is recorded as `assumed` and WARNed. `external` and `joined` providers' facts are **assertions** (`provides`), recorded as such; an `external` Realization MAY declare `probe = { fact = "http:/health" }`-style probes that CIU runs, and a `joined` provider's facts come from the reference's own receipt. S8.5.4 The gates are written as `[resolved.gates.<k>] healthy = [...]  completed = [...]  facts = [...]`. S8.5.5 **Acceptance.** `ciu up` succeeds only when every wave's gate passed, every service of every `ciu_stack` Realization brought up on this host is healthy per S8.6.3 at the end (a container with a healthcheck has reached `healthy`; one without is `Running`), every `one_shot` service completed with exit 0, and every `verify` fact of a selected LogicalService (S5.3.6) probed true — whether or not any consumer binds them. A selected variant service that never becomes healthy fails `up` even when nothing depends on it.
...
With `enabled = true`, CIU injects into every non-exempt service block the compose fields that Docker translates to the cgroup-v2 files — with the **conversions written here, because the compose fields are not the cgroup files**: `cgroup_parent`; `mem_limit` ← `memory_max`; `memswap_limit` ← `memory_max + memory_swap_max` (compose's field is the *total* of memory and swap, while `memory.swap.max` is swap alone; `memory_swap_max = "0"` therefore yields `memswap_limit = memory_max`, i.e. no swap; `"max"` on either side yields `-1`); `mem_reservation` ← `memory_low`; `cpu_shares` ← the inverse of Docker's cgroup-v2 mapping `weight = 1 + ((shares − 2) × 9999) / 262142`, i.e. `shares = 2 + (weight − 1) × 262142 / 9999`, rounded, so that the daemon writes the declared `cpu.weight`; `cpus` ← `cpu_max` (quota ÷ period as a decimal; `"max"` injects nothing); `blkio_config` ← `io_*`; `pids_limit` ← `pids_max`. `memory_high` has no compose field: CIU writes it to the container's cgroup immediately after start, and the interval between start and that write is uncapped by it (documented). `memory_min` is preflight-only (S13.2.1) and is never written to a cgroup. S13.3.2 After every start CIU reads back `memory.max`, `memory.swap.max`, `memory.high`, `memory.low`, `cpu.weight`, `cpu.max`, `io.weight`, `pids.max` from the container's cgroup directory and records them as `applied` (S13.4); a read-back that differs from the declared value beyond rounding is a WARN naming both. S13.3.1 KSM: `ksm_optin = "builtin"` builds the shim into `$XDG_CACHE_HOME/ciu/ksm/` and bind-mounts it; `wrapper`/`preload`/`off` per service (`CIU_KSM=off` disables for one run).
...
S16.6.1 **Admission is a transaction.** Under an exclusive `flock` of the `evidence_dir` directory descriptor, a lane reads the slice's current `memory.max`, sums the `memory_max` of every live reservation in `evidence_dir/.admitted/<lane>-<started>.json` (a reservation whose recorded pid no longer exists or whose start time differs from the pid's is stale and removed), and either writes its own reservation and releases the lock, or releases it and waits up to `--admission-wait` (default 10 m) before retrying; exhausted → NOT_RUN/`no-headroom`. The reservation is removed in a `finally`. Two lanes can therefore never both admit against the same headroom. A slice whose `memory.max` is `max` admits everything (admission is then a record, not a gate). S16.6.2 `ephemeral` lanes run with `--cgroup-parent <slice>` and their caps written to their cgroup before the process starts; `host` lanes run in a child cgroup of the slice. S16.6.3 `CIU_GATE_CGROUPFS_ROOT` overrides the cgroupfs mount for tests. S16.6.4 `exec` lanes run inside the target container's cgroup, and CIU cannot cap a single exec process below the container: their `resources` are **requested admission values only** — validated to be ≤ the container's effective governance (S13.4; `[S16.6] lane 'unit' asks memory_max 3G but 'tester' is capped at 800M`), counted against the container's `memory.max` for admission, and reported as `resources_requested` with `resources_applied = null` (S16.9); a lane that needs an enforced cap of its own uses an `ephemeral` environment. S16.6.5 A lane's LaneResult and history are written under an exclusive `flock` of `evidence_dir/<lane>/` (temp file + rename), so two invocations of one lane never interleave their writes or their pruning.

### S16.7 Assay invocation
S16.7.1 CIU's only interface to assay's lane file is `assay lanes --json --file assay.toml`, run in the lane's environment: it yields lane names, `base_source`, `external_tools`, `argv0`, `env_required`, `budget`. CIU MUST NOT parse `assay.toml` itself. S16.7.2 A lane runs `assay run <assay_lane> --file assay.toml --require-judge-provenance --verdict-json <evidence_dir>/<lane>/verdict.json --resume --progress <evidence_dir>/<lane>/progress.jsonl [--request-base <REF>]`, executed inside the environment with the checkout mounted at `workdir`; `--progress` takes the path of the progress file (run-gate R-38), which lives in the evidence directory, never in a hidden `.assay/`. `--request-base` is passed exactly when `base_source = "request"`; `REF` is `--base` if given, else the merge-base of `HEAD` and the checkout's upstream branch (`[S16.7] no upstream; pass --base`). S16.7.3 The lane's outcome is the verdict's `outcome`; `judge_provenance`, `helpers` (when present) and the resolved `REF` are copied into the LaneResult verbatim.
...
`{ api: "ciu/lane-result", api_version: 1, operation: "gate", status: "ok" | "findings" | "error", lane, kind, environment, project, instance_id?, layout?, started, ended, duration_s, outcome, reason?, exit_code, budget, resources_requested: {...}, resources_applied: {...} | null, members?: [<lane names>], request_base?, judge_provenance?, helpers?, verdict_path?, progress_path?, artifacts: [...], preconditions: {...}, provenance: { <service>: { image_revision, head, match } } }`. `status` is derived from `outcome`: `PASS` → `ok`; `FAIL`, `NOT_RUN`, `BUDGET_EXCEEDED` → `findings`; `ERROR` → `error` (S18.4). `resources_applied` carries the cgroup values read back after placement (S16.6.2) and is `null` for `exec` lanes, whose caps are requested only (S16.6.4). S16.9.1 The file is the last result; the same document is appended under `evidence_dir/<lane>/history/<started>.json`, pruned to `testing.history`. S16.9.2 `ciu gate --list` shows, per lane, the last outcome, duration and the median duration over the history. S16.9.3 A LaneResult and every artifact written for a non-PASS outcome are mode 0600 (evidence may contain secrets echoed by a failing test).
...
S17.3.1 **Release.** `ciu push --layout L [--host H]` builds, per target host `H`, a **release**: the closure of files the host needs — every declaration file, `ciu.hosts.toml`, every stack directory placed on `H` (templates, hooks, config-file templates, init scripts and every file a template or hook references by a repo-relative path), `ciu.instance.toml`, `ciu.instance.generated.toml` (with the realness records of other layouts stripped, S9.4.4) — and a **manifest** `ciu.release.json` (`api = "ciu/release"`, `api_version = 1`) listing every file with its SHA-256 and mode, the image references and digests of the Realizations placed on `H` (S17.6), the layout, the instance id, and the git revision. The release digest is the SHA-256 of the manifest. `bundle_excludes` MAY subtract only files outside the closure; an exclusion that intersects it is an ERROR (`[S17.3] bundle_excludes drops infra/db-core/init-scripts, referenced by infra/db-core/ciu.compose.yml.j2`). S17.3.2 **Secrets capsule.** Alongside the release the sender builds, per host, a **capsule** `ciu.secrets.transport.toml` (0600) whose entries are derived per source for every secret of a Realization placed on `H` (stack-level and derived TLS secrets included): `from = "generate", store = "local"` and `from = "ask"` → the stored value; `from = "file"` → the file is **read now on the sender** into the capsule (the store never held it); `from = "host"` → the entry `[secrets.hosts.<H>.<entry>]`, materialized on the CIU host first (S10.4); `from = "hook"` → the stored value if the sender's hook produced it, else nothing (the target's hook produces it); `from = "ephemeral"` and `delivery = "native"` → nothing; `from = "vault"` / `store = "vault"` → nothing when `H` has a derived resolution for the pseudo-consumer `ciu@H` to the `vault` LogicalService (the target fetches, S10.3), else the sender fetches the value **now** and puts it in the capsule — if the sender cannot reach Vault either, `push` refuses naming the host, the key and both failed reachabilities (`[S17.3] rs1002 has no route to vault and this host cannot reach it: db/postgres/controller_password cannot travel`). `ciu check --layout L` prints, per host, every capsule entry and why it travels (S15.4). S17.3.3 **Transfer.** Release and capsule are transferred with `push_mode` to `<bundle_dir>/releases/<digest>.staging/` on the target, in layout order; the target verifies every manifest entry (size, SHA-256, mode) and the manifest's own digest, refuses on any mismatch, renames the directory to `<bundle_dir>/releases/<digest>/` (atomic), and imports the capsule into its store with `source = "transport:<from>[:<detail>]"` and `created` = the sender's timestamp — an imported entry is **never refreshed** on the target (S10.6.4 applies only to entries this host resolved itself) and is replaced only by the next capsule. An interrupted transfer leaves a `.staging` directory that the next push overwrites; `current` is never touched by `push`. S17.3.4 The instance file and generated file travel so that `instance_id` is shared; the target regenerates `[ciu.host.generated]` (S14.2.3). S17.3.5 `ciu push --prune` removes staged and non-current, non-previous releases on the target.

### S17.4 Activate: atomic switch, receipts, rollback
`ciu activate --layout L [--host H] <bootstrap|apply|health|rollback>` runs the host's `[activate]` command over SSH, in layout order, stopping at the first failure. S17.4.1 `apply` runs in `<bundle_dir>/releases/<digest>/` and, on success, atomically switches the symlink `<bundle_dir>/current` to that release and records the former target as `<bundle_dir>/previous`; `bootstrap` (`ciu instance init --host <H> && ciu check --layout L`) and `health` run in `current`; `rollback` switches `current` back to `previous` and runs the previous release's own `apply` command — never a bare `ciu down` unless the host declares that as its rollback. S17.4.2 Render-on-target: each host renders its own `ciu.resolved.toml` from the release with `[ciu.host.generated].name = H`, so derived resolutions and publications are computed per host. S17.4.3 **Receipts.** A successful `ciu up` writes `ciu.receipt.json` (`api = "ciu/receipt"`, `api_version = 1`) next to the rendered file: `instance_id`, `host`, `layout`, the release digest (when running from a release) and the SHA-256 of `ciu.resolved.toml`, the selection (`resolved.services`), every service brought up with its final state, every completed `one_shot`, every fact probed true (S8.5.2, S8.5.5) with its provider service, every endpoint published, and `created`. `ciu activate apply` fetches the receipt after each host and passes the receipts collected so far to the next host's `apply` as `--receipts <dir>`; `ciu up --receipts` accepts a directory of receipts. S17.4.4 A receipt is **valid** for a consumer host when its `instance_id` equals the consumer's, its layout equals the consumer's, and its resolved-file digest equals the digest of the consumer's own render of the provider host (the consumer renders every host of the layout from the same release, S3.7.3); a receipt from another release, another instance or with a different selection is refused with the reason.

### S17.5 Cross-host runtime and evidence
CIU on a host deploys only the Realizations placed on that host (S7.7). For a provider on another host it accepts a fact or a completed one-shot **only** from a valid receipt (S17.4.4) that lists it; reachability of the derived resolution is still probed (S8.5.3). Without a receipt for the provider host — a hand-started `ciu up`, or no `--receipts` — CIU probes reachability, marks the fact `assumed` in `[resolved.gates.<k>]` and in its own receipt, and WARNs (`[S17.5] pg:role/controller on gstammtisch assumed from reachability; no receipt`); `ciu up --require-receipts` turns that into an ERROR. A host never opens SSH or `docker exec` to another host during `up`.

### S17.6 Build provenance
`ciu build` builds every service that declares `build` (S6.2) from its context, stamps the image with `org.opencontainers.image.revision` = the git commit (`-dirty` appended when the tree is dirty) and `org.opencontainers.image.created`, tags it as the service's `image`, and records `[ciu.instance.build]` (`build_version` = `<date>-<git short sha>`, `build_time`, image digests); every other image is pulled and recorded by digest, never built. An `image` that no service builds but whose reference carries `project.registry.namespace` is a WARN (`[S17.6] dstdns/controller:latest is not built by any service; is a build table missing?`). `ciu provenance [--json]` compares every running container of the instance against `HEAD` (the v7 S17 verdict vocabulary `match | dirty | mismatch | vendor | unstamped`).
...
| `ciu instance list \| show [<label>] \| add --join <ref> --services … \| remove <label> \| reap \| lease --extend [D] \| --perpetual \| --release \| exec --env <e> -- <cmd> \| backup [--to DIR] \| restore --from DIR` | list/show/exec/backup read-only; others mutating | S14.6, S14.7, S9.5.5; `backup`/`restore` copy the non-regenerable files of S2.3.4; `ciu worktree …` accepted as an alias for one release |
| `ciu query identity \| binding \| publications \| capability \| lanes … [--json]` | read-only | S3.7.6 |
| `ciu testing flatten` | read-only | S16.2.1 |
...
## Appendix D — Changes in draft.4 and why (third-party review T-01..T-35)
Each row names the finding, the rule(s) changed, and the reasoning; the full dispositions are in `CIU-V8-THIRD-PARTY-REVIEW-RESPONSE-2026-09-03.md`.
