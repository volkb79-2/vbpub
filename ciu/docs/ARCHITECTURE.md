...
| `worktree.py` | Managed Git-worktree identity/state records, create/adopt/ensure/remove lifecycle, shared infrastructure, cross-instance deployment budget, structured JSON documents + capability allowlist (S16.4/S16.5), and exact selected-worktree `up`/`exec` (S16.6) | `create`, `adopt`, `ensure`, `remove`, `inspect_instance`, `list_instances`, `remove_document`, `capabilities_document`, `up_instance`, `exec_instance`, `resolve_worktree_cap`, `worktree_budget_slot` | S16 | `test_ciu_worktree.py`, `test_ciu_engine_worktree_budget.py`, `test_ciu_documentation_contract.py` |
