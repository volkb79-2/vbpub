...
| `workspace_env.py` | Identity-record read/write (`[ciu.instance.generated]`, S3.1c) + `ciu.env` generation and machine-fact loading; DooD detection; network creation/attach; TLS probe | `read_generated_facts`, `has_generated_facts`, `seed_identity_env`, `bootstrap_workspace_env`, `ensure_workspace_env`, `resolve_env_root`, `validate_required_certs` (`load_workspace_env` remains for the legacy export's own consumers and is NOT the bootstrap path — see its docstring) | S1.1–S1.5, S2.1–S2.8, S3.1c | `test_ciu_workspace_env_v2.py`, `test_ciu_identity_cutover_ciu75.py` |
