...
|---|---|---|---|---|
| `engine.py` | Single-stack CLI entry point; implements the 17-step pipeline [S8.3]; hostdir creation; DooD preflight; `ciu secrets`; `--shipped`; real-Compose-only worktree budget enforcement | `main_execution`, `run_shipped`, `create_hostdirs`, `reset_service`, `privileged_fs_op`, `secrets_command`, `main` | S1–S6, S8–S11, S16.3 | `test_ciu_test_repo.py`, `test_ciu_engine_worktree_budget.py`, `test_ciu_reset_service.py`, `test_ciu_hostdir_creation.py`, `test_ciu_shipped.py` |
...
| `workspace_env.py` | Identity-record read/write — `[ciu.instance.generated]` in its own `ciu.instance.generated.toml` since ciu-P47 (S3.1b/S3.1c) — plus `ciu.env` generation and machine-fact loading; DooD detection; network creation/attach; TLS probe | `generated_facts_path`, `write_generated_facts`, `generated_facts_document`, `read_generated_facts`, `has_generated_facts`, `seed_identity_env`, `bootstrap_workspace_env`, `ensure_workspace_env`, `resolve_env_root`, `validate_required_certs` (the two readers are deliberately distinct: `generated_facts_document` is the TOLERANT whole-document merge view `render_global_chain` layers in, `read_generated_facts` the STRICT identity view that refuses a malformed record — see both docstrings and SPEC S3.1b clause 5. `load_workspace_env` remains for the legacy export's own consumers and is NOT the bootstrap path — see its docstring) | S1.1–S1.5, S2.1–S2.8, S3.1b–S3.1c | `test_ciu_workspace_env_v2.py`, `test_ciu_workspace_env.py`, `test_ciu_identity_cutover_ciu75.py` |
...
**Deleted functions**: `engine.extract_service_definitions` (SPEC S3.8 amended to withdraw it — the global `[service.X.Y.Z]` extraction pattern is not part of the current model).
...
```
operator / nyxloomd
  └─ ./ciu/run-gate.py ciu (host; clean-tree check)
       └─ resolve CGROUP_PARENT_DEV_GATES from the root gate config
            └─ detached tester-unified container in that cgroup slice
                 ├─ install Assay from the selected worktree's assay/
                 └─ assay run ciu --resume --progress .assay/progress-ciu.jsonl
                      ├─ Assay snapshot (repository-minus-unsafe-symlinks)
                      ├─ lane argv: python -m pytest (100% line+branch)
                      ├─ R2 liveness required; argv matches
                      │  run-ciu-tests.py's pytest_args()
                      └─ R1 judges base..HEAD changed-line floor
                           → .assay/verdict-ciu.json (gitignored evidence)
```

CIU source never imports Assay and never runs a nyxloom coverage/mutation/
canary judge; run-gate installs Assay from the selected worktree and the Assay
CLI is the sole evidence judge.
