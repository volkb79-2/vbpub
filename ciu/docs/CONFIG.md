...
```
┌─────────────────────────────────────────────────────────────────┐
│  Layer 1 — Machine identity    (.env.ciu)                       │
...
│  Generated by CIU: <stack>/.ciu/docker-compose.ciu.yml         │
└─────────────────────────────────────────────────────────────────┘
```
...
| File | Status | Purpose |
|---|---|---|
| `ciu-global.defaults.toml.j2` | Committed | Canonical global defaults; used as-is if no override exists |
| `ciu-global.toml.j2` | Gitignored, auto-created | Override layer; auto-copied from defaults when missing |
| `ciu-global.toml` | Gitignored, rendered | Runtime global config; read by `ciu-deploy` |
| `<stack>/ciu.defaults.toml.j2` | Committed | Stack defaults |
| `<stack>/ciu.toml.j2` | Gitignored, auto-created | Stack override layer |
| `<stack>/ciu.toml` | Gitignored, rendered | Runtime stack config; `[state]` preserved across re-renders |
| `<stack>/docker-compose.yml.j2` | Committed | Compose template (Jinja2) |
| `<stack>/docker-compose.yml` | Gitignored, rendered | Runtime compose file |
| `<stack>/.ciu/docker-compose.ciu.yml` | Gitignored, generated | Overlay: secret-file sources + configfile bind-mounts |
| `**/.ciu/` | Gitignored | All CIU artifacts; never hand-edited [S1.6] |
...
## `.env.ciu` Key Provenance Table [S2.7]
