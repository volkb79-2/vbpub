...
sha256sum -c ciu/tools/assay/assay-5.2.0.pyz.sha256

# 2. Inspect the declared lane (validates config, runs nothing)
cd ciu && .venv/bin/python tools/assay/assay-5.2.0.pyz lanes --file assay.toml
...
cd ciu && mkdir -p .assay && \
  .venv/bin/python tools/assay/assay-5.2.0.pyz run ciu \
