...
**Status:** **IMPLEMENTED on the v7 line 2026-09-08** (ciu-P52 / CIU-93 — `ciu host enroll`, `SPEC.md` S14.7; the cmru half **KI-24** shipped in cmru 5.1.0). The v8 form (`SPEC-V8.md` draft.7 S7.2.4, carve row V8-29) is still unimplemented. Accepted in shape by the operator on 2026-09-03 for both lines.
**Corrections found while implementing (the design below is otherwise unchanged):** (1) the `authorized_keys` restriction is written `from="PATTERN" <type> <base64> <comment>` — **whitespace, never a comma** before the key type; the comma form is rejected by a real sshd (measured live in cmru KI-24, see `SPEC.md` S14.7b). (2) §3.3's install URL is real — `get.py` ships as a release asset — but ciu publishes only a wheel, so the *install* step of `get.py enroll` has no `ciu-v<version>.tar.xz` bundle to download yet; that is ciu **CIU-99**, and `--no-install` is the reachable half until it ships.
