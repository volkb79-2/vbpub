...
oracles:
  - id: O1
    ...
    gate: tester-unified
  - id: O2
    observable: "CIU-22/S16.1. After a successful non-dry-run `docker compose up`, both native `engine.main_execution` and `engine.run_shipped` parse the complete intent from the new worktree's already-loaded environment and call `worktree.connect_shared_infra_after_up(repo_root, compose_project, intent)`. It re-resolves the recorded absolute REF against `git worktree list`, re-reads that worktree's explicit `ciu.env`, and refuses if the registered path disappeared or its network no longer equals the recorded network. It requires a running container for EVERY operator-declared reference Compose project on that network (and refuses a recorded reference project equal to the current project); this proves only that something carrying each declared label is running, not that R was derived from the reference path. It obtains every *running* container of each declared diverging service from THIS compose project with Docker's `com.docker.compose.project=<compose_project>` and `com.docker.compose.service=<service>` label filters, and fails before any connect if any declared service has zero running containers. It then snapshots the reference network membership and calls `docker network connect <recorded-network> <container-id>` only for declared-service containers absent from that snapshot. On EVERY non-zero connect result, it re-inspects that network's membership for the target ID: present is a SUCCESSFUL concurrent no-op (do not raise and do not add the ID to rollback because this invocation never connected it); absent is a genuine connect failure. This state check, not Docker diagnostic text, is the complete predicate. Thus a second `ciu up` with all memberships already present succeeds without a connect call. Neither `DOCKER_NETWORK_INTERNAL` nor the base compose/overlay network declarations change; the declared diverging services gain a SECOND membership and all other new-instance containers remain only on their own instance network."
    negative: "Putting the join in a compose `networks:` overlay, attaching every container in the compose project, attaching a reference-tier container, changing `DOCKER_NETWORK_INTERNAL`, joining before Compose succeeds, parsing/matching Docker's human-facing error text, treating a non-zero result whose re-inspection finds the target present as an error, treating a non-zero result whose re-inspection finds the target absent as a no-op, or adding a concurrent no-op to rollback each fail this oracle. The no-socket gate monkeypatches `worktree.procutil.docker` and the compose executor: a combined-axis test with two requested services, one already attached, and a third unrequested service proves exactly one connect is issued and the unrequested service is never named; separate non-zero replies with target-present and target-absent re-inspections prove the state predicate and its failure boundary."
    gate: tester-unified
  - id: O3
    observable: "CIU-22/S16.1 failure and terminal-state contract. A reference that was live at `worktree add` but has no declared-Compose-project-labelled container at post-up revalidation, a recorded reference path no longer registered, a changed reference network, an absent declared target service, a non-zero Docker inspect result, or a non-zero connect result whose re-inspection still finds the target absent makes `ciu up` fail with an `[S16.1]` error naming the reference path/network or target service. No connect is attempted before all reference and target preconditions pass. If a later connect fails after earlier new memberships were made, CIU disconnects only IDs whose connect returned zero in this invocation, in reverse connection order; memberships in the pre-connect snapshot and non-zero-but-now-present concurrent no-ops are never removed. CIU does not run `docker compose down` on this failure: its own stack remains up on its OWN network, clearly not joined, so the operator may restore the reference and retry or explicitly `ciu down`. Dry runs never inspect or connect Docker networks."
    negative: "A ref-down race that silently leaves a running but unjoined instance, OR-combining multiple declared reference projects, a missing second target that still lets the first target connect, a partial-connect failure that leaves CIU-added memberships behind, rollback derived from snapshot absence rather than zero-return connects in this invocation, rollback that removes a pre-existing membership or a non-zero-but-now-present no-op, or dry-run Docker traffic each fail this oracle. Tests use ordered fake Docker replies to assert preconditions precede every side effect, to witness reverse-order rollback, and to distinguish a pre-existing membership from one created by this invocation."
    gate: tester-unified
...
escalate_if:
  ...
  - "The Docker seam cannot distinguish reference-not-running, target-service-absent, a non-zero connect followed by target-present re-inspection, and target-absent genuine connect-failure/rollback paths in `tester-unified` without a Docker socket."
mutexes: [merge-lane]
review_focus:
  - "Reject any route that mutates compose network declarations or replaces the worktree's own network identity."
  - "Attack all-declared-project liveness, state-based concurrent-connect detection, idempotent rerun, and rollback ownership."
---
...
3. Inspect `intent.network` once and derive its current container names. For
   ...
   <intent.network> <container-id>`. Existing membership is success/no-op. On
   **every** non-zero connect result, inspect `intent.network` again and test
   membership for that same target ID. If it is now present, another actor
   joined it between the snapshot and connect: treat this as success/no-op and
   do not record it as this call's work. If it remains absent, it is a genuine
   failure. Do not parse Docker's diagnostic text for either outcome.
4. If a genuine connect failure occurs, disconnect only IDs whose connect returned zero
   ...
   or a non-zero-but-now-present concurrent no-op.
...
| Up: target absent in snapshot, then non-zero connect and re-inspection finds it present | Success | No disconnect/rollback entry for that target |
| Up: target absent in snapshot, then non-zero connect and re-inspection finds it absent | `[S16.1]` error | Reverse-disconnect only earlier zero-return connects |
...
|---|---|---|---|
| CLI and add recording | `worktree.py`, `cli.py` | O1 | Fake docker marks a ref live; parse child env and compare every field. Required masquerader fixture: the ref network has one running container labelled with an undeclared project, while every declared R query is empty; it MUST refuse before git-add, proving a bare labelled-container count is not liveness. Required all-R fixture: two declared R projects, exactly one with a running labelled container; it MUST also refuse before git-add, proving liveness is AND-combined. Break: missing profile/service/ref must make no git-add call. |
| Ref liveness and target selection | `worktree.py`, `engine.py` | O2 | Fake Compose succeeds; fake Docker provides two selected services, one already attached, and one unselected service. Assert exactly the absent selected ID is connected. Required concurrent-join fixture: a target is absent from the one membership snapshot, its connect returns any non-zero result, and the next membership inspection includes it; assert success, zero disconnects, and no rollback-list entry for that target. Required genuine-failure fixture: a target's connect returns a different non-zero result and re-inspection still excludes it; assert `[S16.1]` and ordinary rollback. Neither fixture may assert or depend on Docker diagnostic text. |
| Failure cleanup | `worktree.py`, `engine.py` | O3 | Ordered fake replies cover ref-down-after-add, absent second target, failing second connect, and failing rollback; assert no premature or over-broad disconnect. Required three-target rollback discriminator: target A is in the pre-connect membership snapshot, B is absent and its connect returns zero, C is absent and its connect fails with re-inspection still absent; assert the disconnect set is exactly `[B]` (never A or C). This proves the rollback list comes only from zero-return connects in this call, not every target absent at snapshot. |
