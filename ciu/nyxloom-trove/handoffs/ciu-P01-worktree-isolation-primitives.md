...
- **The fixtures are regenerated canonically and the comparison is now
  ...
  five (measured). All six are now exact `json.dumps(indent=2)` output, and O1's
  instruction is **"compare PARSED documents"**, not byte-for-byte — whitespace
  is out of the contract. A sixth fixture covers the docker-unavailable case.
...
Six documents, one per `overall` value. They are exact `json.dumps(indent=2)`
output plus a trailing newline. O1's test compares **parsed** documents, never
bytes; these SHA-256 values exist to detect an implementer editing a fixture in
place instead of fixing the tool (the round-2 "frozen but writable"
contradiction):
...
subsections in `docs/SPEC.md`), it does not re-plan the roadmap. The six
