...
|---|---|---|---|
| `infra/vault` | `vault_core` | `hashicorp/vault:1.17` (DEV mode) | bootstrap provider; `GEN_LOCAL` root token (a vault stack MUST NOT use `*_VAULT`, **S7.6**); S4.18 entrypoint wrapper; `post_compose` hook persists ONLY the non-secret `initialized` flag to `[state]` (**S9.4**) — the root token is `GEN_LOCAL`-declared and therefore already materialized safely by the ordinary S4 machinery, so a `[state]` copy would be a redundant second exposure (**S3.4a**) and `persist:"secret"` for that name is refused as an S4.6 collision; later stacks reach it via `[vault].token_file`, **S4.16** source #2; root key renamed off the reserved `vault` namespace (**S3.7 / B.2**) |
