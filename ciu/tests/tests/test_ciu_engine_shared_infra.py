...
def _base_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # PRIOR test's "with_intent=True" run can otherwise leak these four keys
    # into this one regardless of test order. Pre-registering them with
    # monkeypatch (even as a no-op delete) makes monkeypatch own their
    # teardown, so this test both starts and ends clean.
    for key in (
...
def _write_ciu_env(tmp_path: Path, *, with_intent: bool) -> None:
    body = "\n".join(
...
def _write_native_repo(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, *, with_intent: bool,
    global_defaults: str = GLOBAL_DEFAULTS,
) -> Path:
    _base_env(tmp_path, monkeypatch)
    (tmp_path / "ciu.global.defaults.toml.j2").write_text(global_defaults)
    _write_ciu_env(tmp_path, with_intent=with_intent)
