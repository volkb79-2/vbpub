...
This accessor mirrors ``service_shipped``/``service_health_enabled``'s exact
validation pattern: absent -> None; a string is returned verbatim (duration
parsing is the CALLER's job, via the existing ``deploy._seconds()`` helper —
this accessor never parses); any other type -> a tagged ``[S7.2]`` ValueError.

NOTE: as of this package, ``resolve_selection_health_containers`` does not
yet call this accessor (see this package's LOG — the deploy.py wiring (O3)
is BLOCKED because it would break several existing tests outside this
package's ``scope.touch``). This file tests the accessor itself, in
isolation, per the handoff's Work item 1.
"""
...
from ciu.deploy_pkg.phases import service_health_timeout  # noqa: E402
