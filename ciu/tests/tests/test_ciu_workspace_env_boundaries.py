...
class TestWorkspaceEnvParsingAndDiscovery:
    ...
    def test_find_walks_from_invocation_and_ignores_ambient_repo_root(self, tmp_path: Path, monkeypatch) -> None:
        nested = tmp_path / "tree" / "deep"
        nested.mkdir(parents=True)
        expected = _write_env(tmp_path / "tree", "REPO_ROOT=/walked\n")
    ...
    def test_resolve_explicit_root_requires_existing_directory(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(WorkspaceEnvError, match=r"\[no-ciu-root\]"):
            resolve_env_root(tmp_path, missing, "marker")

    def test_resolve_walks_to_marker_or_refuses(self, tmp_path: Path) -> None:
        root = tmp_path / "root"
        nested = root / "one" / "two"
        nested.mkdir(parents=True)
        ...
        with pytest.raises(WorkspaceEnvError, match=r"\[no-ciu-root\]"):
            resolve_env_root(no_marker, None, "marker")
