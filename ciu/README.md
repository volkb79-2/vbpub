...
- **nyxloom** can put parallel implementation tasks in CIU worktrees. Each
  worktree receives a durable logical identity, exact CIU-root translation,
  local sparse override, Compose network, volumes, and optional
...
|---|---|---|
| **Plain compose** | a hand-written, committed `docker-compose.yml` | works with `docker compose up` — or `ciu up --dir <stack> --shipped` to add env/network/preflight |
...
4. **DooD / path correctness for free.** CIU computes physical bind paths so a stack runs identically in a devcontainer, native host, and CI (S1.9). A hand-written `./vol-data` mount silently breaks under docker-outside-of-docker. Every compose invocation also passes `--project-directory <repo root>` (S8.1a) so a stack's own relative paths — a `build.context` chief among them — resolve the same repo-root-relative way every other CIU path already does, instead of against the compose file's own directory (`docker compose`'s undocumented-to-authors default).
