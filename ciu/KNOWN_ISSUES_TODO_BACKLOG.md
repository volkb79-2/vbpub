...
| CIU-62 | `worktree.py`'s `_resolve_budget_candidates` (S16.3) guards its sibling-`ciu.env` read with `except (OSError, WorkspaceEnvError)`, which does NOT cover a non-UTF-8 byte: `UnicodeDecodeError` derives from `ValueError`, not `OSError`, and `WorkspaceEnvError` is a different `ValueError` subclass. So a single bad byte in ANY registered sibling worktree's `ciu.env` escapes as a raw traceback out of `ciu up` — defeating the function's own documented contract that "everything else that cannot be resolved truthfully is a loud `[S16.3]` failure" | Low | OPEN — found by ciu-P33's second review round, while applying that same `(OSError, WorkspaceEnvError)` pattern to `cli._env_print` on the reviewer's instruction and discovering it left one of the three named cases open. `_env_print` closed it by naming `UnicodeDecodeError` explicitly in the tuple; this call site (a different subsystem, and an unrequested change mid-fix-round) was filed rather than edited. Fix is the same one token. Worth grepping for other `except (OSError, WorkspaceEnvError)` / `except OSError`-around-`read_text` pairs at the same time — this is a class, not a site |
...
## CIU-56 — `CIU_KSM` missing from the autouse ambient-env scrub fixture

**Filed by:** controller, 2026-08-25, while verifying the CIU-55 fix
...
(CIU-55's fix): `test_absolute_governance_ksm_path_is_preserved_in_overlay`
...
## CIU-57 — shared, on-disk `test-repo/` fixture source is not test-isolated, races under parallel workers

**Filed by:** controller, 2026-08-25, found live while stress-testing the
CIU-55/CIU-56 fixes with repeated full gate runs — not caused by either.
...
This is a **structurally different** problem from CIU-56 (no shared env
