...
they ship, so it now certifies scaffold templates under weaker semantics than
the production pipeline that actually consumes them. Not fixed here; needs
ciu's own scaffold templates rendered and checked for a legitimate undefined-
variable use before `StrictUndefined` can be adopted safely.
