...
Last updated: 2026-08-31 — **CIU-75 FIXED (ciu-P42, after a round-1 REJECT
completed the cutover and a round-2 docs sweep); CIU-82, CIU-83, CIU-84,
CIU-85 FILED.** The v8 F2
...
| CIU-83 | ciu-P43's four items (CIU-77/79/80/81) landed with SPEC/CONSUMERS/CONFIG updates but no `CHANGES.md` entry at all — `git show 815c50d6 --stat` does not list the file. `## [7.7.0]` is the section all five of this checkpoint's items ship in, so as it stands the release notes describe CIU-75 only, while CIU-79 is BREAKING by ciu-P43's own merge message: two breaking changes, one announced | Medium | OPEN — filed by ciu-P42, 2026-08-31, found while rebasing onto ciu-P43. Not fixed there: those entries are ciu-P43's to write, and writing another package's changelog from its merge message is how release notes become fiction. Must be closed BEFORE 7.7.0 is tagged; the releaser checks the section against the checkpoint's merge list, not against one package's report |
| CIU-84 | `ciu check --json` writes `[INFO]` to stdout before the JSON document, breaking a naive `\| jq` — S13.4a and `_emit_check_report`'s docstring both say the document is the only thing the action writes, and `action_check` honors that, but `_run` reaches it past `deploy.py:4322`'s unconditional `info(f"Active service profile(s): …")` | Medium | OPEN — filed by ciu-P42 review round 2, 2026-08-31. **Pre-existing, not caused by CIU-75**: that package's own contribution to this class (the deprecation notice on the bootstrap-regeneration path, which appeared ahead of the document for anyone who followed its migration advice) was fixed in its round 1. Widening the fix to unrelated emitters was left out of a release already carrying a breaking change. Needs a sweep of every stdout write reachable on the `--json` path plus a test asserting stdout parses as ONE document |
| CIU-85 | `worktree._clean_in` (`:1267`) builds its child environment as `dict(os.environ)` + the target's identity, without the `_CIU_IDENTITY_ENV_KEYS` strip its two siblings (`_sanitized_target_env:2962`, `_resolve_budget_candidates:4406`) perform — so the caller's `CIU_SERVICES_PROFILE`, which is in that tuple but is NOT an overlay fact and therefore not in the overwrite, leaks into the child `ciu clean`. Separately, `PUBLIC_FQDN` is missing from `_CIU_IDENTITY_ENV_KEYS` despite being one of the six identity facts since CIU-47 | Low | OPEN — filed by ciu-P42 review round 2, 2026-08-31 (reviewer-reported, confirmed at source). Neutralized in practice — `_clean_in` refuses on an empty table and CIU-75's STEP 1 override-seeds all six keys anyway — but incidentally, not by design. Fix is the strip plus deriving the tuple's identity half from `GENERATED_FACT_ENV_KEYS` rather than maintaining a second hand-written list |
...
## CIU-84 — `ciu check --json` still writes `[INFO]` to stdout ahead of the document (S13.4a)
...
## CIU-85 — `_clean_in`'s child environment skips the identity strip its two siblings perform, and `PUBLIC_FQDN` is missing from `_CIU_IDENTITY_ENV_KEYS`
