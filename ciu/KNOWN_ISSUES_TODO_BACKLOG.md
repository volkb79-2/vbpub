...
completed the cutover); CIU-82, CIU-83 FILED.** The v8 F2
...
**Shell (verified, and it corrects the first audit's inventory).** There is NO
`grep`/`sed`/`awk`/`cut` extraction from `ciu.env` anywhere in dstdns — every
site is a whole-file `source`, which keeps working: `scripts/ciu-env.sh:66`
(the canonical loader, asserting `REPO_ROOT`/`PHYSICAL_REPO_ROOT` at `:68`;
sourced by `scripts/devcontainer-exec.sh:28` and
`scripts/admin-debug-exec.sh:26`), `.vscode/run-ciu-render.sh:12`,
`.vscode/run-ciu-render-all.sh:13`, `.vscode/run-deploy.sh:16`,
`.vscode/copilot-cmd.sh:53`, `env-workspace-setup-generate.sh:256`, and
`.devcontainer/finalize.post.d/10-dstdns-ciu.sh:29` — the last also writing
the `~/.bashrc` block (`:68`) that sources `ciu.env` into every interactive
shell. The one grep-the-file recipe is in a HANDOFF DOC
