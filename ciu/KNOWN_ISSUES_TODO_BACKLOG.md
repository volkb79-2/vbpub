...
| CIU-58 | Multiple tests build their fixture tree via `shutil.copytree` FROM the real, checked-in `test-repo/` directory (a SHARED, on-disk, non-per-test-isolated source) rather than from a synthetic/generated source — a concurrent xdist worker rendering into (or otherwise mutating) that same shared source directory races the `copytree` read, observed as a `shutil.copytree`/`os.scandir` failure over entries that include unexpected generated artifacts (`ciu.compose.yml`, `__pycache__`) alongside the template files | Medium | FIXED 2026-09-08 by **ciu-P51**, together with CIU-91 (read that row for the lock mechanism and for why `pytest.mark.xdist_group` could not be used). This row's own ask was the ENUMERATION — every consumer of a shared, non-`tmp_path` `test-repo/` path, not just the two files already known to collide. Full audit of all 11 test files mentioning `test-repo`/`TEST_REPO`, with the verdict for each: **WRITERS (mutate in place, now `LOCK_EX`)** — `test_ciu_test_repo.py`, 7 tests (`test_bootstrap_workspace_env_generates_env_file`, `test_render_global_and_stack_configs`, `test_app_config_full_pipeline_runs_under_dry_run`, `test_app_config_secrets_list`, `test_deploy_profiles_and_phases_match_spec`, `test_shipped_profiles_filter_flags_and_deduplicate_extra_stacks`, `test_workers_stack_configfile_fans_out_and_dev_profile`) — `_bootstrap` writes `test-repo/ciu.env` and the renders create/`_clean_stack_artifacts`-unlink `ciu.toml`/`ciu.toml.j2`/`ciu.compose.yml`/`.ciu/`/`vol-*` in the committed stacks. **DIRECTORY-WALKING READERS (now `LOCK_SH`)** — `test_ciu_render_selection_context.py`'s `_add_stack`/`_identity_probe_stack` (4 tests; the confirmed CIU-91 victim, and the one `copytree` with NO `ignore` filter at all); `test_spec_contracts.py`'s `add_stack`/`build_repo` (module-level mark, 78 tests); `test_ciu_identity_cutover_ciu75.py`'s `verb_repo` fixture (6 consumers, locked via the fixture since a fixture cannot carry a mark); `test_ciu_test_repo.py::test_deploy_render_all_configs_respects_phases`, which `copytree`s the WHOLE `test-repo/` tree including `ciu.env` (marked reader, not writer — it works entirely in `tmp_path` afterwards). Note that `test_spec_contracts.py` and `test_ciu_identity_cutover_ciu75.py` DO pass an `ignore=` callback that drops `ciu.toml`/`ciu.compose.yml`/`.ciu`/`__pycache__`/`vol-*` — but NOT `ciu.toml.j2`, which `_clean_stack_artifacts` also unlinks and which a prior render auto-creates, so both were genuinely racy on that one file even though they looked filtered. **NOT RACY, deliberately unmarked** — `test_hook_interfaces.py`, `test_ciu_shipped_hook_contracts.py`, `test_ciu_templates.py` and `test_ciu_test_repo.py::test_test_repo_exists`/`test_detects_standalone_root` read only COMMITTED paths (`ciu.defaults.toml.j2`, `config.toml.j2`, `pre_compose_app.py`, `post_compose_vault.py`, `ciu.global.defaults.toml.j2`, `files/demo-ca.pem`, `standalone/project/`) that no writer ever unlinks, via point `read_text`/`exists` calls rather than a directory walk — there is no TOCTOU window to close; `test_ciu_provisioning_ciu70_probe_container.py`, `test_ciu_deploy_actions.py` and `test_ciu_composefile.py` only MENTION `test-repo` in comments and never touch it. **The heavier fix this row proposed — (a) one pristine per-session copy, or (b) a synthetic fixture tree — was deliberately NOT done** (out of ciu-P51's scope by its own contract): the lock closes the confirmed TOCTOU mechanism and every same-shape collision the enumeration above found, so it is a complete fix for the class as currently understood rather than a stopgap. The one hazard it does NOT cover is the one this row already named as speculative — a NON-TEST process (an interactively-run `ciu` command against this same tree) mutating it concurrently; the lockfile is keyed by the resolved `test-repo/` path rather than per-run, so two concurrent pytest runs against the same checkout DO serialize, but a bare `ciu` invocation takes no such lock. Original filing follows: found live while stress-testing the CIU-56/CIU-57 fixes (2026-08-25), not caused by either; not investigated further — a structurally different, likely broader test-fixture-isolation problem across the suite, out of scope for this wave. A future package should enumerate every `copytree`/direct-read use of `test-repo/` (or any other shared non-tmp_path source) and either isolate a pristine copy once per session (not per test) or generate the fixture tree synthetically per test. **Review-added facts (ciu-P26 reviewer):** 9 test files read `test-repo/`, 3 via `copytree` — CIU-56's `--dist loadfile` fix removes only SAME-file races, cross-file concurrent access remains possible, so it likely makes this rarer without closing it; the directory currently holds gitignored, accumulating residue (`applications/app-config/ciu.compose.yml` + two `__pycache__` dirs) that is NOT tracked fixture content, so the race surface drifts between a fresh clone/CI and a developer machine — a cheap partial mitigation (render into `tmp_path`, or clean the residue) shrinks the surface without the full audit |
...
| CIU-88 | `ciu/nyxloom-trove/nyxloom.toml`'s `[gates.tester-unified].asserts` array includes `"assay-verdict"`, which is NOT a valid value in nyxloom's own asserts schema enum (`src/nyxloom/schemas/nyxloom-config.schema.json`, closed vocabulary `tests-pass\|changed-line-coverage\|mutation\|canary-verified`) — a live, checked-in schema violation, not just aspirational backlog prose. Currently undetectable in ciu's own pipeline: ciu has no self-lint unit test analogous to nyxloom's own dogfood `test_repos_own_config_no_findings`, and ciu's gate never invokes `nyxloom lint` against its own config | Low | FIXED UPSTREAM — **this row's premise is now false and was verified so, not assumed.** nyxloom-P98 landed NL-4 (`vbpub@479d7e71`, "fix(nyxloom): P98 -- NL-4, add assay-verdict to the asserts enum"), so `"assay-verdict"` IS a valid member of the closed vocabulary today: `nyxloom/src/nyxloom/schemas/nyxloom-config.schema.json` now enumerates `tests-pass\|changed-line-coverage\|mutation\|canary-verified\|assay-verdict`. ciu-P51 (2026-09-08) ran nyxloom's OWN CFG1 rule — `lint.lint_config`'s `Draft202012Validator(nyxloom-config.schema.json).iter_errors(...)` — directly against `ciu/nyxloom-trove/nyxloom.toml` and got **0 findings**, so there is nothing left to close and the file needs no edit. **ciu-P51's own handoff prescribed the opposite fix** (drop `"assay-verdict"` from the array, mirroring nyxloom-P48's comment) — NOT DONE, deliberately: that would now DELETE a valid, semantically meaningful declared assert from ciu's gate config and add a comment that is factually wrong ("it isn't a supported value ... yet"). Two follow-on facts for whoever picks this up: (1) `vbpub/nyxloom/nyxloom-trove/nyxloom.toml`'s own "assay-verdict is NOT listed" comment is now stale in the same way and should be revisited — out of ciu-P51's scope, not filed as a ciu issue since it is nyxloom's file; (2) the ORIGINAL secondary finding in this row still stands untouched — ciu has no self-lint unit test analogous to nyxloom's dogfood `test_repos_own_config_no_findings`, and ciu's gate never invokes `nyxloom lint` against its own config, so a FUTURE schema violation here would still be undetectable in ciu's own pipeline. Original filing follows: found 2026-09-02 by an independent adversarial reviewer during nyxloom-P48's own gate migration (an unrelated package moving nyxloom's OWN gate onto the same assay pattern ciu already uses), which hit the identical defect in nyxloom's copy first (filed there as NL-4) and then confirmed ciu already carries the same invalid value. Likely fix: once NL-4 adds `"assay-verdict"` to the schema enum, re-validate this file; or in the meantime just drop `"assay-verdict"` from ciu's `asserts` array the same way nyxloom-P48 did for its own copy, with a comment explaining the omission is deliberate, not forgotten |
| CIU-89 | CIU-70's own fix (ciu 7.5.1.dev24+) resolves a `pg:`/`minio:` probe's target container via `_resolve_probe_container` → `_stack_container_name(config, stack_path)` (`src/ciu/provisioning.py:401-448,645-670`), which takes the PROVIDING STACK's declared path (e.g. `infra/db-core`) and uses its final path SEGMENT (`db-core`) as the literal service-name argument to `container_name()` — i.e. it assumes the stack directory's own basename is itself a valid compose service key inside that stack. This holds for a single-service stack (redis-core → container `redis`) but is false BY CONSTRUCTION for a multi-service stack whose directory name matches none of its own compose service keys: dstdns's `infra/db-core` stack declares `provides = ["pg:db/dstdns", ...]` at the STACK level (`infra/db-core/ciu.defaults.toml.j2:19-20`) but runs postgres as a service keyed `postgres` (`container_name: {{ deploy.project_name }}-{{ deploy.environment_tag }}-{{ db_core.postgres.name }}`, `ciu.compose.yml.j2:61`) alongside five sibling services (`postgres-init`, `pgadmin`, `pgadmin-init`, `adminer`, `minio`, `minio-init`) — no container named `...-db-core` is ever created. The probe therefore always resolves `cname = container_name(config, "db-core")`, which never exists, and `docker exec` fails with "no such container" — worded IDENTICALLY to "the role/db is genuinely missing" from the caller's perspective (same failure shape CIU-70 itself was filed to fix, now reintroduced by CIU-70's own resolution strategy). Confirmed live 2026-09-02 (`ciu 7.10.1`) on a genuinely fresh Mode-B bring-up (dstdns-P152, `nyxloom-trove/reports/dstdns-P152-LOG.md`): `infra/db-init`'s `pg:db/dstdns` requirement failed provisioning preflight with `container 'p152-...-df56c9-db-core' unavailable (no such container) — pg db 'dstdns' was NOT checked`, even though the `dstdns` database and `controller_ddl` role both genuinely existed on the correctly-named `...-postgres` container (verified directly via `psql`) — i.e. a real, met precondition failing a broken *check*. This is NOT the CIU-70 entry's own predicted failure mode (a service keyed OTHER than `postgres`/`minio`, or two same-keyed services colliding) — it is the reverse: a correctly-keyed service (`postgres`) inside a stack whose OWN directory name differs from every one of its service keys, which CIU-70's stack-basename heuristic cannot express at all. Before CIU-70, the literal-hardcoded-`"postgres"`/`"minio"` behavior it replaced was ACCIDENTALLY correct here (dstdns happens to key its postgres service `postgres`), which is exactly why this coupling never surfaced until now. Proposed fix: resolve the probe container from the compose SERVICE that actually declares the `provides` entry, not the STACK's own path basename — either (a) let a stack's `provides` list carry an optional per-entry service-key override (e.g. a sibling `provides_container = {"pg:db/dstdns": "postgres"}` table, or a `pg:db/dstdns@postgres` selector suffix `parse_ref` learns to strip before matching but keeps for resolution), or (b) walk the stack's OWN rendered compose service list for one whose canonical service key matches the ref kind's own convention (`postgres`/`minio`) before falling back to the stack-basename guess — restoring today's dstdns-correct case while still refusing loudly (never guessing) when neither resolves. Controlled wrong implementation: a fixture with a stack directory `infra/foo-core` (provides `pg:db/bar` at the stack level) whose compose file keys its Postgres service `postgres` (not `foo-core`) must fail today (`container '...foo-core' unavailable') and pass after the fix (`pg db 'bar' exists`, probed against the `...postgres` container). Owning SPEC section: S13.2, same as CIU-70; `--no-preflight` (documented break-glass) is today's working consumer-side mitigation, used by dstdns-P152 to unblock its own bring-up after independently verifying the real precondition was met | Medium | FIXED — ciu-P49. Built the sibling `provides_container` override table (route (a) from this row's own proposed fix, not the compose-service-list walk — the handoff's design call, `nyxloom-trove/handoffs/ciu-P49-...md`): a stack's root-key table may declare `provides_container = { "<ref>" = "<service-key>" }`, validated by `config_model.validate_stack_provisioning` (`src/ciu/config_model.py:1267-1299`, new `[S13.2]`-tagged checks — a key not itself already in that stack's own `provides` list, or a non-string/empty value, is rejected, never silently ignored). `_resolve_probe_container` (`src/ciu/provisioning.py:401-465`) now checks `stacks[stack_path]['provides_container'].get(ref)` before falling back to `_stack_container_name`'s basename guess, passing an override straight to `container_name()` (same unresolvable-project/env-tag fallback `_stack_container_name` itself uses). Threaded the new key through the two `stacks`-dict builders that actually feed a live probe: `deploy.provisioning_graph()` (`src/ciu/deploy.py:632-649`, the `ciu up` per-phase probe graph) and `action_check`'s own builder (`src/ciu/deploy.py:3141-3167`, `ciu check --live`'s separate feed path) — deliberately NOT threaded into `action_graph`'s builder (`deploy.py:3306-3339`), since that dict is echoed verbatim into `--graph --fmt json`'s public `stacks` key and `--graph` never resolves a probe container (confirmed by a real test failure when first threaded there, then reverted). `docs/SPEC.md` S13.2 gained a `provides_container` subsection; `docs/CONFIG.md`'s requires/provides section gained a worked `infra/db-core`/`postgres` example. Purely additive — a ref absent from `provides_container`, or a stack declaring none, is byte-identical to before this package (pinned as a regression guard). Tests: `tests/tests/test_ciu_provisioning_ciu70_probe_container.py` (dedicated CIU-89 section — the exact `infra/foo-core`/`postgres` fixture this row's own "controlled wrong implementation" describes, both the before-fix-still-wrong pin and the after-fix-correct case, plus the undeclared-ref rejection) and `tests/tests/test_ciu_provisioning.py` (direct `_resolve_probe_container`/`probe_ref`/`validate_stack_provisioning` unit coverage, plus an `action_check --live` threading test) — all mutation-verified by actually reverting each fix line and confirming the exact predicted tests fail (ciu-P49 REPORT) |
| CIU-90 | Governance injects `mem_limit`/`memswap_limit`/`mem_reservation`/`blkio_config` per-service compose keys but has no CPU-quota key at all — `src/ciu/governance.py`'s `INJECTED_KEYS` tuple (the exhaustive, commented list of "compose service-level keys governance may inject") lists exactly those four plus `cgroup_parent`; nothing named `cpus`/`cpu_quota`/`cpu_period`/`nano_cpus`/`cpu_shares` exists anywhere in the module. A stack that enables governance and relies on it (rather than authoring its own compose-level CPU key, which none of dstdns's stacks currently do) gets memory and block-IO capped but literally unbounded CPU, silently — there is no error, warning, or doc note; the author simply never gets what the memory/IO precedent would lead them to assume is a parallel guarantee. Confirmed live 2026-09-02 on dstdns's `test-runner` container (`ciu`-governed, `cgroup_parent=dev-background.slice`): `docker inspect` shows `Memory=1073741824` (1GiB), `MemorySwap=18253611008` (~17GiB, i.e. ~16GiB swap on top), `MemoryReservation=268435456` (256MiB) — all correctly injected — alongside `NanoCpus=0 CpuQuota=0 CpuPeriod=0 CpuShares=0`, i.e. no CPU governance at all. Confirmed load-bearing, not merely theoretical: host PSI (`/proc/pressure/cpu`) showed `some avg300=12.05` (processes stalled waiting for a CPU ~12% of the time over 5 min) at the same moment `/proc/pressure/memory` showed `some avg300=2.16` — the actually-scarce, actually-contended resource across this host's concurrent ciu-governed containers is CPU, which is exactly the one governance dimension left unenforced; memory pressure, despite alarming-looking `free -h` swap figures, was not the real signal. This is a ratified product call (dstdns `decisions.md` D-264 PC-1: "worker-io needs a CPU quota CIU governance does not inject... File a `vbpub/ciu` backlog entry (durable fix)") that was never actually filed upstream — D-291 re-confirmed the omission (`grep -ci cpu KNOWN_ISSUES_TODO_BACKLOG.md` → 0) — filed now with the concrete live specimen the earlier note lacked. Proposed fix: add a CPU key to `INJECTED_KEYS` (`cpus` is the modern Compose Specification key, translating cleanly to a container's `NanoCpus`; `cpu_quota`+`cpu_period` is the legacy two-key form governance's memory keys already mirror) sourced from a new `governance.cpu_limit` config table entry, following the exact same "author's explicit key always wins, governance only fills what's unset" precedence `INJECTED_KEYS`' own docstring already states for the other four keys | Medium | FIXED — ciu-P49. Added the modern single-key `cpus` (Compose Specification's own key name, no config-table/compose-key translation needed, unlike `mem_swap_limit`→`memswap_limit`) to `GOVERNANCE_DEFAULTS` (`src/ciu/governance.py:80-88`, default `""` = unset/uncapped — deliberately NOT a nonzero default: D-264 PC-1's actual ask was that a stack that WANTS a quota gets one to configure, not that every already-governed, currently-uncapped service gets silently throttled by an invented estate-wide default on upgrade) and `INJECTED_KEYS` (`:260-278`). `resolve_config()` validates `""` (unset) or `float(cpus) > 0`, raising `[S15.21]` otherwise (`:422-438`, mirrors `io_weight`'s S15.14 shape/reasoning). `build_injections()` injects `frag["cpus"] = cpus` only when `governance.cpus` is set AND the author hasn't already set their own `cpus:` key (`:1033-1038`, same S15.3 author-precedence the other four keys already have), and the `cpus=` note is appended only when actually set, not unconditionally (`:1082`). New `docs/SPEC.md` S15.21 section (mirrors S15.14's shape) and a `docs/CONFIG.md` governance worked example. Tests: `tests/tests/test_ciu_governance.py` `TestResolveConfig`/`TestBuildInjections` — unset-default regression guard, valid values, `"0"`/`"-1"`/non-numeric rejection, injection, author-precedence — all mutation-verified by actually reverting each fix line and confirming the exact predicted oracles fail (ciu-P49 REPORT). Live verification, mirroring this row's own `docker inspect` method: a real `governance.build_injections()`-computed fragment (`cpus: "1.5"`) fed into an actual `docker compose up`, `docker inspect --format '{{.HostConfig.NanoCpus}}'` reporting `1500000000`; the unset-default case on the same harness reports `0` |
| CIU-91 | `tests/tests/test_ciu_render_selection_context.py::test_engine_threads_selection_into_configfiles_and_hooks` (via its `_add_stack` helper, `shutil.copytree(TEST_REPO/"applications"/"app-config", dst)`) intermittently fails `shutil.Error: [Errno 2] No such file or directory` on `test-repo/applications/app-config/ciu.toml` when the full suite runs against a genuinely fresh checkout under pytest-xdist `--dist loadfile`. `ciu.toml` under each `test-repo/` stack directory is a generated, gitignored file (`.gitignore: **/ciu.toml`) that `tests/tests/test_ciu_test_repo.py` renders DIRECTLY IN PLACE into the committed `test-repo/` tree, not into a `tmp_path` copy — e.g. `test_render_global_and_stack_configs` (renders all four stacks' `ciu.toml`) and `test_app_config_full_pipeline_runs_under_dry_run`, whose own `_clean_stack_artifacts` helper UNLINKS `ciu.toml` at the START of that test as prep for its own fresh render. `--dist loadfile` only guarantees ordering WITHIN one test file, never BETWEEN files, and both test files share the same ON-DISK `test-repo/` directory (not per-worker copies) — a genuine cross-worker TOCTOU race, and specifically that shape, not a broader "ran before anything was generated" ordering gap: the captured traceback's own `entries` list (`os.scandir`'s snapshot, taken once at the START of `shutil.copytree`) DOES include `<DirEntry 'ciu.toml'>` — the file demonstrably existed at listing time — so the failure is `_clean_stack_artifacts` (or another worker's own fresh-render cleanup) UNLINKING that same file in a different xdist worker strictly AFTER `os.scandir` ran but BEFORE `copy_function` reached it a few lines later; `shutil.copytree` cannot and does not raise this way for a file that was simply never generated (that file would not appear in `entries` at all, and the copy would silently proceed without it — corrected here after the adversarial ciu-P49 review round pointed out the original wording implied a mechanism `shutil.copytree` cannot actually produce). This is the same `test-repo/`-in-place-generation design `.gitignore`'s own comment ("ciu-P47. The integration suite generates into the committed test-repo/") acknowledges as intentional, but nothing in that design accounts for cross-file xdist parallelism sharing the one directory. Reproduced live during ciu-P49's own real gate run at commit `66c990ba` (a fresh assay-sandboxed checkout, no locally-primed `test-repo/` state): R0 failed with exactly this `shutil.Error` and traceback (full traceback in that run's `.assay/verdict-ciu.json` `result_stdout_tail`). An immediate re-run of the gate at the next commit (`69c573a0`, code-identical aside from one unrelated coverage-only test addition) passed cleanly with no such failure — did not reproduce on retry, consistent with a genuine intermittent race, not a deterministic regression from ciu-P49's own CIU-89/CIU-90 changes (neither of which touches `test-repo/`, `test_ciu_test_repo.py`, or `test_ciu_render_selection_context.py`). Proposed fix directions, not implemented here (out of ciu-P49's own scope — its own CIU-89/CIU-90 sections named disjoint files only): (a) `_add_stack` (and any other cross-test-file consumer of a `test-repo/` stack directory) renders its OWN copy of `app-config` from the `.j2` templates rather than depending on another test file having already rendered `ciu.toml` in place as an uncontrolled side effect; **(b) — the one that actually addresses the confirmed mechanism —** give the in-place-render tests in `test_ciu_test_repo.py` a dedicated non-parallel xdist group or a module-scoped lock, so `_clean_stack_artifacts`'s unlink can no longer race a concurrent worker's read of the same file; ~~(c) simplest — `_add_stack` renders `ciu.toml` itself from the template when absent rather than assuming it~~ — **struck, not a fix**: the confirmed mechanism is TOCTOU (the file IS present at `_add_stack`'s own `os.scandir` time, per the corrected reproduction above, and vanishes mid-copy), not absence — an "if absent, render it" guard in `_add_stack` would not even trigger, since the file was present when checked, and the underlying race with `_clean_stack_artifacts`'s concurrent unlink would still fire during the copy itself | Medium | FIXED 2026-09-08 by **ciu-P51**, via fix direction (b) — but by (b)'s SECOND named alternative (a lock), not its first (an `xdist_group` marker), because **the marker route is inert in this suite and that was measured, not assumed**: `pytest.mark.xdist_group` is honoured ONLY by the `loadgroup` scheduler (`xdist/remote.py:241`'s group-suffix rewrite is guarded by `if config.getvalue("loadgroup")`), and `run-ciu-tests.py:40` runs `--dist loadfile`. Measured on the pinned pytest-xdist 3.8.0: four tests carrying the SAME `xdist_group` name landed on four DIFFERENT workers under `--dist loadfile` and on one worker under `--dist loadgroup`. Switching the runner to `loadgroup` was rejected — it makes every UNMARKED test its own work unit (i.e. plain `load` distribution), which is exactly the non-deterministic coverage split CIU-56 adopted `loadfile` to fix, and `run-ciu-tests.py` was outside the package's Touch list besides. The shipped fix is an `fcntl` reader/writer lock in `tests/conftest.py` (`_hold_test_repo_lock`, `_serialize_shared_test_repo_access`, `shared_test_repo_read_lock`) keyed by the resolved `test-repo/` path in the system temp dir — a FILESYSTEM lock, not the "module-scoped lock" this row's own wording suggests, because xdist workers are separate PROCESSES and an in-process lock would serialize nothing. Writers (`@pytest.mark.ciu_test_repo_inplace`, 7 tests in `tests/tests/test_ciu_test_repo.py`) take `LOCK_EX` for the whole test; readers (`@pytest.mark.ciu_test_repo_reader`, 85 tests across `test_ciu_render_selection_context.py` + a module-level `pytestmark` on `test_spec_contracts.py`, plus `test_ciu_identity_cutover_ciu75.py`'s `verb_repo` fixture requesting the lock fixture directly) take `LOCK_SH`, so readers never block each other and a writer never overlaps a reader. Both marks are registered in conftest's own `pytest_configure` (this suite has NO `[tool.pytest.ini_options]` at all, so strict-markers is off and `xdist_group` itself is registered by pytest-xdist — but these two are ciu's own and needed registering). Same fix closes CIU-58; see that row for the full enumeration. Original filing follows: filed by ciu-P49 (fresh implementer), 2026-09-02, during that package's own gate run; mechanism corrected 2026-09-02 by ciu-P49's adversarial reviewer (the alternative-mechanism claim and fix direction (c) were both factually wrong, per the corrected description above) |
| CIU-92 | **v8 design set revision 3.0 / SPEC-V8 8.0.0-draft.3 (2026-09-02)** — a fresh adversarial review of the v8 design set (`docs/CIU-V8-ADVERSARIAL-REVIEW-2026-09-02.md`, 78 findings R-01..R-78, operator interviewed on ten forks) replaced proposal rev 2.1 and spec draft.2 with fresh texts. Structural changes every V8 implementer must read before carving: (1) `ciu gate` lifts run-gate's functionality but run-gate STAYS maintained standalone (operator decision; the "absorb and freeze" plan of rev 2.1 contradicted the estate's no-hard-dependency rule) and `ciu gate` gains a mandatory zero-instance mode (no instance/lock/rendered file/docker for a `[project]`+`[testing]`-only project, S16.11) with per-need preflights (no startup docker check, S18.3); (2) every declaration file is plain TOML — `ciu.toml`, `ciu.site.toml`, `ciu.instance.toml`, `ciu.stack.toml`, rendered `ciu.resolved.toml` — Jinja only in compose/config-file templates (P11); (3) consumers declare BINDINGS (`[ciu_stack.<svc>.binds.<local>] to/wait/delivery/env_prefix/facts`, `requires` as sugar) delivered like secrets (env or template); `routes`, `init_requires`, `uses`, `after`, the two-pass stack render and the hand-typed `contract` are gone — the contract is derived from bindings and every variant is checked against it (S5.3); (4) secrets are structured tables (`from = vault|generate|ask|file|host|ephemeral`, `path` resolved against a checked `[vault.paths]`, `delivery` incl. `hook`); (5) the instance lock is `flock` on the checkout directory, the rendered file is atomic (no in-place render, no completion marker); (6) hooks stay subprocess+JSON but ciu ships `ciu.hookkit` (`wait_healthy`/`wait_tcp`/`emit`/…, S12.5); (7) push secrets travel per SOURCE (local-source values travel; vault-sourced values are fetched by any target with a resolution to `vault`, else pre-fetched, S17.3.1); (8) v7 verbs all have dispositions (S18: `init`, `status`, `show`, `dev`, `ssh`, `provenance`, `migrate`, `schema`, `doctor`, `instance lease`), v7 exit-code meanings kept (+4 lock, +5 remote), one JSON envelope (`schema_version`); (9) `sequence` lanes, `[testing] inherit` (run-gate R-22), environment `binds`, exec mount proof (v7 S16.7 restored), provenance in every LaneResult; (10) fixed `ciu.*` ownership labels (no consumer prefix), declared host `fqdn`, `seeded` level name, `[project]`/`[bundles]`/`[layouts]`/`[realness]` top-level tables, `ciu.rendered/<svc>/<mirrored path>` directory mounts (v7 S5.3a kept), `ciu migrate --hostdirs` for the `vol-*` → `ciu-data/` data move. The demo `docs/v8-dstdns-demo/` is rewritten to the new notation (27 stacks + `examples/minimal/`). Carve list: proposal §4.4 V8-1..V8-22 with suggested checkpoints A–E; N18–N21 are safe-now items (run-gate reads `ciu.resolved.toml` identities; `ciu schema --json`; `ciu.hookkit` on v7; run-gate `sequence` lanes) | Medium | OPEN — design only, nothing implemented; 8.0.0 stays reserved for the cutover; the operator's interview answers are in the review §4 and the proposal §4.3.1. **2026-09-03: rev 3.1 / draft.4** after an independent third-party review (`docs/CIU-V8-THIRD-PARTY-REVIEW-2026-09-02.md`, 35 findings, verdict "not implementable as written" — accepted; dispositions in `docs/CIU-V8-THIRD-PARTY-REVIEW-RESPONSE-2026-09-03.md`): operator kept the in-checkout state posture (v7) and adopted manifested releases + activation receipts; draft.4 Appendix D lists every changed rule; new carve rows V8-23 (releases/receipts), V8-24 (gate conformance fixtures shared with run-gate), V8-25 (query surface, api headers); the minimum drivable judge is assay 4.1.0. **2026-09-03 (later): rev 3.2 / draft.5** after the same reviewer's round-2 delta audit (`docs/CIU-V8-THIRD-PARTY-REVIEW-ROUND2-2026-09-03.md`: disposition audit + T2-01..T2-10; all folded — spec Appendix D §D.2, response §6): receipts with a canonical subject and an activation id, remote facts refused without one unless `--allow-assumed` (S17.4.3–S17.5); the acceptance partition (S8.5.5) and `verify`/`unchecked` for empty-contract prepared variants (S5.3.6); every draft.4 key/flag/state/reason put into its closed set plus a generated conformance test (S3.8.5); the `pre_secrets` source matrix (S6.10); release closure from non-ignored stack files + hook `inputs`, image transport by digest or archive, `candidate`, CIU-owned `current`/`previous` switch and rollback (S17.3–S17.4); host-network `listen` (S6.3, S7.8); admission keyed by the slice's cgroup directory, per-run gate directories with `--resume`, one lane per exec target (S16.5.7, S16.6.1, S16.9.4); `owner_id`/`ciu.owner` behind `--move` (S4.5.3); exact `cpu_shares` inverse (S13.3); per-value lint exemption (S2.4.2). Operator decisions 2026-09-03: **v8 is built as the new subproject `vbpub/ciu8`** (console script `ciu8`; v7 `ciu/` maintenance-only; copy-adapt, never import; the 8.0.0 cutover renames `ciu8` → `ciu`, v7 → `ciu7`); the canonical lock keys + `ciu lease` (S14.4.7–S14.4.8; V8-26); `[ciu] inherit` for a closed policy list (S3.1.5; V8-27) with a per-project two-file tester stack over `tester-unified/` instead of a shared `location`. Checkpoint A = V8-1, V8-27, V8-14, V8-2, V8-11, V8-26, V8-13, V8-5. **Round 3 folded the same day → rev 3.3 / draft.6** (`docs/CIU-V8-THIRD-PARTY-REVIEW-ROUND3-2026-09-03.md`, T3-01..T3-10; spec Appendix D §D.3, response §7): every mutable file lives in a **state root** outside any release (S2.6; `ciu.host.toml` replaces `[ciu.host.generated]`, realness records move into `ciu.instance.json`); the activation manifest (`ciu activate plan`) and the prepare → apply → health → receipt → switch state machine (S17.4); the lock matrix + lock-free lease records + `CIU_LEASE_FDS` (S14.4.9); cold moves, copies refused, `adopt --owner` withdrawn (S4.1.2); inherit/build-context reach = the containing worktree, flattened into releases (S3.1.5, S6.2); random run ids, no `ciu gate --resume` (S16.9.4); consumer-vantage host-network probes (S8.5.2a); the per-reference image map (S17.6.1); `memory_max` mandatory, every hard cap aborts (S16.6.1, S13.3.2); the spec's enumerations tested (S3.8.6); demo `examples/monorepo/` fixture. New carve row V8-28 (executable fixtures). Handoff: `docs/CIU-V8-HANDOFF-2026-09-03.md` |
| CIU-93 | **No mechanism exists to enroll a genuinely fresh remote host into `.ciu.hosts.toml`** — S14 (v7)/S7.2+S17 (v8 draft.6) fully specify `ciu ssh`/`ciu up --host`/`--thin` push-activate for an **already-trusted** host (a working `ssh_key` + pinned `known_host` already present in the inventory file), but nothing covers getting from "bare host, nothing installed, no prior SSH trust" to that state — today it is 100% manual (`ssh-keygen` once, out-of-band public-key distribution, `ssh-keyscan` for TOFU, hand-edit the TOML), confirmed still fully manual live during dstdns-P171's carve (2026-09-02/03, seeded-infra remote-placement research). This is the exact gap dstdns decisions.md **D-097** (2026-08-19) named and explicitly deferred: "self-hosted `get.py --bootstrap-url --token` (behind tls-edge, GitHub-independent) DESIGNED + DEFERRED." Why ciu owns it: the artifact being populated (`.ciu.hosts.toml`) and every verb that consumes it (`ciu ssh`, `ciu up --host`, `--thin` activation) are ciu's; a consumer-side workaround would just be re-inventing S14/S17's own trust model per-project. Proposed contract, oracles, and the full design (a new `ciu host enroll <name>` verb; reuse of the already-production `cmru get-py`-generated installer pattern — proven live via `vbpub/tls-edge/get.py` — as the one-shot bootstrap artifact a remote admin runs by hand; control-side key generation so the private half never transits the wire; the same `bootstrap｜apply｜health[｜rollback]` adapter-verb vocabulary S14.6/S17.4 already require of a `--thin` host's `activate` contract, so an enrolled host's bootstrap script IS its activation script with no second artifact; refusal states for a dead/expired token, an unconfirmed host-key callback, and a target missing `openssh-server`) are written up in full at `docs/CIU-HOST-ENROLLMENT-PROPOSAL.md`. **Open cross-repo dependency, not yet filed**: `cmru get-py`'s template/renderer needs a second, self-hosted (non-GitHub-Releases) download backend before this verb is implementable end-to-end — see that proposal §6 | Medium | **FIXED** 2026-09-08 by **ciu-P52** (the v7 backport only; the v8 form, `SPEC-V8.md` S7.2.4 / V8-29, is untouched). `ciu host enroll` ships as a verb GROUP (`src/ciu/cli.py:1807`, `_host`) matching `worktree`'s existing grouped-verb shape rather than a third hyphenated flat verb — the proposal's own §6 table ("a `host` group beside the flat `host-secrets`") settles that naming, and `host-secrets` is left exactly where it is. Steps 1 and 2 are ONE subcommand disambiguated by `--ssh-host`/`--fingerprint`: `host_enroll.enroll_step1` (`src/ciu/host_enroll.py:167`) shells out to the real `ssh-keygen` into the existing S14.3a namespace via `secrets.materialize.host_secret_store` (0700/0600/0644), prints the public key + the version-pinned installer URL + the step-2 command, and writes NOTHING to the inventory; `host_enroll.enroll_step2` (`src/ciu/host_enroll.py:391`) keyscans, fingerprints each scanned key with OpenSSH's own `ssh-keygen -lf`, refuses on mismatch naming BOTH fingerprints (non-TTY without `--fingerprint` is a refusal, never a silent TOFU), proves the login with `ciu version` through the EXISTING `transport_ssh.ssh_exec` (so S14.4a's fail-closed pin and S14.4c's `[ADDR]:N` temp-known-hosts file are reused, not reimplemented), and only then writes the row. `--abort` (`src/ciu/host_enroll.py:229`) removes a pending step-1 pair. **The inventory file's first WRITER** is `hosts.write_host_row` (`src/ciu/hosts.py:510`), deliberately beside the reader it must round-trip against: **stdlib-only targeted text surgery, NOT `tomlkit`** — tomlkit is absent from both ciu's dependency closure and the gate's own `tester-unified:local` image (verified: `docker run tester-unified:local /opt/tester-venv/bin/python -c 'import tomlkit'` → ModuleNotFoundError), and that image's venv is built in cmru, a path this package is forbidden to touch. Safety comes from three places instead: `_classify_lines` (`src/ciu/hosts.py:265`) tracks multi-line basic/literal string state so a `[table]`-looking line inside a `"""` value is never read as a header, every header and key spelling is handed to `tomllib` itself rather than split by hand, and `_verify_round_trip` (`src/ciu/hosts.py:369`) REFUSES rather than writing anything it cannot prove is a pure single-row edit (result parses, the row is as asked, the operator's own keys survive, and the whole rest of the document is deep-equal). A new row is APPENDED at EOF (every prior byte copied verbatim); `--replace` rewrites only the managed keys inside the existing table's own body (`_replace_row_body`, `src/ciu/hosts.py:441`), keeping sub-tables, unmanaged keys and even a trailing comment on a rewritten line. Atomic (temp sibling + `os.replace`, mode preserved). A file using the top-level `[hosts.*]` form keeps it (`resolve_hosts_file`/`_inventory_prefix`) — writing `[deploy.hosts.*]` there would have made `load_hosts` stop seeing every existing row. Also shipped: `cmru.toml` `[project.installer]` + `--extra-asset get.py` on the push step, and the rendered installer committed at `ciu/get.py`. Specified as `docs/SPEC.md` **S14.7** (`docs/SPEC.md:2775`), which also now records KI-24's measured `from="P" <type>` (whitespace, never comma) correction and CIU-99. Oracles: `tests/tests/test_ciu_host_enroll.py` (98 tests) — O1 `TestStep1`, O4 `TestStep2`, O5's three controlled wrong implementations each pinned as their own test (`TestControlledWrongImplementations`), O6 `TestRenderedInstaller` (the committed `get.py`'s `enroll --help` flag set, byte-identity with a fresh `cmru get-py --project ciu` render, and the release coordinates cross-checked against ciu's own `cmru.toml`), plus `TestEnrollEndToEnd` — the chain oracle that runs step 1's OWN printed one-liner against a real sshd in a container, chains the fingerprint the target prints into step 2, and proves both outcomes of the `ciu version` probe. Prior status, for history: OPEN — design only, nothing implemented. Formalizes a tentative, never-filed idea from the same P171 carve session. Needs operator review on the four open questions in the proposal §7 (interactive-vs-async enrollment flow, `--host-hint` necessity, where the bootstrap tls-edge endpoint lives, and confirming this number is still the right one to use) before any implementation is carved. **2026-09-03 operator direction → proposal revision 2 (ACCEPTED for both lines):** no token, no callback, no tls-edge endpoint, no cmru download-backend prerequisite. `ciu host enroll <name>` (step 1) generates the key pair control-side and PRINTS a version-pinned one-liner — ciu's own `get.py` (new: `cmru get-py --project ciu`, committed at `ciu/get.py` + a release asset) run as `enroll --authorized-key '<public key>' --controller <FQDN> [--user U] [--docker] [--from PATTERN]` — which installs ciu, creates the deploy user, appends the key once and prints the host-key fingerprint; `ciu host enroll <name> --ssh-host ADDR --fingerprint SHA256:…` (step 2) keyscans, refuses on mismatch, proves the login with `ciu version`, and writes the pinned row with a round-trip writer. Specified as `docs/SPEC.md` **S14.7** (v7 backport package — an explicit operator exception to v7's maintenance-only posture, because dstdns P171 needs remote placement before ciu8) and `docs/SPEC-V8.md` draft.7 **S7.2.4** (carve row V8-29). The cmru half (`get.py enroll` template subcommand) is cmru **KI-24**. Oracles O1–O6 in the proposal §8. Open questions 1–3 of §7 withdrawn; question 4 settled (this number) |
| CIU-94 | **Governance has no per-container `memory.min` mechanism, and the config key that gestures at one (`mem_min`) is declared-intent-only — never injected, never admission-checked.** Confirmed live 2026-09-03 during dstdns P165's sql-mutation-gate host-stability incident (a disposable Postgres's oversized `shared_buffers` produced periodic zswap-refault CPU bursts that stalled a co-located production game server; full incident + design reasoning: dstdns `nyxloom-trove/decisions.md` **D-373**, `modern-debian-tools-python-debug/host-setup/CGROUP-NOTES.md` §"Per-container memory.min guarantees"). `src/ciu/governance.py`'s `GOVERNANCE_DEFAULTS["mem_min"]` (default `""`) is checked only against the resolved `cgroup_parent` slice's own live `MemoryMin=` at deploy-time preflight (`governance_slice_preflight`, S15.16) and is **never** written onto the container itself — unlike `mem_limit`→`memory.max` and `mem_reservation`→`memory.low`, both of which ARE injected as real Docker-level flags. There is no Docker/Compose field for a per-container `memory.min` (confirmed absent from both the Docker CLI and Compose Specification), so injection would need a post-start step writing directly to the container's own transient scope cgroup file — not a `docker run`/compose flag. Two things are missing, not one: (a) actually injecting the declared `mem_min` value onto the *child's own* scope once it starts, and (b) **admission control**: before starting a container with a declared claim, sum the currently-running siblings' own claims (under the same dedicated slice — see the host-setup design doc for why a shared, heterogeneous slice like `dev-background.slice` can't safely carry any `MemoryMin` at all, via cgroup v2's proportional-surplus-redistribution semantics) and refuse to start if admitting this one would exceed a static, human-provisioned slice-level ceiling — the chosen design (over a dynamic parent-reconciling script) specifically because it needs zero new host-systemd privilege for ciu and has no drift/leak class of bug from out-of-band container stops. Proposed shape: extend `INJECTED_KEYS`' pattern with a genuinely new (not `docker run`-flag-backed) injection path — a post-start `systemctl set-property --runtime <scope> MemoryMin=<value>` call (never a raw cgroupfs write — a routine `daemon-reload`, e.g. from an unrelated `apt install`, silently wipes those; `set-property` survives it, per `CGROUP-NOTES.md`'s own established doctrine) — plus a pre-start admission check reading live sibling claims from cgroupfs or ciu's own tracked stack state, refusing (loud, matching this whole estate's fail-fast convention) rather than silently starting under-guaranteed. Sizing/ceiling provisioning itself is host-setup's job (a new `dev-memory_min_guaranteed.slice`), not ciu's — this entry is the consumer-side half only | Medium | **FIXED** 2026-09-08 (ciu-P50; per `nyxloom-trove/decisions.md` D-012 this was built in v7 now, without waiting on the v8 comparison the row asked for). Both halves shipped, specified as SPEC.md **S15.23** (`docs/SPEC.md:3655`). **(b) admission control:** `governance.check_mem_min_admission` (`src/ciu/governance.py:1170`) reads the slice's OWN live `memory.min` from cgroupfs as the ceiling — never a mirrored CIU-side config value, so nothing can drift from what host-setup provisioned — sums every current occupant's own claim via CIU-95's new downward walk, and admits iff `sum + candidate <= ceiling` (an exactly-full slice ADMITS, mirroring `check_slice_memory_min`'s `>=`). Wired as `deploy.mem_min_admission_check` (`src/ciu/deploy.py:1236`), called per entry immediately before `_run_stack` (`src/ciu/deploy.py:2079`) so it sees occupants that earlier entries in the SAME run brought up; over-ceiling goes through `warn_or_raise(severity="ERROR")`, which raises by default (S10.3 → exit 2). No provisioned ceiling, or not host-rooted → informational skip, deliberately NOT a second deploy-blocking reason for the condition S15.16 already reports from the declaration side. **(a) injection:** `governance.container_transient_scope` (`src/ciu/governance.py:1253` — a port of `mdt-dev-governance-reconcile.sh`'s `/proc/<pid>/cgroup` `0::`-line + first-`.scope` trim) plus `governance.set_scope_memory_min` (`src/ciu/governance.py:1296`, `systemctl set-property --runtime <scope> MemoryMin=<bytes>` — never a raw cgroupfs write, which a routine `daemon-reload` would silently wipe), wired as `deploy.apply_mem_min_injections` (`src/ciu/deploy.py:1315`) right after a successful `_run_stack` (`src/ciu/deploy.py:2108`), enumerating the stack's real rendered `container_name`s. Injection failures are `[WARN]`, deliberately the opposite of admission's ERROR (the deploy has already succeeded by then). Every new function gates FIRST on `_systemd_is_pid1()` — a silent no-op inside a devcontainer. **Two known v1 limitations, documented rather than hidden:** admission counts ONE claim per STACK while injection applies the declared value to EVERY non-exempt service, so a multi-service stack dilutes protection for every OTHER occupant of the slice (S15.23's granularity note — forced by the compose YAML not being rendered yet at the admission call site); and the concurrent-admission race stays unlocked, as CGROUP-NOTES.md's own alternatives table already accepted. The `set-property` call is CIU's first-ever host write, reconciled against DESIGN-NOTES **D2**'s "verify-only, forever" framing in new **D9** (`docs/DESIGN-NOTES.md:532`), which also draws the line a future package must not drift past. `test-repo/infra/db-core` is the worked-example fixture (the "disposable Postgres" case from the source incident). **Live verification is NOT yet done** — the implementing environment has no host-rooted systemd, so `systemctl show <docker-<id>.scope> --property=MemoryMin` after a real deploy, and a genuine over-ceiling refusal, are deferred to whoever merges with host access (see `nyxloom-trove/archive/ciu-P50-REPORT.md`) |

| CIU-95 | **No preflight walks the slice tree to confirm a declared `memory.min`/`memory.low` would actually be effective — only the ancestor-chain VALUES are checked, never the `memory_recursiveprot` mount flag those values depend on.** `check_slice_memory_min`/`check_memory_min_ancestor_chain` (`src/ciu/governance.py:827`/`908`, S15.16) walk UPWARD from a named slice checking each ancestor's declared `MemoryMin`, but neither checks whether `/sys/fs/cgroup` is actually mounted with `memory_recursiveprot` — and `modern-debian-tools-python-debug/host-setup/CGROUP-NOTES.md` §5 documents, with a live incident (game host, 2026-07-17), that this flag can be silently stripped by a runtime remount: *"a slice's MemoryLow/MemoryMin does not reach the container pages below it — every floor and soft protection in both tiers silently protects nothing, while `systemctl show` happily reports the value you set."* A ciu deploy against a host in that state would report the ancestor chain "OK" (every declared value present and numerically adequate) while the actual protection is completely inert — a false-positive preflight, not a missing one. Filed alongside CIU-94's `dev-memory_min_guaranteed.slice` design work (dstdns D-373) which depends on this preflight actually meaning something. Second, related gap: nothing in governance.py walks **downward** (parent → children) at all — only upward ancestor checks exist — so there is no way to warn "this slice's `MemoryMin` protects OTHER, unexpected children too" (the proportional-surplus-redistribution hazard `CGROUP-NOTES.md`'s new "Per-container memory.min guarantees" section describes for a shared tier). Proposed: (a) add a `memory_recursiveprot` mount-flag check to the existing ancestor-walk functions (`grep cgroup2 /proc/mounts` for the flag, exactly `mdt-host-check.sh`'s own check — that script already treats this as FAIL not warn, for good reason) so a preflight can't report a green ancestor chain the flag itself has invalidated; (b) a downward enumeration (`cgroup.procs`/`cgroup.subtree_control` walk under a named slice, or `systemd-cgls`-equivalent) surfaced as an informational preflight note when a slice carries any `MemoryMin`/`MemoryLow`, listing every OTHER occupant that would share in surplus redistribution. Related but distinct from existing tooling: `scripts/cgroup-profiler`'s `--damon`/effective-limits-table already reports ancestor-vs-declared limits for a single *profiling run's* target, on demand — this entry is about a standing *deploy-time* preflight gate, not an on-demand observability report; the two could share the effective-limits computation rather than reimplement it | Medium | **FIXED** 2026-09-08 (ciu-P50, alongside CIU-94). Specified as SPEC.md **S15.22** (`docs/SPEC.md:3580`). **(a) the mount flag:** `governance.check_memory_recursiveprot` (`src/ciu/governance.py:1105`) reads `/proc/mounts` directly (no `findmnt` subprocess, unlike `mdt-host-check.sh`), finds the `cgroup2` line and tests its options for `memory_recursiveprot` as a whole comma-separated token. Deliberately a SEPARATE function called ONCE from `governance_slice_preflight` (`src/ciu/deploy.py:1568`), and only when some stack in the selection actually declares `mem_min` — it is a host-wide property of the mount, not a per-slice/per-ancestor one, so folding it into the existing ancestor walk would re-run an identical, slice-independent check once per ancestor per slice for zero extra information. Severity is **ERROR** (raises under the default `ciu.exit_on`), matching `mdt-host-check.sh`'s own FAIL-not-warn treatment and deliberately stronger than the `[S15.16]` ancestor-chain WARN sitting two lines below it: blast radius, not confidence — an inadequate chain scopes to ONE slice, a missing mount flag invalidates EVERY declared `mem_min` on the whole host at once. A pure cgroup-v1 host (no `cgroup2` line) abstains rather than falsely accusing. **(b) downward enumeration:** `governance.enumerate_slice_children` (`src/ciu/governance.py:1051`), built on `governance.CGROUP_ROOT` (`src/ciu/governance.py:307`) and `governance.slice_cgroup_path` (`src/ciu/governance.py:1004`, a pure reversal of `slice_ancestor_chain` into the cgroupfs's nested-directory layout), reads straight from cgroupfs with `pathlib` — the technique ported from `host-setup/scripts/mdt-slice-memory-min-low-audit.py`, no `systemctl`/`docker` per child. Its contract carefully separates `None` (ABSTENTION — not host-rooted; a devcontainer's own `/sys/fs/cgroup` is its OWN namespace-local root, so walking it would enumerate the wrong tree and report it as the right one) from `[]` (a DEFINITIVE zero — slice inactive or genuinely empty). Rather than shipping only as an informational preflight note, this primitive is load-bearing: it IS the sum CIU-94's admission control decides on. The shared effective-limits computation with `scripts/cgroup-profiler` suggested in this row was NOT done — the two still compute their own; that remains open if anyone wants it |
| CIU-96 | **`governance.check_mem_min_admission` / `deploy.mem_min_admission_check` (CIU-94/S15.23, `src/ciu/governance.py:1170`, `src/ciu/deploy.py:1236`) double-count a stack's own already-running prior instance on redeploy, and can spuriously refuse a no-net-change redeploy.** Admission sums every CURRENT occupant of the target slice (`governance.enumerate_slice_children`, CIU-95) with no notion of "this candidate is a replacement for an occupant already in that sum," and `mem_min_admission_check` runs before `_run_stack` stops the entry's own previous container. Found by ciu-P50's own adversarial code reviewer (2026-09-08), not by the implementer. Concretely: a slice sized correctly for its intended occupant(s) — exactly what `CGROUP-NOTES.md`'s sizing doctrine calls for, and exactly the case "an exactly-full slice still admits" (S15.23) is designed to allow — will refuse EVERY subsequent redeploy of a stack that already fully claims its ceiling, even though the net claim across the whole operation never changes; an ordinary config/image-update `ciu deploy` of a continuously-processing workload (CGROUP-NOTES.md's own motivating case) is arguably the MORE common path than a first deploy, not an edge case. Distinct from the already-accepted concurrent-admission race (CGROUP-NOTES.md's alternatives table: a graceful over-admit between two DIFFERENT stacks) — this is a spurious hard refuse of the SAME stack replacing itself. Documented as a known v1 limitation with an operational workaround (provision headroom for the redeploying stack's own claim, or `ciu down` before `ciu deploy` for guaranteed-slice stacks) in SPEC.md S15.23 and CHANGES.md rather than shipped silently. Proposed fix: exclude an entry's own current occupant(s) from the live sum before admitting its replacement — needs correlating a live cgroup child (an opaque `docker-<id>.scope` name) back to the SPECIFIC CIU entry that owns it, which nothing at the admission call site currently has (no container-name or compose-label correlation is available pre-`_run_stack`); real follow-up work, not a one-line patch | Medium | FIXED 2026-09-08 by **ciu-P51**, by the container-name correlation route (approach 1 of the two the handoff sketched). New `deploy._entry_prior_instance_scopes` (`src/ciu/deploy.py:1236`) reads the entry's PRIOR rendered `ciu.compose.yml` — still on disk at admission time, because the render happens inside `_run_stack`, AFTER the check — and for each non-exempt service walks the same chain `apply_mem_min_injections` already walks in the other direction (`container_name` → `_inspect_state` `.State.Pid` → `governance.container_transient_scope`), landing on the same `docker-<id>.scope` names `enumerate_slice_children` reports as child cgroups. `governance.check_mem_min_admission` (`src/ciu/governance.py:1170`) takes those as a new keyword-only `exclude_scopes` and drops them from the live sum by NAME INTERSECTION (never by subtracting an assumed value); `deploy.mem_min_admission_check` (`src/ciu/deploy.py:1400`) threads them through and logs the exclusion at `[INFO]`. **Approach 2 (label correlation) was rejected on evidence, not preference:** `governance.build_injections` emits only `cgroup_parent`/`mem_limit`/`memswap_limit`/`mem_reservation`/`cpus`/`blkio_config` (plus the KSM `environment`/`volumes` opt-in — still nothing label-shaped) — there is NO existing label infrastructure to build on, so that route meant inventing a labeling scheme, which the package's own scope explicitly forbade. Fails closed in every direction: no prior compose file (a genuine first deploy), unreadable/malformed YAML, no services, an exempt or unnameable service, a stale name, or a stopped container each yield no scope, so nothing is excluded and admission behaves exactly as before. **The dangerous direction is NOT unreachable in principle, per ciu-P51's own adversarial reviewer**: Docker's name uniqueness only guarantees at most one container holds a name at an instant, not that a name read out of a possibly-stale prior compose file still resolves to THIS entry's container — over-exclusion needs a stale name since claimed by a DIFFERENT entry AND that entry's container sharing the SAME guaranteed slice (exclusion is by intersection against `enumerate_slice_children`, so any other slice's container can never be dropped regardless); CIU's `{project}-{env}-{service}` naming makes the collision unlikely but does not rule it out — a known, low-probability residual risk, documented as such in SPEC.md and `deploy.py`'s own docstring, not a proven-impossible one. SPEC.md S15.23's known-limitation callout is replaced with the fix's own description (the separate concurrent-admission over-admit is called out there as still open and deliberately accepted). Oracles: `TestMemMinAdmissionExcludesOutgoingInstance` (`tests/tests/test_ciu_governance.py`, 7 tests — first-deploy regression guard, the redeploy fix, the exclusion-dropped controlled-wrong-implementation pinned as its own test, a DIFFERENT stack still refused, a second concurrent instance of the SAME stack still refused, a non-occupant exclusion as an exact no-op) + 9 tests in `tests/tests/test_ciu_deploy_actions.py` covering the correlation itself and the wiring |
| CIU-101 | `tests/tests/test_ciu_engine_luna_medium97.py::test_engine_module_guard_help_exits_cleanly_without_warning_summary` intermittently fails `assert all(item.category is RuntimeWarning for item in caught)` — some OTHER warning category (not identified further) is being caught alongside the expected `RuntimeWarning` in at least one full-suite run. Found by ciu-P51's adversarial reviewer (2026-09-08) while independently reproducing CIU-91's race — 1 failure in 15 full-suite (`run-ciu-tests.py`, `-n auto --dist loadfile`) runs against plain `main` (no ciu-P51 changes present at the commit tested), so this is unrelated to CIU-91/CIU-58/CIU-96/CIU-88 and pre-existing on `main`. Not investigated further — filed for visibility, same as CIU-58 was originally | Low | OPEN — found live, not reproduced a second time, no root cause yet |
| CIU-99 | **`ciu host enroll`'s printed one-liner names an installer whose *install* step cannot resolve an asset for ciu — ciu releases publish a bare wheel, not the release BUNDLE `get.py` downloads.** `cmru get-py`'s emitted installer resolves `<tag><asset_suffix>` (cmru `docs/SPEC.md` S6.3 step 3) from the GitHub release, verifies its SHA256 + minisign manifest, stages it and installs any `[[project.installer.wheels]]` it carries into a private venv. ciu's `[steps.push]` runs `cmru.handlers wheel-publish`, whose only assets are `ciu-<version>-py3-none-any.whl` + its `.sha256` (plus, since ciu-P52, `get.py` itself via `--extra-asset`) — there is no `ciu-v<version>.tar.xz`, no `manifest.json`, and the wheel's own filename does not match `<tag><asset_suffix>` under ANY suffix, because the tag is `ciu-v7.11.0` while the wheel is `ciu-7.11.0-py3-none-any.whl`. Consequence: the one-liner S14.7a prints is correct and its URL resolves (the `get.py` asset is real), but a target admin who runs it WITHOUT `--no-install` gets an install-step failure, and S14.7c's `ciu version` proof then fails with the tagged "key accepted but ciu is not installed" refusal — the enrollment stops one step short of its own contract ("enrollment ends at 'ciu can SSH in, and ciu is installed there'", proposal §10). Found by ciu-P52 while wiring `[project.installer]`, not by a consumer. Not fixed there because the fix is release machinery, not enrollment logic, and the package's own scope was the verb: it needs a build step that assembles a `ciu-v<version>.tar.xz` containing `vendor/ciu-*.whl` + a `manifest.json` with the wheel's sha256, and a publish step that attaches it — i.e. moving ciu from cmru's wheel-project shape toward tls-edge's tarball+installer shape, or adding a second artifact alongside the wheel. Interim, documented behaviour: `enroll --no-install` installs the deploy user + key + prints fingerprints, and the target's ciu is installed by whatever means the estate already uses. Oracles for the fix: a rendered `get.py install --version ciu-v<version>` against a real release resolves, verifies and installs a working `ciu` (the same container shape `tests/tests/test_ciu_host_enroll.py::TestEnrollEndToEnd` already builds), and S14.7's end-to-end chain then completes with the install step ENABLED — the controlled wrong implementation being a bundle whose manifest sha256 does not match the wheel it ships, which must fail verification before extraction | Medium | OPEN — found and documented by ciu-P52 (2026-09-08); `docs/SPEC.md` S14.7b and `cmru.toml`'s `[project.installer]` comment both name this entry so nobody reads the config as a working install path |
| CIU-100 | **`docs/SPEC.md` S14.7a specifies `--controller`'s auto-default source as `topology.external.public_fqdn`, but no config path in ciu ever sets that key — the real, established equivalent elsewhere in this codebase is `infrastructure.public_fqdn`** (`src/ciu/workspace_env.py:392`'s own resolution order, `docs/SPEC.md:219`). Confirmed by grep: `topology.external.public_fqdn` appears NOWHERE else in the source or docs except S14.7a's own text and `src/ciu/host_enroll.py`'s literal implementation of it (`resolve_controller`, `cli.py`'s own help text). Practical consequence: `ciu host enroll`'s documented convenience — "defaults from the declared controller, `--controller` only required when it is not declared" — never actually fires in real use, because the config key it reads is never set by anything; every real invocation silently requires `--controller` explicitly, with no error pointing at WHY the default never applied (the code correctly refuses with a clear message when NEITHER is present, so this is not a silent wrong answer, just a silently-dead convenience). Found and flagged, not fixed, by ciu-P52 (implementing S14.7 literally as written is the correct call per a fresh implementer's own scope — AUTHORING.md "product decisions are decisions, not silent workarounds" — this is a normative-text question for S14.7's own owner, not an implementation bug). Proposed fix: either correct S14.7a/proposal text to read `infrastructure.public_fqdn` (and `resolve_controller` to match) if that key is genuinely the intended source, or explain in S14.7a why host-enrollment needs a DIFFERENT, new config key than every other public-hostname consumer in this codebase — whichever is right, `resolve_controller` (`src/ciu/host_enroll.py:93-110`) and `cli.py:454`'s help text need to agree with the corrected spec | Low | OPEN — found by ciu-P52 (2026-09-08) |
...
See `nyxloom-trove/archive/ciu-P28-hotfix-worktree-branches-prune-safety-LOG.md`.
...
`nyxloom-trove/archive/ciu-P26-ciu25-lease-schema-and-labels-LOG.md`.
...
## CIU-97 — `docs/CONFIG.md`'s own `known_host` "format" prose contradicts its own worked examples (and the source), and caused a downstream consumer doc defect
