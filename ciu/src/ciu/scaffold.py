...
#
# CIU-61: reconciled against the repo root's `.gitignored.ciu` sample-rules
# file, which is this list's independent source of fact — the two had
# already drifted once (this list was missing three real entries) before
# either side had a mechanism to catch it. `tests/test_init_scaffolding.py`'s
# `test_gitignore_entries_match_gitignored_ciu_sample` now parses
# `.gitignored.ciu` and asserts the two AGREE on the set of real (non-comment)
# patterns, so a future drift fails loudly instead of silently, in either
# direction.
#
# Deliberately NOT listed, even though `ciu.toml`'s RENDERED form is above:
# `ciu.global.toml.j2` and `**/ciu.toml.j2` are committed, hand-authored
# SPARSE OVERRIDE templates (SPEC S3.1a, CIU-8) — CIU never auto-creates
# either one, and gitignoring them would silently drop operator-authored
# config from a consumer's own repo. `.gitignored.ciu` had exactly this bug
# (both wrongly listed as ignored) until CIU-61 corrected it in the same
# commit as this list's reconciliation — see that file's own callout.
_GITIGNORE_ENTRIES = (
    ("ciu.env", "shell export of generated machine facts (S2.7) — regenerated by `ciu env generate`"),
