...
def _project_relative_paths(
    value: object, where: str, project_root: Path,
) -> List[str]:
    """Parse project paths without allowing escapes or symlink hops.

    Evidence is commonly generated by a gate, so a declared path may be absent
    while configuration is loaded. Existing path components are still checked;
    the transaction repeats this check against the isolated source worktree.
    """
    ...
    for raw_path in paths:
        ...
        for part in candidate.parts:
            ...
            if current.is_symlink():
                _error(f"{where} path is or crosses a symlink: {raw_path!r}")
...
def _parse_project_document(
    ...
    _reject_unknown(
        release, {
            "git_tag", "build_step", "commit_generated", "changelog", "artifact_dirs",
            "evidence_paths",
        },
        "project.release",
    )
    ...
    evidence_paths = _project_relative_paths(
        release.get("evidence_paths", []),
        "project.release.evidence_paths",
