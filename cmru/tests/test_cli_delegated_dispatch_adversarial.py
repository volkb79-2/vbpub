...
def test_cli_delegates_handler_and_tester_gate_arguments(monkeypatch):
    ...
    cli.main(["handler", "wheel", "demo"])
    cli.main(["tester-gate", "--json", "demo"])
    assert calls == [
        ("handler", ["wheel", "demo"]),
        ("tester-gate", ["--json", "demo"]),
    ]
...
def test_cli_delegates_standards_resolve_and_get_commands(monkeypatch):
    ...
    cli.main(["resolve", "demo"])
    cli.main(["get-py", "demo", "--output", "x.whl"])
    assert calls == [
        ("standards", ["--check"]),
        ("resolve", ["demo"]),
        ("get", ["demo", "--output", "x.whl"]),
    ]
