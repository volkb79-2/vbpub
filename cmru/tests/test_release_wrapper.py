...
def _run(
    log_path: Path, cmru_bin: Path, *args: str,
) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["CMRU_RELEASE_LOG"] = str(log_path)
    env["CMRU_BIN"] = str(cmru_bin)
...
def test_release_wrapper_overwrites_then_appends_with_divider(tmp_path):
    ...
    fake_cmru = tmp_path / "cmru"
    fake_cmru.write_text(
        "#!/usr/bin/env bash\n"
        "printf 'fake cmru: %s --show-run-details\\n' \"$*\"\n",
        encoding="utf-8",
    )
    fake_cmru.chmod(0o755)

    first = _run(log, fake_cmru, "--help")
    ...
    second = _run(log, fake_cmru, "--help", "--log-append")
