"""Shared GitHub Releases publishing + resolution for the vbpub monorepo.
...
Design (see docs/plan-release-architecture.md):
...
installing anything — they just add ``release-manager/src`` to ``sys.path``.
"""
...
class GitHubReleases:
    ...
    def create_release(self, tag: str, title: str, notes: str,
        ...
        if target_commitish:
            # Pin the release (and the lightweight tag GitHub auto-creates when the tag
            # doesn't yet exist) to an exact commit, so it can't drift to default-branch
            # HEAD if `main` advanced between local tagging and this API call.
    ...
    * ``latest_pointer`` → refresh the thin ``<prefix>-latest`` redirect:
    ...
    if released:
        ...
        body = (notes or f"{prefix} {version}") + (
            ...
            f"(highest semver); see docs/plan-release-architecture.md."
        )
        ...
        # Dev/dirty build: no immutable version release. Consumers resolving latest via
        # latest.json or by scanning <prefix>-v* will not find a dev build — a real
        # release must exist for them to resolve. The -latest pointer below carries the
        # dev asset directly as a stopgap for CI iteration.
