...
`90a37794` (CIU instance `80e9c8`, created 2026-09-19 UTC). The released
...
| [`ciu/`](ciu/assay.toml) | `4a59d43a / 2026-09-22` | ✅ | ✅ | ✅ | ✅ | Assay requires branches and a 100% floor | Assay lane in `tester-unified`, `dev-gates.slice` | Final post-fix tester-unified verdict pending; an intermediate serialized run is active | Declared complete R0-R3 lane with full pytest-cov line+branch command | Hypothesis property tests cover path, size, and argument grammars with deterministic gate settings; Schemathesis N/A for this CLI surface | Bind the final exact-commit verdict and independent checklist review |
| [`cmru/`](cmru/assay.toml) | `0da8f158 / 2026-09-22` | ✅ | ✅ | ✅ | ✅ | Assay requires branches and a 100% floor | Assay plus raw `run-gate.toml` command lanes in `tester-unified`, `dev-gates.slice` | Final post-fix aggregate pending; preceding aggregate passed before the latest CIU/property/library updates | Declared complete R0-R3 lane plus supplemental coverage, mutation, canary, and enrollment lanes | Hypothesis property tests cover naming/scope invariants with deterministic gate settings; no OpenAPI surface for Schemathesis | Re-gate the final stable tip and bind the exact transaction verdict and independent checklist review |
| [`libraries/worktree/`](libraries/worktree/assay.toml) | `4a59d43a / 2026-09-22` | ✅ | ✅ | ✅ | ✅ | Assay requires branches and a 100% floor | Dedicated Assay lane in `tester-unified`, `dev-gates.slice` | Newly declared; final tester-unified verdict pending | Complete internal S1 R0-R3 lane with line+branch floor, serial mutation, and import-break canary | Hypothesis identity/path properties use deterministic gate settings; no API surface for Schemathesis | Execute the lane and bind its exact receipt; obtain independent checklist review |
| [`modern-debian-tools-python-debug/`](modern-debian-tools-python-debug/assay.toml) | `90a37794 / 2026-09-20` | ✅ | 🟡 | ✅ | ✅ | Branch data is collected, but `require_branch` is unset and the floor is 60% | Assay full lane plus host/VM command lanes | Assay receipt at `9653f0f7` ended `BUDGET_EXCEEDED`: 203 candidates, 107 killed, 91 survived, 5 budget-exceeded; `90a37794` carries the bounded Docker pull diagnostics and fixes the missing `mdt_cli` copy in the image build; corrected smoke evidence is pending | Project docs exist; the mutation receipt is a real non-green result, not a release certification | No project-owned Hypothesis tests; no HTTP API for Schemathesis | Decide the branch floor and close or explicitly accept the mutation gap; keep VM evidence separate |
...
| [`shared-ramdisk-depot-manager/`](shared-ramdisk-depot-manager/run-gate.toml) | `a4420c42 / 2026-09-20` | 🟡 Raw unit lane | 🟡 Raw coverage lane | ❌ | 🟡 Separate canary/e2e behavior | Go branch coverage is structurally unavailable | Raw unit/coverage/e2e commands; no `assay.toml` | Unit PASS through the declared `tester-unified-go`/SRDM gate container: `go test ./...`, `gofmt`, and `go vet` exit 0; coverage and privileged e2e are running against the same integration worktree with host-path and `dev-gates.slice` admission | No complete methodology inventory | No property framework found; consider Go fuzzing | Finish coverage/e2e or record their exact limitation before merging |
