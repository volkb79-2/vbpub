...
| [`cmru/`](cmru/assay.toml) | `0795ebb9 / 2026-09-19` | ✅ Assay R0 | ✅ Separate command lane | ✅ Separate mutation command | ✅ Separate canary command | Command lanes enforce 100% line+branch; Assay declaration remains R0 | Assay plus raw `run-gate.toml` command lanes in `tester-unified` | Prior to version/evidence merge: `cmru-v5.3.1` PASS, `1796 passed, 10 skipped`; final gate pending | Prior release evidence includes behavioral, coverage, mutation, and canary results | No project-owned Hypothesis tests; no OpenAPI surface for Schemathesis | Keep command-lane evidence bound to the promoted commit; rerun for `0795ebb9` |
| [`modern-debian-tools-python-debug/`](modern-debian-tools-python-debug/assay.toml) | `0795ebb9 / 2026-09-19` | ✅ | 🟡 | ✅ | ✅ | Branch data is collected, but `require_branch` is unset and the floor is 60% | Assay full lane plus host/VM command lanes | Prior smoke was dirty and ineligible; final release gate pending | Project docs exist; final checklist status remains open | No project-owned Hypothesis tests; no HTTP API for Schemathesis | Set the intended branch floor; add properties for planner/config invariants; keep VM evidence separate |
...
| [`shared-ramdisk-depot-manager/`](shared-ramdisk-depot-manager/run-gate.toml) | `0795ebb9 / 2026-09-19` | 🟡 Raw unit lane | 🟡 Raw coverage lane | ❌ | 🟡 Separate canary/e2e behavior | Go branch coverage is structurally unavailable | Raw unit/coverage/e2e commands; no `assay.toml` | No eligible run after version merge; Go toolchain unavailable in cockpit; final release gate pending | No complete methodology inventory | No property framework found; consider Go fuzzing | Add Assay R0/R1 or an explicit external receipt; record Go limitations |
...
| `scripts/debian-install-v2/` | `0795ebb9 / 2026-09-19` | ✅ Raw | 🟡 Raw branch-aware command plus VM lane | 🟡 Assay R2 has a known mutation gap and is excluded from aggregate | ✅ Raw canary | Branches are collected; no complete Assay branch policy | Assay R2 plus host and QEMU/VM command lanes | No eligible current full matrix; version surface awaits final gate; R2 baseline was not green | VM/testing docs exist; no complete checklist audit | No actual Hypothesis suite; VM acceptance is the important additional surface | Close or explicitly accept the R2 mutation gap; bind VM results to receipts |
