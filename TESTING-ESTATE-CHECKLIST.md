...
| `scripts/debian-install-v2/` | `61111578 / 2026-09-19` | ✅ Raw | 🟡 Raw branch-aware command plus VM lane | 🟡 Assay R2 has a known mutation gap and is excluded from aggregate | ✅ Raw canary | Assay R2 plus host and QEMU/VM command lanes | Container gate PASS: 317 passed, 11 skipped, 95% line/branch report; R3 PASS: 3 rejected, 0 survived; QEMU VM lane pending | VM/testing docs exist; the container gate is current but cannot certify VM-only swap/loop behavior | No actual Hypothesis suite; VM acceptance is the important additional surface | Run and bind the VM receipt; close or explicitly accept the R2 mutation gap |
