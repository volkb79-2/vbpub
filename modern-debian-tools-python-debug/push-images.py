...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Push modern-debian-tools-python-debug images")
    parser.add_argument(
        "--ignore-new-releases",
        action="store_true",
        help=(
            "Continue push even when dynamic pre-check detects newer stable devcontainer "
...
def main() -> None:
    ...
    try:
        ...
        if exc.returncode == 3 and exc.cmd and "resolve-devcontainers-release.py" in " ".join(map(str, exc.cmd)):
            ...
            print(
                "\n[ERROR] Push stopped because newer stable devcontainer release(s) were detected.",
                file=sys.stderr,
            )
            print(
            ...
            print("[HINT]   ./push-images.py --ignore-new-releases", file=sys.stderr)
            raise SystemExit(3) from None
        print("[ERROR] Push failed. Review command output and log path above.", file=sys.stderr)
