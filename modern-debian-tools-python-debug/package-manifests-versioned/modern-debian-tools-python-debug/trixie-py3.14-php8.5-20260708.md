...
This image provides a Debian `trixie`-based development and debugging environment
with Python 3.14 and PHP 8.5 runtimes, augmented with first-party Python wheels
(`ciu`, `cmru`), AI CLI tools, container inspection utilities, security scanners,
and a full suite of system-level debugging and administration packages. It is
designed for interactive troubleshooting, CI/CD pipeline debugging, and
developer workstation use.
...
This image ships both Python 3.14 and PHP 8.5 runtimes for polyglot debugging
and automation workflows.

| Runtime | Version | Source |
|---------|---------|--------|
| Python | `3.14` | Debian `trixie` (apt) |
| PHP | `8.5` | Debian `trixie` (apt) |

## Version Selection Policies

- **Debian packages** — versions are pinned from the `trixie` repository at
  build time via `apt`. The snapshot above reflects the candidate versions at
  the time of the build.
- **First-party wheels** — versions are pinned in the project's build
  configuration and verified by SHA256 digest.
- **AI CLI tools and supporting binaries** — each tool is pinned to a specific
  upstream release version; artifacts are downloaded from the official
  distribution URL and verified against published checksums where available.
- **Floating tag** (`trixie-py3.14-php8.5-latest`) points to the most recent
  build for this Debian/runtime combination.
