...
Debian `trixie`-based VS Code devcontainer image bundling Python 3.14, PHP 8.5, and a curated suite of modern developer, debugging, and operations tools. Designed for interactive development, CI/CD pipelines, and remote debugging sessions where a rich CLI environment is needed out of the box.

## Python & PHP Runtime

Pre-installed Python and PHP runtimes with common extensions and tooling.

| Runtime | Version | Key Packages |
|---|---|---|
| Python | `3.14` | `python3-venv` (`3.13.5-1`) |
| PHP | `8.5` | (provided via OS packages) |
| OCI/Container | — | `skopeo` (`1.18.0+ds1-1+b5`) |

## First-Party Wheels

First-party Python wheels built and bundled into the image.

| Wheel | Version | SHA256 Digest |
|---|---|---|
...
| Category | Policy |
|---|---|
| AI CLI Tools | Latest npm/GitHub release at build time (override via build arg). Resolved dynamically during `stage_tool_artifacts` from npm, PyPI, and GitHub Releases. |
| Container Inspection Tools | Latest GitHub release at build time (override via build arg). Downloaded as pre-built binaries from upstream releases. |
| Security & Debug Tools | Latest GitHub release at build time (override via build arg). Binaries verified via upstream SHA256 checksums before installation. |
| Custom Tooling | Latest GitHub release at build time (override via build arg). Compiled from source where applicable (nvim, htop); remainder are pre-built binaries. |
| System Packages | Version pinned from the Debian `trixie` repository at build time via `apt`. |
| First-Party Wheels | Version pinned at image build time; built and bundled from internal CI. |
