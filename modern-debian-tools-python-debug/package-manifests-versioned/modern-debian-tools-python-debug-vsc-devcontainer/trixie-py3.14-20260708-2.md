...
Provides a Debian trixie-based VS Code devcontainer environment with Python 3.14, first-party CI/CD tooling (CIU, cmru), and a comprehensive suite of debugging, security, and container inspection tools. Designed for software development workflows requiring immediate access to modern CLI tooling (AI assistants, database clients, file utilities) without manual installation.
...
| `claude` | `2.1.204` | latest | https://github.com/anthropics/claude-code |  |
| `codex` | `0.143.0` | latest | https://github.com/openai/codex |  |
| `openclaw` | `2026.6.11` | latest | https://github.com/openclaw/openclaw |  |
| `reasonix` | `1.17.7` | latest | https://github.com/reasonix/reasonix |  |
...
| `dtop` | `0.7.7` | latest | https://github.com/amir20/dtop |  |
...
- Python interpreter: `3.14` (from Debian trixie)

## Version Selection Policies

All staged tool versions are resolved to their **latest stable release** at build time from upstream package registries or GitHub releases, unless overridden via a build-time argument. This ensures the image always ships the most recent available versions while allowing pinning for reproducibility. First-party wheels (CIU, cmru) follow the same latest-from-PyPI policy. System packages come from the Debian trixie repository.
