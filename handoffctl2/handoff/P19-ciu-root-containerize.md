# P19 — handoffctl2 as a ciu root stack; controller in a container

> Tier: sonnet5-high · Date: 2026-07-15 · User directive: ciu global lives
> in `handoffctl2/`; `ntfy` becomes one stack; the controller/daemon moves
> to `handoffctl2/controller/` as its OWN ciu project stack with a
> Dockerfile (run the daemon in a container, not directly). handoffctl2
...
## Owned paths
- NEW `handoffctl2/ciu.global.defaults.toml.j2` + `ciu.global.toml.j2` (root)
...
- NEW `handoffctl2/controller/`: `Dockerfile`, `ciu.defaults.toml.j2`,
  `ciu.compose.yml.j2`, a small `README.md`.
- `docs/DEPLOY.md` (how to bring the root up).
- Do NOT touch `src/handoffctl/**` logic (only add a console entry if
  needed) or the tests.
...
- **the XDG state dir** ($XDG_STATE_HOME/handoffctl) on a persistent volume
...
handoffctl2/controller starts the daemon; the daemon does NOT manage its own
...
1. `ciu up --dir . ` (or the documented root command) from handoffctl2/
   renders and starts BOTH the ntfy stack and the controller stack under one
   root global; container `handoffctl-<env>-controller` reaches healthy.
2. Inside the controller container: `docker ps` works (sock reachable),
   ...
   works (CLI+auth present), and `handoffctl status --project groop` reads
