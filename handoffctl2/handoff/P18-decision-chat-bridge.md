...
## Owned files
- `src/handoffctl/decision_chat.py` (new — the bridge)
- `src/handoffctl/daemon.py` (wire the listener + endpoints; ~30 lines)
- `src/handoffctl/render.py` (decisions page: list OPEN decisions, per-entry
  chat transcript + an answer box)
- `src/handoffctl/config.py` (ONLY: NotifyConfig.decision_topic +
...
## Channel + security
- New ntfy topic (config `decision_topic`, e.g. `handoffctl-decisions`) with
