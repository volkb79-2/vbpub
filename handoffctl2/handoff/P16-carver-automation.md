...
## Owned files
- `src/handoffctl/reconcile.py` (carve-trigger planning),
  `src/handoffctl/daemon.py` (CarveDispatch execution + summary persistence
  + endpoints), `src/handoffctl/render.py` (summary interleave + toggle),
  `src/handoffctl/config.py` (ONLY: add Policy fields `carve_ahead_target:
...
5. **Persistence + UI interleave** (render): CarveSummaries are stored as
   files under the state dir (`$XDG_STATE/handoffctl/<project>/carves/
