...
> tests) · Read first: handoff/STANDING.md, src/handoffctl/cli.py
> (docstring = normative subcommand table), types.py, storage.py.

## Owned files
- `src/handoffctl/cli.py`
- `tests/test_cli.py`
...
12. `version` prints __version__ and exits 0 even when every sibling
    ...
    assert 'import handoffctl.daemon' does NOT appear at module top —
    ...
    sys.modules['handoffctl.daemon']=None).
