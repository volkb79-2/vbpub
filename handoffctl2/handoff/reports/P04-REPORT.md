...
- `src/handoffctl/wrapper.py` — implementation of WrapperSpec, launch_detached, wrapper_main (10 steps)
- `tests/test_wrapper.py` — comprehensive test suite with 10 test classes covering all oracles

## Gate command

```
cd /workspaces/vbpub/handoffctl2 && /workspaces/vbpub/.venv/bin/python -m pytest tests/test_wrapper.py -q
```
...
### Wrapper bugs found and fixed by the real tests (src/handoffctl/wrapper.py)
