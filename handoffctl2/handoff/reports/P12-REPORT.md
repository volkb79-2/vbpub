...
Command: `cd /workspaces/vbpub/handoffctl2 && /workspaces/vbpub/.venv/bin/python -m pytest tests/test_commands.py -q`
...
Command (full suite, oracle 7): `cd /workspaces/vbpub/handoffctl2 && /workspaces/vbpub/.venv/bin/python -m pytest tests/ -q`
...
| 4 | `handoffd-reply` tag -> `None` (loop guard) | `test_handoffd_reply_tag_is_ignored` | pass |
| 5 | Transport: reply POST (publisher token, `handoffd-reply` tag) + reconnect carrying `since` after connection ends | `test_transport_reply_and_reconnect_carries_since` | pass |
...
- `src/handoffctl/commands.py` (new) — `CommandListener` class: pure verb
...
- `tests/test_commands.py` (new) — 16 tests incl. a local fake-ntfy-server
  ...
  transport oracle.
- `src/handoffctl/daemon.py` — narrow ~18-line exception per the handoff:
