...
`src/handoffctl/storage.py`, `apply_event()`, the `TASK_TRANSITIONED` /
...
- `src/handoffctl/storage.py` (edited — owned path per handoff)
...
```
cd /workspaces/vbpub/.worktrees/handoffctl2-P20/handoffctl2 && PYTHONPATH=src /workspaces/vbpub/.venv/bin/python -m pytest tests/ -q
```
