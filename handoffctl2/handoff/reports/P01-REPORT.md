...
Implemented complete frontmatter parsing and lint rule checking for handoffctl2. All contract requirements from P01-frontmatter-lint.md are satisfied.

## Implementation

### Files implemented
- `src/handoffctl/frontmatter.py` (all 5 functions)
- `src/handoffctl/lint.py` (all 3 public functions + 12 rule checkers)
...
```
cd /workspaces/vbpub/handoffctl2 && /workspaces/vbpub/.venv/bin/python -m pytest tests/test_frontmatter.py tests/test_lint.py -q
...
- `src/handoffctl/frontmatter.py` (NEW - 172 lines)
- `src/handoffctl/lint.py` (NEW - 510 lines)
