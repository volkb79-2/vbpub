# Review of handoffctl (draft 1) — and why draft 2 exists

Date: 2026-07-15 · Reviewer: frontier session (Fable 5) · Scope: full read of
`handoffctl/{README.md,docs/ARCHITECTURE.md,docs/SPEC.md,docs/MIGRATION.md,docs/ROADMAP.md,schemas/*}`
...
contract) **and** `.handoffctl/handoffs/<task>.json` (machine metadata). Two
...
`handoffd` (ARCHITECTURE.md §Control plane, ROADMAP phases 2–4) owns subprocess
...
`$XDG_STATE_HOME/handoffctl/leases/`. **Draft 2:** flock-based leases; a
...
click-through to the dashboard entry; `handoffctl decide D-0XX --choose b`
...
No pause. The user's emergency brake today is killing sessions. `handoffctl
