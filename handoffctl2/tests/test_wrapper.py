...
from handoffctl.wrapper import WrapperSpec, launch_detached, wrapper_main, SESSION_CAPTURE_DELAY
...
def mock_adapters():
    ...
    with patch("handoffctl.wrapper.adapters") as m:
...
class TestLeaseRace:
    ...
    def test_lease_race(self, tmp_state, tmp_path, fake_cli, mock_adapters):
        ...
        from handoffctl import leases as leases_module
...
class TestDetach:
    ...
    def test_launch_detached_script(self, tmp_state, tmp_path, fake_cli):
        ...
        # Mock adapters
        with patch("handoffctl.wrapper.adapters") as mock_adapters:
            mock_adapters.extract_usage.return_value = Usage(basis=Basis.UNKNOWN)
            mock_adapters.capture_session.return_value = None
            mock_adapters.classify_log_tail.return_value = None

            # Patch SESSION_CAPTURE_DELAY to 0 for faster testing
            with patch("handoffctl.wrapper.SESSION_CAPTURE_DELAY", 0):
