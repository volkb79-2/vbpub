"""Tests for handoffctl.commands (P12: ntfy inbound command listener)."""
...
from handoffctl.commands import (
    CommandListener, HELP_TEXT, REPLY_TAG, UNKNOWN_REPLY,
)
from handoffctl.config import load_registry, register_project
...
# =========================================================================
# Oracle 4: handoffd-reply tag is the loop guard
# =========================================================================

def test_handoffd_reply_tag_is_ignored(sample_project):
    cl = CommandListener(load_registry())
...
def _register_cmd_project(tmp_path, port: int):
    ...
    (root / ".handoffctl" / "project.toml").write_text(_CMD_PROJECT_TOML.format(port=port))
...
def test_digest_verb_truncates_to_max_chars(tmp_state, sample_project, monkeypatch):
    monkeypatch.setattr(
        "handoffctl.commands.notify.digest",
