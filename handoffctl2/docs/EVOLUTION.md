...
| Sonnet controller session + heartbeats | `handoffctl tick` | retired at M2 |
...
| `controller-workflow-v2.md` | stays the *role/protocol* rationale; operational sections marked "absorbed by handoffctl2 §…" as each milestone lands | shrinks over time |
| dstdns `docs/ai-dev/controller-workflow.md` | project deltas move into `.handoffctl/project.toml` + project adapter; doc keeps the narrative why | shrinks at M4 |
