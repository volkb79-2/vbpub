...
.handoffctl/project.toml        # project policy: gates, mutexes, globs, caps, redaction
...
Writes go through `handoffctl` commands: append event + rewrite statefile via
...
`handoffctl tick` is a stateless reconciler — the v2 §10 controller loop with
...
timer on the host; inside a devcontainer without an init, `handoffctl tick
...
wrapper (or by `handoffctl lease run <resource> -- <cmd>` for manual work):
...
`handoffctl render` regenerates `www/` on every tick and on every mutating
...
3. User decides, three equivalent surfaces:
   - `handoffctl decide D-013 --choose b --note "..."` (CLI);
   ...
   - **discuss first**: open any Claude surface — mobile app, claude.ai/code,
     ...
     resume prompt (`handoffctl discuss D-013` prints/launches it). The
...
so the operator has one channel — but `handoffctl` events remain the workflow
truth. Tokens are spent only when the user opts into a discussion.

## 9. When a daemon would actually earn its place

Graduate from ticks to a resident `handoffd` (draft 1's shape) only when a
