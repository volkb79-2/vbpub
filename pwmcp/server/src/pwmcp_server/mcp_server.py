...
from pwmcp_shared.constants import (
    DEFAULT_MCP_HOST,
    DEFAULT_MCP_PORT,
...
MCP_SERVER_NAME = os.getenv(ENV_MCP_SERVER_NAME, DEFAULT_MCP_SERVER_NAME)
...
MCP_AUTH_TOKEN = os.getenv(ENV_MCP_AUTH_TOKEN, '')
AUTH_REQUIRED = os.getenv(ENV_AUTH_REQUIRED, 'true').lower() == 'true'

WS_ARTIFACT_ROOT = os.getenv("WS_ARTIFACT_ROOT", "/workspaces/artifacts")
...
async def screenshot(path: Optional[str] = None, full_page: bool = True) -> dict:
    try:
        ...
        screenshot_path = Path(WS_ARTIFACT_ROOT) / Path(path).name
