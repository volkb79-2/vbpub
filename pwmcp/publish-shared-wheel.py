#!/usr/bin/env python3
"""Publish PWMCP shared wheel to GitHub Releases."""
...
def main() -> None:
    ...
    project_root = root / "shared"
    dist_dir = root / "dist" / "shared"
    ...
    package_name = os.getenv("PWMCP_PACKAGE_NAME", project_meta.get("name", "pwmcp-shared"))
    ...
    release_notes = os.getenv(
        "PWMCP_SHARED_RELEASE_NOTES",
        (
            f"{package_name} wheel {version}\n\n"
            "Includes:\n"
            "- PWMCP shared library\n"
        ),
    )

    latest_notes = os.getenv(
        "PWMCP_SHARED_LATEST_NOTES",
        (
