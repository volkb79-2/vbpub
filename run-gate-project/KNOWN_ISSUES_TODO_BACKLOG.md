...
| RG-30 | `doctor` and `--check-env` both pass `None` to `resolve_repo_and_worktree` (`run-gate.py:1789`, `:2068`) instead of the caller's `--worktree` value, so `doctor --worktree B` silently reports the INVOKING tree's answers, not B's — including RG-21's worktree-specific host-lane git-view WARN, which is exactly the per-tree answer that can legitimately differ | Medium | OPEN 2026-08-31 — found by the run-gate-P03 reviewer while verifying RG-27's `history --worktree` disclosure fix; RG-27 closed the identical read-scope hazard for `history`, this is the last remaining instance of that same pattern estate-wide. Proposed: thread `--worktree` into both call sites the same way `history`'s fix does, with the same disclosure (name which tree an answer describes) rather than silent invoking-tree fallback |
