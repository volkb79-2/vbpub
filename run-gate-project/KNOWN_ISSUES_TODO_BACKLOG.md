...
| RG-31 | `assay_toolchain_findings()`'s own `resolve_repo_and_worktree` call (the toolchain-fitness probe shared by `doctor` check 5 and `--check-env`) still takes the RAW `worktree_override` string, not RG-30's new validated `resolve_worktree_scope()` — so a bad `--worktree` combined with an assay lane present degrades safely (a `[SKIP]` on that check, no false-`[OK]`) but with a MISLEADING reason string (blames "an assay older than 3.2.0" rather than naming the real `--worktree` problem, which `doctor` check 3 already reported correctly two checks earlier in the same report) | Low | OPEN — found by the run-gate-P04 reviewer (2026-08-31) while re-verifying RG-30's fix; reproduced directly (bad `--worktree` + an assay lane present): no crash, no false certification, just a confusing SKIP reason. Non-blocking for RG-30 (the actual read-scope hazard — wrong-tree-under-right-name — is fully closed regardless, since check 3's `[FAIL] git` line already names the real cause in the same report). Fix: either route `assay_toolchain_findings`'s override through `resolve_worktree_scope` too (so a bad override refuses/degrades the SAME way in both places), or make the SKIP reason distinguish "no assay installed" from "the requested worktree could not be resolved" |
