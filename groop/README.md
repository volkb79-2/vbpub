...
| P66 | Ready | Daemon client versioned health | v2/v3 API | Extend the P63 typed `DaemonClient` with the versioned `health` op; independent small prerequisite for P92. Handoff: `handoff/P66-daemon-client-versioned-health.md`. |
...
| P81 | Ready; priority 1 | Shared fail-closed redaction | operator-console security | One server-side enforcement point and typed marker across HTTP/MCP and future query/process/lifecycle payloads. Handoff: `handoff/P81-redaction-single-enforcement-point.md`. |
...
| P86 | Ready | CIU-grouped TUI end-to-end gate | current TUI | Drive the real view and prove synthetic group rows stay inert. Handoff: `handoff/P86-ciu-grouped-view-end-to-end.md`. |
| P87 | Ready; priority 1 | Docker action owner/protected-ID safety | lifecycle safety | Close the 64-hex protected-ID bypass and refuse raw mutation of owner-managed containers. Handoff: `handoff/P87-docker-action-owner-safety.md`. |
