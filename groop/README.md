...
| P67 | Done | Versioned read HTTP gateway (trust-boundary hardened) | v2/v3 API | Stdlib, loopback-default HTTP gateway over P63 typed reads only; trusted-local-proxy identities map to server-side `Sensitivity` ceilings, and mutating/CORS routes are structurally absent. `health` remains gated pending P66's typed versioned client method. Handoff: `handoff/P67-versioned-read-http-gateway.md`. Report: `handoff/reports/P67-REPORT.md`. |
...
| P79 | Queued | Corrupt recording inputs are typed errors | v1.5 recording | A corrupt/truncated `.jsonl.zst` currently crashes `groop report` with a raw `ZstdError` traceback and exit 1 — a raw exception across a CLI boundary. Make every damaged-input failure a typed exit-2 error, and fix the environment-conditional test that hid it (it only exercises the missing-extra path, so the suite has been RED on main in the clean venv). Carve source: **review-derived** — found by rerunning the gates from main during the P72/P74 merge, not from either diff. Handoff: `handoff/P79-corrupt-recording-typed-errors.md`. |
