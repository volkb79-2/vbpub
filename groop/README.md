...
| P88 | Done | Unified frame query core | operator-console core | `src/groop/query/`: one typed `FrameSource` over recordings and P63 daemon history (zero P2/P52 wire change), strict `Query` object, six declared value semantics reusing P54/P70 math with differential oracles, registry-policy subtree aggregation, coverage/gap/eviction/reset truth in every result, pre-materialization row/point/byte bounds, byte-identical payloads across sources, and the `groop query --json` CLI. Handoff: `handoff/P88-unified-frame-query-core.md`. Report: `handoff/reports/P88-REPORT.md`. |
