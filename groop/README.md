...
| P67 | Queued | Versioned read HTTP gateway | v2/v3 API | HTTP/WebSocket gateway over the P52 envelope via the P63 typed client. Carve source: review-derived (P63). Prerequisite for P69. Handoff: `handoff/P67-versioned-read-http-gateway.md`. |
...
| P69 | Queued | Web UI over daemon API — scoping & analysis | v2/v3 product | Docs-only scoping package for the standing web-UI product goal: read-surface gap analysis, page inventory, sensitivity/redaction UX, trust boundary vs P67, framework recommendation, and DECISIONS-INBOX entries. No source changes; output feeds the implementation carves. Carve source: **product-goal-driven**. Handoff: `handoff/P69-web-ui-scoping.md`. |
| P70 | Queued | Steady-state detector performance | v1.5 recording | Make `--window auto` linear-time without changing which window it selects (measured: 0.98s at 480 frames, quadratic → ~35-90s at the 4h/5s default profile). Differential test against the current detector is the acceptance oracle. Serialize-with P64/P65. Carve source: review-derived (P62). Handoff: `handoff/P70-report-detector-performance.md`. |
| P71 | Queued | ZFS ARC host provider | optional plugins | Host-level ARC size/target/max/min/hit-ratio metrics + banner annotation, so groop stops reporting false memory pressure on ZFS hosts. No per-cgroup ARC claim. Carve source: **roadmap-driven** (first package ever carved from the Optional-plugins bucket). Handoff: `handoff/P71-zfs-arc-provider.md`. |
| P72 | Queued | Admin action kill/update verbs | v2 actions | Add `kill` (closed signal allowlist, `--force` for KILL, protected-entity refusal) and `update` (Docker `--memory`/`--cpus`, refuse limits below current usage) to the P46 execution kernel. Closes the action verb set. Carve source: **roadmap-driven**. Handoff: `handoff/P72-admin-action-kill-update.md`. |
