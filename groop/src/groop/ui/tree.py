...
from .table import RenderedRows, _make_table, _row_cells, display_name, format_metric_value, metric_sort_value, resolve_columns
...
def render_tree_table(
    ...
    *,
    ...
    table = _make_table(columns, title=f"TREE | profile={profile} | sort={sort_by or 'name'}")
    row_keys: list[str] = []
    rows = _ordered_rows(frame, sort_by=sort_by, filter_text=filter_text)
    for depth, entity_frame in rows:
        row_keys.append(entity_frame.entity.key)
        cells = [format_metric_value(column_name, entity_frame) for column_name in columns]
        if cells:
            prefix = _tree_prefix(frame, entity_frame, depth)
            cells[0] = prefix + cells[0]
        table.add_row(*_row_cells_from_cells(cells, selected=entity_frame.entity.key == selected_key))
    if not row_keys:
        table.add_row("no rows", *[""] * (max(0, len(columns) - 1)))
    return RenderedRows(table=table, row_keys=tuple(row_keys), title=table.title or "")


def _ordered_rows(frame: Frame, *, sort_by: str, filter_text: str) -> list[tuple[int, EntityFrame]]:
