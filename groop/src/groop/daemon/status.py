...
speaking the expected groop frame protocol?"
"""
...
class DaemonStatusReport:
    ...
    def to_text(self) -> str:
        lines = [
            f"groop daemon status: {self.socket_path}",
            f"Group: {self.group_name}",
            "",
            "--- Preflight ---",
        ]
        if self.preflight is not None:
            from groop.daemon.deploy import render_preflight_text
...
def _check_protocol(socket_path: Path) -> ProtocolStatus:
    ...
    try:
        ...
        if socket_path == DEFAULT_DAEMON_SOCKET:
            guidance = " Try: groop daemon preflight. If not installed: groop daemon install-plan."
        else:
            guidance = f" Try: groop daemon preflight --socket {socket_path}."
        ...
        return ProtocolStatus(
            ok=False,
            message=f"Protocol error: {exc}. Check that the process at the socket is a compatible groop daemon and review daemon logs.",
        )
