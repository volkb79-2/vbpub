...
from groop.model import EntityFrame, Finding
...
class Rule:
    ...
    evaluator: Callable[[EntityFrame, GroopConfig], RuleMatch | None]


def evaluate_rules(entity_frame: EntityFrame, config: GroopConfig) -> list[Finding]:
...
def _protected_disk_refault(entity_frame: EntityFrame, config: GroopConfig) -> RuleMatch | None:
...
def _protected_file_refault(entity_frame: EntityFrame, config: GroopConfig) -> RuleMatch | None:
    if not entity_frame.entity.is_protected:
        return None
...
def _memory_high_rising(entity_frame: EntityFrame, config: GroopConfig) -> RuleMatch | None:
...
def _memory_high_user_visible(entity_frame: EntityFrame, config: GroopConfig) -> RuleMatch | None:
...
def _io_cap_expected_throttle(entity_frame: EntityFrame, config: GroopConfig) -> RuleMatch | None:
...
def _governance_drift(entity_frame: EntityFrame, _config: GroopConfig) -> RuleMatch | None:
...
def _socket_buffers_material(entity_frame: EntityFrame, config: GroopConfig) -> RuleMatch | None:
...
def _host_netns_na(entity_frame: EntityFrame, _config: GroopConfig) -> RuleMatch | None:
