...
REGISTRY: dict[str, MetricSpec] = {
    ...
    "host_zfs_arc_size": _m("host_zfs_arc_size", "bytes", "gauge", "local", "n/a", False, ("/proc/spl/kstat/zfs/arcstats:size",), "Current ZFS ARC size in bytes. The ARC is the Adaptive Replacement Cache, a kernel-managed memory pool that automatically shrinks under pressure. On ZFS hosts this memory is reported as used by MemAvailable, so the ARC annotation helps operators see how much 'used' RAM is actually reclaimable cache."),
    "host_zfs_arc_target": _m("host_zfs_arc_target", "bytes", "gauge", "local", "n/a", False, ("/proc/spl/kstat/zfs/arcstats:c",), "Current ZFS ARC target size in bytes. The ARC target is the size the ARC is attempting to reach or maintain. It fluctuates based on memory pressure and system activity."),
    "host_zfs_arc_max": _m("host_zfs_arc_max", "bytes", "gauge", "local", "n/a", False, ("/proc/spl/kstat/zfs/arcstats:c_max",), "ZFS ARC maximum size in bytes (the zfs_arc_max tunable). This is the hard ceiling operators configure to cap ARC memory consumption."),
    "host_zfs_arc_min": _m("host_zfs_arc_min", "bytes", "gauge", "local", "n/a", False, ("/proc/spl/kstat/zfs/arcstats:c_min",), "ZFS ARC minimum size in bytes (the zfs_arc_min tunable). The ARC will not shrink below this floor."),
    "host_zfs_arc_hit_ratio": _m("host_zfs_arc_hit_ratio", "ratio", "derived", "local", "n/a", False, ("/proc/spl/kstat/zfs/arcstats:hits", "/proc/spl/kstat/zfs/arcstats:misses"), "ZFS ARC hit ratio computed as a rate over the sample interval: delta hits / (delta hits + delta misses). A high ratio (>90%) means the ARC is effectively caching; a falling ratio suggests working-set pressure. The raw hit counter is carried in MetricValue.raw for reset detection. Per-cgroup ARC attribution is unavailable from the kernel."),
    "host_gpu_vram_total": _m("host_gpu_vram_total", "bytes", "gauge", "local", "n/a", False, ("/sys/class/drm/card*/device/mem_info_vram_total",), "Host GPU VRAM total in bytes summed across all amdgpu cards. Not every GPU driver exposes this file: amdgpu does; i915/xe and the nvidia proprietary driver do not. On such cards the metric reports unavail_kernel. Per-cgroup GPU attribution is unavailable from the kernel."),
    ...
    "host_gpu_count": _m("host_gpu_count", "count", "gauge", "local", "n/a", False, ("/sys/class/drm/card*",), "Number of DRM render cards (cardN nodes) detected under /sys/class/drm. Connector nodes (e.g. card0-DP-1) are not counted. This metric always reports a count (even 0 when no card exists) and is available on all hosts."),
}
