...
REGISTRY: dict[str, MetricSpec] = {
    ...
    "cpu_quota_us": _m("cpu_quota_us", "us", "gauge", "local", "local_only", False, ("cpu.max",), "CPU quota in microseconds; unavailable when cpu.max is unlimited."),
    ...
    "mem_high": _m("mem_high", "bytes", "gauge", "local", "local_only", False, ("memory.high",), "Configured memory.high throttle threshold; unavailable when unlimited."),
    "mem_max": _m("mem_max", "bytes", "gauge", "local", "local_only", False, ("memory.max",), "Configured memory.max hard limit; unavailable when unlimited."),
    ...
    "pids_max": _m("pids_max", "count", "gauge", "local", "local_only", False, ("pids.max",), "Configured pids.max limit; unavailable when unlimited."),
    ...
    "net_rx_bps": _m("net_rx_bps", "bytes/s", "derived", "local", "n/a", False, ("provider:network",), "Per-entity network receive rate. P1 emits unavailable placeholders until P3 providers exist."),
    "net_tx_bps": _m("net_tx_bps", "bytes/s", "derived", "local", "n/a", False, ("provider:network",), "Per-entity network transmit rate. P1 emits unavailable placeholders until P3 providers exist."),
    "pressure": _m("pressure", "count", "derived", "local", "n/a", False, ("diag:score",), "Registry-backed pressure score placeholder until P6 diagnostics exist."),
