...
def run_bpf_gate(
    *,
    ...
    pin_root: Path = Path("/sys/fs/bpf/groop"),
    ...
    return BpfGateReport(
        ...
        probe_commands=(
            ...
            "groop bpf gate --proc-root <fixture-or-live-proc> --json",
        ),
        live_commands=(
            "bpftool prog load <ingress.o> /sys/fs/bpf/groop/<name>/ingress",
            "bpftool prog load <egress.o> /sys/fs/bpf/groop/<name>/egress",
            "bpftool cgroup attach /sys/fs/cgroup cgroup_skb ingress pinned /sys/fs/bpf/groop/<name>/ingress",
            "bpftool cgroup attach /sys/fs/cgroup cgroup_skb egress pinned /sys/fs/bpf/groop/<name>/egress",
        ),
        baseline=baseline,
    )
