...
``groop.actions.squeeze`` (suffix handling, overflow, max, range) — the
...
``groop action set-property`` — no second governance path.
...
from groop.actions.catalog import DOCKER_EXECUTABLE, ActionKind
...
def _reject_systemd_target(target: str) -> None:
    ...
    if _SYSTEMD_SUFFIX_RE.search(target):
        raise ValueError(
            ...
            "use 'groop action set-property' for systemd resource changes"
        )
