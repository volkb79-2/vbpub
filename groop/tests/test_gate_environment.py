...
def test_dev_extra_installs_every_other_optional_extra() -> None:
    """``pip install -e 'groop[dev]'`` must build a gate env that reaches
    ...
    for extra in optional:
        if extra == "dev":
            continue
        assert f"groop[{extra}]" in dev_requirements, (
            f"optional extra {extra!r} is not installed by 'groop[dev]', so the "
