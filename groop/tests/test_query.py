"""Tests for groop.query — the P88 unified bounded frame query core.
...
import groop.query.semantics as semantics_module
...
class TestFrameSource:
    ...
    def test_from_history_result_bridges_the_p63_typed_read(self):
        ...
        from groop.daemon.client import DaemonHistoryResult
        from groop.query.source import DaemonHistoryFrameSource as DFS
...
class TestDeterminism:
    ...
    def test_cli_stdout_byte_identical(self, tmp_path):
        ...
        with RecordWriter(path, fsync=False) as w:
            for f in frames:
                w.write_frame(f)
        cmd = [sys.executable, "-m", "groop.cli", "query", str(path), "--shape", "summary", "--metric", "ram", "--json"]
