...
from groop.damon.control import APPROVAL_TEXT, NoFreeKdamond, RootRequired, stop_owned_sessions
from groop.damon.paddr import plan_start_paddr_session, start_paddr_session
...
def test_paddr_control_starts_owned_host_session_and_passive_reads_it(tmp_path: Path) -> None:
    ...
    assert frame.entities[""].damon["host_sessions"][0]["owner"] == "groop"
...
def test_paddr_cli_start_uses_fixture_root(tmp_path: Path) -> None:
    ...
    assert proc.stdout.strip() == "started groop-owned paddr DAMON session on kdamond 0"
    assert (damon_root / "0" / "contexts" / "0" / "operations").read_text().strip() == "paddr"
...
def test_paddr_banner_and_host_memory_status_render_foreign_session(tmp_path: Path) -> None:
    ...
    banner = render_banner(frame, GroopConfig())
    ...
    text = render_host_memory_text(frame, config=GroopConfig(), damon_root=damon_root)
