...
from groop.model import Frame, MetricValue, frame_to_jsonable
from groop.record.headless import (
...
class TestHeadlessRecordDriver:
    ...
    def test_no_textual_import_on_headless_path(self, tmp_path: Path) -> None:
        ...
        test_code = """
import sys
sys.path.insert(0, "groop/src")
from pathlib import Path
import tempfile

from groop.collect.collector import Collector
from groop.config import GroopConfig
from groop.record.headless import HeadlessRecordDriver
from groop.record.writer import RecordWriter

# Minimal collector for a single frame
cgroup_root = Path("groop/tests/fixtures/cgroupfs/gstammtisch")
collector = Collector(
...
"""
        ...
        proc = subprocess.run(
            ...
            cwd=fixture_root().parents[1].parent,  # groop/ directory
