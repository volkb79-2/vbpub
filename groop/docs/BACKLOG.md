...
| B-042 | implementer-report | `P88-REPORT.md` | `groop query` offers no flat slice-grain rollup (`group_by slice`); the hierarchy projection surfaces slice structure and `groop report` keeps its rollup. A future consumer wanting flat slice sums can add it on the existing `subtree_aggregate` primitive. | Low | Open. |
