...
**Discussion input (user, 2026-07-15):** Groop serves HTTP on loopback only and
...
operator surface. Groop serves a capability-token-protected loopback HTTP
...
up ordinary local port forwarding to Groop's endpoint. Groop does not manage
...
sensitivity ceiling. A browser-supplied `X-Groop-Principal` is never trusted.
...
mode without adding SSH lifecycle or identity handling to Groop."
...
**Question:** Exactly when may zero-argument `groop` fall back from its preferred
daemon to local collection?

**Why it matters:** The user decided that `groop` should start like `top`, prefer
...
interactive Groop always remains usable through a visible local fallback;
explicit daemon mode fails closed and explicit local mode does not probe.
Encoded in `TUI-SPEC.md` §0.2 and `docs/ROADMAP.md`.

**Context pointers:** `handoff/GROOP-OBSERVABILITY-DISCUSSION.md`; `src/groop/cli.py`
...
needed for documented percentile/error bounds. `groop daemon stats` must show
...
**Context pointers:** `src/groop/daemon/broker.py`; `src/groop/record/ring.py`;
...
**Context pointers:** `handoff/GROOP-OBSERVABILITY-DISCUSSION.md` questions 3-4.
...
**Context pointers:** `src/groop/report.py`; roadmap P54/P61/P62/P64/P65;
`handoff/GROOP-OBSERVABILITY-DISCUSSION.md` question 5.
...
Docker/CIU joins in `src/groop/collect/dockerjoin.py`.
...
visible. Groop may identify the owning subsystem and explicitly hand off to a
...
Groop serves its committed React bundle and projected read routes from one
...
**DECISION (user, 2026-07-15):** Accepted as recommended. Groop owns same-origin
...
**Context pointers:** unmerged `feat/groop-web-ui-arch-reflection`;
`docs/WEB-UI-SCOPING.md`; P67/P73/P77/P81 handoffs.

**Resume prompt:** "Discuss D-011: approve Groop-owned same-origin static hosting,
...
**Context pointers:** `src/groop/ui/tree.py`, `table.py`, `grouping.py`;
`handoff/GROOP-OBSERVABILITY-DISCUSSION.md` question 8.
...
**Context pointers:** `src/groop/collect/procs.py`; `docs/OPERATOR-QUESTIONS.md`.
...
**Context pointers:** `src/groop/collect/host.py`; TUI banner §3.0;
`docs/OPERATOR-QUESTIONS.md`.
...
and refuse when no safe adapter exists; (c) keep Groop permanently read-only.

**Recommendation:** (b). Groop may keep narrow, audited Docker resource updates
