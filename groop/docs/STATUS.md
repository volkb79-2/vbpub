...
| v1 read-only TUI | 80-85% | medium | Core daily triage works. P33 adds a repeatable rootless smoke harness; remaining gaps are live 5-minute evidence, UI polish, richer host banner/device surfaces, and some acceptance criteria. |
...
- **Acceptance evidence:** P12 records tests, packaging, fixture JSON, replay
  ...
  repeatable rootless acceptance smoke command. P17 records the safe BPF gate
  and current live-BPF blocker. P18 records the fixture-tested BPF provider
  implementation. `MEASUREMENTS.md` still needs a full 5-minute live perf/RSS
  run, DAMON, and privileged BPF measurements.
...
| 13. Unprivileged smoke | P33 provides `python -m groop.acceptance smoke` for repeatable rootless safe-path evidence; fresh live-host results should be pasted into `MEASUREMENTS.md` before a release claim. |
...
Most recent full-suite validation (P33 - Release smoke harness, after P32/P33
merge):
...
- Python compile over merged P32/P33 changed files.
- P32 focused daemon/status tests on `main`: `46 passed in 16.18s`.
- P33 acceptance smoke command with fixture cgroup root and replay fixture:
  exit `0`, `ok: true`, 8 entities, 572 metric source labels, wall `0.1794s`,
  max RSS `89256 KB`.
