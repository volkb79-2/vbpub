...
| v2 daemon/BPF/admin actions | 74-78% | low | Provider abstractions, a read-only Unix-socket daemon, attach/deployment/status tooling, preview planning, validated Docker/systemd start/stop/restart execution, BPF gate/provider/snapshot bridge, bounded Docker/cgroup/journald inspect-files reads, daemon-owned paddr lifecycle, guided memory.high squeeze, systemd memory.high governance, and CIU stack metadata (detection + frame fields) exist. Live BPF load/attach, GPU/ZFS plugins, and CIU grouping/actions remain. |
