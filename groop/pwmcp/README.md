# Groop PWMCP browser-test service

This directory is Groop's consumer deployment of the upstream `vbpub/pwmcp`
service. It is test infrastructure for the operator-console React UI; it is not
part of Groop's runtime or production authentication boundary.
...
byte from that bundle; Groop-specific policy lives only in the sparse
`ciu.global.toml.j2` and `ciu.toml.j2` overrides.

Current policy:

- internal Docker networks only: the owned `groop-ui-test` service network and
...
## Reaching a Groop UI under test

The browser runs in a different container, so it cannot reach a Groop server
bound to the devcontainer's loopback address. A browser-test harness must:

1. attach the Groop test server/runner to the configured consumer network (the
...
This test-only bind is not permission to weaken Groop's production loopback-
