...
So: **every grouping and ordering assertion in this package must drive groop's
...
- Grouping is a **pure function over entities**, unit-testable without Textual,
  living outside `src/groop/ui/` (spec §6.1: Textual only under `ui/`). The UI
...
1. **Numeric phase ordering, driven by groop's code.** A stack with phases 1, 2,
...
```bash
PYTHONPATH=groop/src python3 -m pytest <focused P83 tests> -q -W error -p no:schemathesis
