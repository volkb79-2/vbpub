...
**Cut:** v1 (final). **Depends:** P2–P6 merged. Branch: `feat/groop-p7-integration`.
...
1. `cli.py` finalization: `groop` (live TUI), `--once --json`, `--record FILE`
...
4. Real-host verification (root and non-root, read-only): startup time,
   sample cost (CPU% of groop itself — spec §9 budget), degradation banner
   ...
   audit statement).
5. Packaging: `pyproject.toml` (console script `groop`, textual dependency,
...
- `pip install -e groop/ && groop --replay golden.jsonl` works from a clean
  venv.
