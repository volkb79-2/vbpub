...
> **Session-hint:** fresh
> **Serialize-with:** none
> **Escalate-if:** a named contract cannot be met as specified; closing the gap would require changing `groop.mcp.server` behavior rather than exercising it
...
has ever run `groop mcp serve` against a real daemon over a real socket. The
optional-extra packaging bug the P58 review had to fix (the suite aborted at
collection without `groop[mcp]`) is exactly the class that a fake-client-only
suite cannot see. This package closes that with the harness pattern groop already
uses three times (P33 smoke, P35 steady, P38 tui-smoke), not with a new one.
-->

## Goal

Add an `mcp-smoke` leg to `python -m groop.acceptance` that starts a real daemon,
runs a real `groop mcp serve` process, drives all four MCP tools through a real
...
- Starts after merged P58. Consumes `groop.mcp.server` and `groop.daemon` as they
  are; this package **exercises** the frontend, it does not change it. If a
  contract below cannot be met without editing `groop/src/groop/mcp/server.py`,
  that is a finding for the REPORT and a BLOCKED exit -- not a silent edit.
- Branch: `feat/groop-p75-mcp-live-acceptance-leg`
- Worktree: `.worktrees/groop-p75-mcp-live-acceptance-leg`
- Touch only `groop/**`; write P75-LOG.md/P75-REPORT.md; commit, do not merge.

## Context To Read First (bounded)

`groop/README.md` (Workflow protocol), this handoff,
`groop/src/groop/acceptance.py` (the three existing legs -- `run_smoke`,
`run_steady`, `run_tui_smoke`, and their `format_*_json`/`format_*_text` pairs and
`build_parser` wiring are the exemplar to copy), `groop/src/groop/mcp/server.py`,
`groop/src/groop/daemon/client.py`, `groop/handoff/reports/P58-REVIEW.md`,
`groop/docs/DAEMON.md` (MCP frontend section). Do not read UI, DAMON/BPF, actions,
...
- `python -m groop.acceptance mcp-smoke` follows the existing leg shape exactly:
...
1. `groop mcp serve` connects to the live daemon and completes the `hello` probe.
2. Tool discovery over a real MCP client session lists exactly the four tools
   `groop_health`, `groop_overview`, `groop_entity`, `groop_history`.
3. Each of the four tools returns a well-formed successful result against the live
   daemon, with `groop_entity`/`groop_history` driven by an entity key **taken from
   the live `groop_overview` response**, not a hardcoded fixture key -- the point of
...
run explicitly, not part of the default `pytest groop/tests` sweep). Do not add a
test to the default suite that starts a daemon.

## Gates And Evidence

```bash
PYTHONPATH=groop/src python3 -m pytest <focused P75 tests> -q -W error
...
- Any change to `groop/src/groop/mcp/server.py` behavior, tool set, or bounds.
