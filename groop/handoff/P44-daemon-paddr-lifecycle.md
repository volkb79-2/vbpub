...
- Touch only `groop/**`
- Keep `groop/handoff/reports/P44-LOG.md` current
- Finish with `groop/handoff/reports/P44-REPORT.md` and focused commits
...
- When enabled, daemon startup plans and starts exactly one groop-owned paddr
  session as root. Treat explicit configuration as operator authorization, but
  retain ownership markers and audit evidence.
- Be idempotent across an already-live groop-owned marker/session: adopt or
...
- On graceful daemon shutdown, stop only the session owned by this daemon run
  and restore any groop-managed DAMON conflict state using existing controls.
