...
Branch: `feat/groop-p10-snapshots`. Follow `groop/README.md` workflow protocol.
...
1. `snapshot/bundle.py`: `create(entity_key, ring, frame, config) -> Path`
   ...
   - `manifest.json`: groop version, schema version, host id, ts, entity,
...
3. `groop snapshot inspect FILE`: CLI that validates the manifest and prints
