...
listening HTTP port onto a daemon whose only boundary today is a 0660 root:groop
...
Today the daemon's boundary is the filesystem: a `0660 root:groop` Unix socket
(`docs/DAEMON.md:15-21`). Membership in the `groop` group *is* the authorization
decision, enforced by the kernel, and P52's default authorization hook is
deliberately a no-op allow because of that (`src/groop/daemon/api.py:177-183`).

An HTTP listener destroys that property. The gateway process is in the `groop`
...
- The gateway module must not pull heavy imports at `groop.daemon` import time.
