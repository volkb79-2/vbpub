...
`feat/groop-p28-io-cap-saturation`. Follow `groop/README.md` workflow protocol
exactly.

## Goal

Make the existing dormant diagnostics input `io_cap_saturation_pct` real. When
a cgroup has finite `io.max` limits, groop should estimate how much of the
...
- `groop/handoff/P6-diagnostics.md` and `groop/handoff/reports/P6-REPORT.md`.
- `groop/src/groop/collect/cgroup.py` for `io.max` and `io.stat` parsing.
- `groop/src/groop/collect/collector.py` for rate derivation.
- `groop/src/groop/registry.py`, `groop/src/groop/ui/table.py`,
  `groop/src/groop/diag/score.py`, `groop/src/groop/diag/rules.py`.
- `groop/tests/test_collector.py`, `groop/tests/test_diag.py`,
...
- `groop/handoff/reports/P28-LOG.md` and
  `groop/handoff/reports/P28-REPORT.md` are written and current.

## Handoff Requirements

- Keep `groop/handoff/reports/P28-LOG.md` current using
  `groop/handoff/AGENT-LOG-TEMPLATE.md`.
- Write `groop/handoff/reports/P28-REPORT.md` with implementation summary,
