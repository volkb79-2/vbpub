...
- Extends the existing docker enrichment. `groop/src/groop/collect/dockerjoin.py`
...
- Touch only `groop/**`; write P76-LOG.md/P76-REPORT.md; commit, do not merge.

## Context To Read First (bounded)

`groop/README.md` (Workflow protocol), this handoff, `groop/TUI-SPEC.md` §4.3
(the authoritative spec for this package -- read it first and in full),
`groop/src/groop/collect/dockerjoin.py`, `groop/src/groop/model.py` (`DockerMeta`),
`groop/src/groop/registry.py`, and P71's ZFS provider + its tests as the exemplar
...
"groop guessed". Collapsing these is a merge blocker.
...
```bash
PYTHONPATH=groop/src python3 -m pytest <focused P76 tests> -q -W error
...
- Proposing or implementing the label schema **inside `ciu` itself** (`ciu/` is a
  ...
  explicit that this is "groop asking ciu for a small favor", not groop's change to
