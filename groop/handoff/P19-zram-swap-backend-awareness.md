...
`feat/groop-p19-zram-swap-backends`. Follow `groop/README.md` workflow
protocol.
...
- `python3 -m pytest groop/tests -q` passes.
- `python3 -m py_compile` passes for `groop/src`.
- `groop --once --json` includes backend classification and host ZRAM metrics
