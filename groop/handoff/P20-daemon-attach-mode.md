...
`feat/groop-p20-daemon-attach`. Follow `groop/README.md` workflow protocol.

## Goal

Let a non-root `groop` client consume frames from the read-only daemon broker
...
1. Add a daemon client module, preferably `groop/src/groop/daemon/client.py`,
   that:
...
2. Add top-level CLI support:
   - `groop --attach SOCKET` starts the existing TUI over daemon frames;
   - `groop --attach SOCKET --once --json` prints one daemon frame as canonical
...
3. Preserve the same UI model as live/replay:
   ...
   - no Textual imports outside `src/groop/ui/`.
4. Add focused tests:
...
- Error messages should be actionable for non-root users, e.g. "cannot connect
  to /run/groop/groop.sock" or "daemon returned unsupported operation".

## Acceptance

- `groop --attach SOCKET --once --json` returns a canonical frame from a test
  broker socket.
- `groop --attach SOCKET --ui-smoke` renders the same UI smoke path over daemon
  frames.
- Full tests pass.
...
- `groop/handoff/reports/P20-LOG.md`
- `groop/handoff/reports/P20-REPORT.md`
