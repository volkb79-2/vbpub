---
schema_version: 1
id: groop-P73-web-ui-read-only-shell
project: groop
...
stack: none
depends_on: [groop-P89-source-auto-backfill, groop-P92-loopback-web-transport]
scope:
  touch: ["groop/**"]
  forbid: []
oracles:
  ...
  - id: O7
    ...
    gate: groop-suite
  - id: O8
    observable: "the built React bundle is committed as package data and pip install groop / runtime does not require Node"
  ...
  - id: O9
    ...
    gate: groop-suite
  - id: O10
    observable: "PWMCP browser tests via groop/pwmcp on CIU assert Overview, Explore, shareable non-secret URL state, disconnected/stale rendering, keyboard navigation, landmark/table semantics and zero console errors, with the fixture gateway publishing no host port"
