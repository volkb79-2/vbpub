...
`feat/groop-p24-replay-jump`. Worktree:
`.worktrees/-groop-p24-replay-jump`.
...
- Read `groop/CONTRACTS.md`.
- Read `groop/docs/STATUS.md` and `groop/docs/ROADMAP.md` replay sections.
- Read existing replay/UI code and tests:
  - `src/groop/record/replay.py`
  - `src/groop/ui/app.py`
  - `src/groop/ui/keys.py`
...
- Focused replay/UI tests pass, plus full `groop/tests` and py_compile over
  changed files.

## Handoff artifacts

- Keep `groop/handoff/reports/P24-LOG.md` current using
  `handoff/AGENT-LOG-TEMPLATE.md`.
- Write `groop/handoff/reports/P24-REPORT.md` with implementation summary,
