...
> **Session-hint:** fresh
> **Serialize-with:** P59 (shared file: `src/groop/cli.py` argument parsing / `_filter_kwargs`)
...
- `src/groop/registry.py`: `REGISTRY` (all metric names), and P55's
  `METRIC_GROUPS` / `COMPACT_GROUPS` (family -> metric-name tuples).
- `src/groop/collect/collector.py`: `Collector.__init__` `metrics_mode` handling
...
- `src/groop/cli.py`: `--metrics` argparse entry (currently
