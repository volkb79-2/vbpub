...
- Touch only `groop/**`; do not inspect other worktrees or later commits.
- Read only the minimum relevant context first: `groop/README.md`, this handoff,
  `groop/CONTRACTS.md`, the current broker/client/daemon-serve code, their tests,
  and the daemon/spec/readiness/status/measurements documents that must change.
- Write `groop/handoff/reports/P51-LOG.md` and `P51-REPORT.md`, run every gate,
...
```bash
PYTHONPATH=groop/src <project-python> -m pytest <focused P51/daemon tests> -q -W error
PYTHONPATH=groop/src <project-python> -m pytest groop/tests -q -W error
<project-python> -m py_compile <all Python files under groop/src/groop and groop/tests>
git diff --check
```
