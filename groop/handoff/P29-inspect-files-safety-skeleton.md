...
`feat/groop-p29-inspect-files-safety`. Follow `groop/README.md` workflow
protocol exactly.
...
- `groop/TUI-SPEC.md` §4.8 "File/log/content inspection (`--inspect-files`,
  v2)" and related privacy notes.
- `groop/handoff/P21-admin-action-gating-skeleton.md` and
  `groop/handoff/reports/P21-REPORT.md` for no-execution/gating patterns.
- `groop/src/groop/cli.py`, `groop/src/groop/actions/*`,
  `groop/src/groop/collect/dockerjoin.py`.
- `groop/tests/test_actions.py` and CLI tests/smokes if present.
- `groop/handoff/AGENT-LOG-TEMPLATE.md`.

## Scope - In

1. Add a small read-only inspection planning module, for example
   `groop/inspect_files/plan.py`.
2. Add an explicit CLI command such as:

```bash
groop inspect-files plan --target ENTITY_OR_CONTAINER --kind docker-logs --admin
```
...
- `groop/handoff/reports/P29-LOG.md` and
  `groop/handoff/reports/P29-REPORT.md` are written and current.

## Handoff Requirements

- Keep `groop/handoff/reports/P29-LOG.md` current using
  `groop/handoff/AGENT-LOG-TEMPLATE.md`.
- Write `groop/handoff/reports/P29-REPORT.md` with implementation summary,
