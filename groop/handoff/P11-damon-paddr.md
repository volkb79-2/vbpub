...
Parallel to P10 is fine. Branch: `feat/groop-p11-damon-paddr`.
Follow `groop/README.md` workflow protocol.

## Goal

A MANUALLY started, groop-owned `paddr` DAMON session that measures physical
...
1. `damon/paddr.py`: start/stop a groop-owned paddr context on a FREE kdamond
...
4. Host-memory status page (drill-down on the banner / hotkey): region-size
   histogram as text bars, session parameters, session owner (groop vs
