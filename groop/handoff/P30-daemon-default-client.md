...
Branch: `feat/groop-p30-daemon-default-client`. Follow `groop/README.md`
...
without repeating `/run/groop/groop.sock`, and scripts should have a clear
...
- `groop/src/groop/daemon/client.py`, `groop/src/groop/daemon/deploy.py`.
- `groop/tests/test_attach_cli.py`, `groop/tests/test_daemon_client.py`,
...
```bash
groop --attach
groop --attach --once --json
groop --attach --ui-smoke
```

   Keep existing `groop --attach /custom/path.sock ...` behavior.
2. Add a read-only daemon subcommand for one-frame retrieval, for example:

```bash
groop daemon current --json
...
4. Add focused tests using the fixture broker:
   ...
   - `groop daemon current --socket TMP --json` returns canonical frame JSON;
   - pretty JSON works;
...
```bash
PYTHONPATH=groop/src python3 -m pytest groop/tests/test_attach_cli.py groop/tests/test_daemon_client.py groop/tests/test_daemon_deploy.py -q
PYTHONPATH=groop/src python3 -m pytest groop/tests -q
PYTHONPATH=groop/src python3 -m py_compile groop/src/groop/cli.py
```

Also run one CLI smoke against a fixture broker for `groop daemon current`.

## Handoff Requirements

- Keep `groop/handoff/reports/P30-LOG.md` current using
  `groop/handoff/AGENT-LOG-TEMPLATE.md`.
- Write `groop/handoff/reports/P30-REPORT.md` with implementation summary,
