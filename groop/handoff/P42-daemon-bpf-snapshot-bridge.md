...
- Touch only `groop/**`
- Keep `groop/handoff/reports/P42-LOG.md` current
- Finish with `groop/handoff/reports/P42-REPORT.md` and focused commits
...
- Validate the map path is beneath the configured groop BPF root. Reject
...
- Emit the existing P18 `snapshot.json` contract with schema/version,
  ...
  non-world-writable.
- Integrate periodic snapshot refresh into `groop daemon serve` only behind an
