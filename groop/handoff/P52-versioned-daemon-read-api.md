...
`src/groop/daemon/` (broker, serve, client, protocol as merged by P47/P51),
their tests under `groop/tests/`, `src/groop/registry.py` (metric metadata),
...
- Include registry-derived source/unit/semantic/sensitivity metadata (from
  `src/groop/registry.py`) in `entity` (and where cheap, `current`) responses
...
```bash
PYTHONPATH=groop/src python3 -m pytest <focused P52/daemon tests> -q -W error
