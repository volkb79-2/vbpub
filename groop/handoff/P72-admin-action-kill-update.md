...
> **Base:** main
> **Session-hint:** fresh
> **Serialize-with:** none   <!-- but it is the only package allowed in src/groop/actions/ at a time; do not run concurrently with any other actions-area package -->
...
- `src/groop/actions/` — the whole P46 kernel: plan construction, target
...
- `groop/CONTRACTS.md` standing contracts (input trust; bounds enforced then
...
8. **Closed limit allowlist:** `--memory` and `--cpus` only, on Docker containers
   ...
   error pointing at `groop action set-property`.
...
`groop/README.md` (quickstart line + work-package row), `docs/OPERATIONS.md`
...
```bash
PYTHONPATH=groop/src python3 -m pytest groop/tests/<action test files> -q -W error
