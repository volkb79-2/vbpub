...
Date: 2026-07-13. Branch: `feat/groop-p58-daemon-mcp-frontend-v3` (worktree
`.worktrees/groop-p58-daemon-mcp-frontend-v3`, base `e27ba90`).
...
`groop/src/groop/mcp/server.py:45` sets `MAX_RESPONSE_BYTES = DEFAULT_MAX_RESPONSE_BYTES`.
Verified by grep: **the name appears nowhere else in `groop/src/`.** `_ok()` is:
...
1. **Docker selector.** `groop_entity`'s description says it "Accepts an exact
...
3. **History limit.** `groop_history`'s description says "Limit max: 1000 data
   ...
   `groop_history` has no caller-settable limit to validate.
...
groop/tests/test_textual_boundary.py` is empty), and REPORT deviation #2 says the
...
def _metric_sensitivity(name: str) -> Sensitivity:
    """Mirrors ``groop.daemon.api.metric_sensitivity`` without importing api
...
- **The stated justification is false.** `server.py:24` already does
  `from groop.daemon.api import DEFAULT_MAX_RESPONSE_BYTES, Sensitivity` at module
  level. The api module is imported regardless; there is nothing to avoid.
  `groop.daemon.api.metric_sensitivity(name)` is a public, exported function.
...
at all?* Standing contract: "Shared behavior belongs in `src/groop/` helpers, not
in copied package-local parsers."

**Fix:** delete `_metric_sensitivity` and `_SENSITIVITY_LEVELS`; call
`groop.daemon.api.metric_sensitivity`, and order via the enum, not a private
...
def test_tool_discovery_lists_four_tools(mock_client: DaemonClient) -> None:
    ...
    expected = {"groop_health", "groop_overview", "groop_entity", "groop_history"}
...
- `_handle_*` use bare `assert isinstance(...)` for control flow; these vanish
  under `python -O`.
- `groop/CONTRACTS.md` loses its trailing newline (`\ No newline at end of file`).
...
Handoff updated with a v3 carve note: `groop/handoff/P58-daemon-mcp-frontend.md`.
