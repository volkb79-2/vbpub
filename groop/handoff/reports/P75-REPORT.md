...
A rootless, self-contained `mcp-smoke` leg for `python -m groop.acceptance` that
...
- **`_run_mcp_client_session()`** - async MCP client session driving 6 checks:
  ...
  2. `tool_discovery` - exactly 4 tools: groop_health, groop_overview, groop_entity, groop_history
...
find groop/src/groop groop/tests -name '*.py' -newer ... | while read f; do
