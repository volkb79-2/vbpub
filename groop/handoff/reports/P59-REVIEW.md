...
- Focused: `PYTHONPATH=groop/src /tmp/p43-clean-venv/bin/python -m pytest groop/tests/test_p59_container_selector.py -q` → `9 passed in 0.16s`.
- Full suite (post-fix): `PYTHONPATH=groop/src timeout 400 /tmp/p43-clean-venv/bin/python -m pytest groop/tests -q -W error` → `923 passed, 2 skipped in 120.93s`.
...
- Scope: all 10 files under `groop/**`.
