...
| File | Change |
|---|---|
| `src/groop/collect/host.py` | Added `_gpu_metrics()`, `_gpu_detail()`, `_read_sysfs_int()`, `import re`, `_CARD_RE`. Integrated into `collect_host()` and `collect_host_meta()`. |
| `src/groop/registry.py` | Added 4 new `MetricSpec` entries: `host_gpu_vram_total`, `host_gpu_vram_used`, `host_gpu_busy_pct`, `host_gpu_count`. |
| `src/groop/ui/banner.py` | Added `_gpu_line()` and wired into `render_banner()` after ARC line. |
...
# 4 pre-existing failures: 3x test_fieldlist_* (groop/src command not found),
...
```bash
$ PYTHONPATH=src python3 -m groop.cli --once --json | python3 -c "
import sys,json
...
- **Live amdgpu acceptance**: this host has no amdgpu GPU, so the present-path
  ...
  amdgpu host should run `groop --once --json` and verify the banner by eye.
