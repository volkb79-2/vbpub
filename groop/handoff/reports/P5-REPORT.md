...
- Added a read-only Textual UI under `src/groop/ui/` only:
...
- Added tests for:
  - Textual import boundary outside `src/groop/ui/`
...
- Additive config parsing was implemented inside the existing `groop.config.load()` path so UI defaults can come from config without changing frame or provider interfaces.

## Test evidence

Command:
`PYTHONPATH=/tmp/groop-pytest:/tmp/vbpub-groop-p5-ui/groop/src python3 -m pytest groop/tests -q`

Output tail:
`34 passed in 3.26s`

Command:
`PYTHONPATH=/tmp/vbpub-groop-p5-ui/groop/src python3 -m py_compile $(find groop/src/groop -name '*.py' | sort)`

Output tail:
`<no output>`

Command:
`PYTHONPATH=/tmp/vbpub-groop-p5-ui/groop/src python3 -m groop.cli --once --json --cgroup-root groop/tests/fixtures/cgroupfs/gstammtisch`

Output tail:
`{"entities":{"":{"entity"...},"host":{...},"interval_s":5.0,"schema_version":1,"ts":...}`

Command:
`PYTHONPATH=/tmp/groop-pytest:/tmp/vbpub-groop-p5-ui/groop/src python3 -m groop.cli --replay groop/tests/fixtures/frames/gstammtisch-once.jsonl --step --ui-smoke`
