...
### New module: `groop/src/groop/actions/kill_ops.py`
...
### New module: `groop/src/groop/actions/update_ops.py`
...
- `_reject_systemd_target(target)` — refuses systemd unit targets with a
  message directing users to `groop action set-property`.
...
### Tests: `groop/tests/test_p72_kill_update.py` — 45 new tests
...
```text
PYTHONPATH=groop/src python3 -m pytest groop/tests/test_p72_kill_update.py -q
...
python3 -m py_compile groop/src/groop/actions/catalog.py \
  groop/src/groop/actions/kill_ops.py groop/src/groop/actions/update_ops.py \
  groop/src/groop/actions/preview.py groop/src/groop/actions/__init__.py \
  groop/src/groop/actions/execute.py groop/src/groop/cli.py \
...
package-private under `groop/actions/`. The existing `ActionPlan` /
