...
- Added owner detection for passive DAMON sessions using groop ownership
  markers; unmarked sessions render as `foreign`.
- Added `groop.damon.paddr` for controlled groop-owned paddr session start,
  reusing P9 root checks, free-slot selection, ownership markers, sysfs writes,
  and audit logging.
- Added `groop damon paddr start --confirm START`; existing
  `groop damon stop --all-mine` tears down the owned paddr session because it
...
- Added tests for paddr passive classification, groop vs. foreign ownership,
...
- Start refuses busy kdamond slots and refuses duplicate groop-owned paddr
  sessions for the same DAMON root.
- Stop still operates only on groop marker files and leaves unmarked foreign
  sessions untouched.
...
```bash
PYTHONPATH=/tmp/groop-pytest:/tmp/vbpub-groop-p11-damon-paddr/groop/src python3 -m pytest groop/tests -q
# 79 passed in 11.17s
```

```bash
PYTHONPATH=/tmp/vbpub-groop-p11-damon-paddr/groop/src python3 -m py_compile $(find groop/src/groop -name '*.py' | sort)
# clean
```

```bash
PYTHONPATH=/tmp/vbpub-groop-p11-damon-paddr/groop/src python3 -m groop.cli --once --json --cgroup-root groop/tests/fixtures/cgroupfs/gstammtisch
# once 1 8
```

```bash
PYTHONPATH=/tmp/groop-pytest:/tmp/vbpub-groop-p11-damon-paddr/groop/src python3 -m groop.cli --replay groop/tests/fixtures/frames/gstammtisch-once.jsonl --step --ui-smoke
