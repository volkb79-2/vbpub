...
suite tests shell out via the repo-root-relative path `groop/src`).
...
```bash
PYTHONPATH=groop/src python -m pytest groop/tests -q -W error
```
...
With zstandard installed, a corrupt or truncated `.zst` recording makes `groop report`
...
```
$ groop action preview --kind docker-update --target my-container --admin --memory 512M
...
$ groop action preview --kind docker-update --target nginx.service --admin --memory 512M
target 'nginx.service' looks like a systemd unit; use 'groop action set-property'
for systemd resource changes                                              exit=2

$ groop action preview --kind docker-kill --target c1 --admin --signal 9
signal must be a symbolic name, not a number: '9'                         exit=2

$ groop action preview --kind docker-kill --target c1 --admin --signal KILL
...
```
groop/tests/test_gpu.py                              15 passed
groop/tests/test_p72_kill_update.py + test_actions.py  251 passed
