...
- Decision: Accept `_sleep` and `_time` injectable parameters in `run_steady()`
  Reason: Tests must not sleep for real; injectable time source enables deterministic testing
  Impact: Tests pass `_sleep=time_ns_sleep` and `_time=time_ns`
- Decision: Use reusable `_output_and_exit()` helper shared by smoke and steady main paths
  Reason: Reduces duplication in acceptance_main(); both commands need json/text dispatch and exit code return
