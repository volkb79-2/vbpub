...
- **`[banner] net_device_exclude` / `[banner] disk_device_exclude` config keys**
  ...
  follow-up.
- **Live system acceptance:** the golden fixture comparison was updated for this
  host's (`vda`, `dm-0`, `sr0`) block devices; a host with different block/network
  topology would produce a different `host_meta`. The fixture may need regeneration
  on the gstammtisch reference host.
...
| `groop/tests/test_ui_banner.py` | Added 6 tests: NET/DISK rendering, collecting, round-trip |
| `groop/tests/test_p23_zram_drilldown.py` | Updated 2 tests for new host_meta shape |
| `groop/tests/fixtures/frames/gstammtisch-once.jsonl` | Updated host_meta with net/block device entries |
