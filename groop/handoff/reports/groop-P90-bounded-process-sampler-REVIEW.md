# groop-P90-bounded-process-sampler — REVIEW (independent frontier merge gate)

**Reviewer:** independent frontier reviewer (Opus 4.8), 2026-07-15 — did NOT
write this code.
**Branch reviewed:** `feat/groop-P90-bounded-process-sampler` @ `996a3db`
(diff `main...feat/groop-P90-bounded-process-sampler`, 12 files, +1794/-2).
...
```
# groop-suite (frontmatter gate, zero-skip full suite)
$ python -m pytest groop/tests -q
1642 passed in 192.48s      # 0 failed, 0 skipped, exit 0

# focused P90 file
$ python -m pytest groop/tests/test_p90_process_sampler.py -q
24 passed in 3.63s

# py-compile gate
$ python -m compileall groop/src/groop/procs groop/src/groop/config.py \
    groop/src/groop/model.py groop/src/groop/registry.py \
    groop/src/groop/query/semantics.py            # exit 0
...
- **Contract 5 (feeds P88, no second engine):** `ProcessFrameSource` implements
  ...
  semantic resolves.
- **Scope:** all touched files are under `groop/**` (frontmatter `scope.touch`).
...
the mandatory gates (`groop-suite` zero-skip full suite, `py-compile`) were
