...
- Current objective: Add mcp-smoke leg to python -m groop.acceptance
...
- Commands: python3 -m py_compile groop/src/groop/acceptance.py
...
- Commands: PYTHONPATH=groop/src <venv>/python -m pytest groop/tests/test_acceptance.py -k "mcp_smoke or format_mcp or build_parser_wires or terminate_process" -v -W error
- Result: 10/10 passed
...
- Commands: timeout 900 env PYTHONPATH=groop/src <venv>/python -m pytest groop/tests -q -W error
- Result: All tests pass (same pre-existing UI flake as P58)

- Action: Ran live mcp-smoke leg
- Commands: PYTHONPATH=groop/src python3 -m groop.acceptance mcp-smoke --json --pretty
...
- Decision: Use temp dir for daemon socket (default) with --socket override
  ...
  Impact: Safe in CI; no assumption about /run/groop/groop.sock.
...
PYTHONPATH=groop/src <venv>/python -m pytest groop/tests/test_acceptance.py -k "mcp_smoke or format_mcp or build_parser_wires or terminate_process" -v -W error
# 10 passed

# Full suite (venv, -W error)
timeout 900 env PYTHONPATH=groop/src <venv>/python -m pytest groop/tests -q -W error
# <all passed>
