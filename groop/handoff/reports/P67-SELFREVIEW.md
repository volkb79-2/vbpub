...
**FINDING: none after fixes.** Every changed file is under `groop/**`. The
...
2. **Authentication/redaction:** every GET authenticates a single configured
   `X-Groop-Principal` from a loopback peer before routing. Metric values above
...
The gateway remains outside `groop.daemon.__init__`; the new subprocess test
proves importing `groop.daemon` does not load the HTTP gateway.
