...
1. **`groop/src/groop/record/headless.py`** — New module containing:
...
2. **CLI flags** added to `parse_args()` in `groop/src/groop/cli.py`:
...
None. The headless module is additive and package‑private within `groop/record/`. No shared interfaces in `CONTRACTS.md` were modified.
...
```bash
$ PYTHONPATH=groop/src python3 -m pytest groop/tests/test_headless_record.py -q -W error::RuntimeWarning
........s................
...
$ python3 -m py_compile groop/src/groop/record/headless.py
$ python3 -m py_compile groop/tests/test_headless_record.py
```
...
```bash
$ PYTHONPATH=groop/src python3 -m groop.cli --once --json --cgroup-root groop/tests/fixtures/cgroupfs/gstammtisch
{"schema_version":1,...}
```
...
```
M groop/README.md
M groop/CONTRACTS.md
M groop/src/groop/cli.py
A groop/src/groop/record/headless.py
A groop/tests/test_headless_record.py
A groop/handoff/reports/P53-LOG.md
A groop/handoff/reports/P53-REPORT.md
```
