...
- Branch: feat/groop-p87-docker-owner-safety
- Worktree: /workspaces/vbpub/.worktrees/groop-p87-docker-owner-safety
...
Environment: fresh venv in the worktree, `.venv/bin/pip install -e './groop[dev]'`
(exit 0). Commands run with `PYTHONPATH=groop/src` from the worktree root.
...
- Action: Added groop/src/groop/actions/owner_safety.py (single-inspect resolver,
  ...
  Added groop/tests/test_p87_owner_safety.py (all 5 oracles + single-inspect +
