...
1. **A declared, installable test environment.** `[project.optional-dependencies] dev`
   in `pyproject.toml` pins `groop[zstandard]` + `pytest>=8.0`, making the gate
   reproducible.
...
3. **Documentation.** The `groop/README.md` "Gate environment" section documents
   `pip install -e 'groop[dev]'` and shows the banner output. `docs/STATUS.md`
...
| File | Change |
|---|---|
| `pyproject.toml` | Add `[project.optional-dependencies] dev = ["groop[zstandard]", "pytest>=8.0"]` |
...
|---|---|---|
| 1. Declared test environment with zstandard pinned | Done | `pyproject.toml`: `[dev]` extra with `groop[zstandard]` |
| 2. zstd oracles no longer skip in the gate env | Done | `pip install -e 'groop[dev]'` installs zstandard; all oracles execute |
| 3. Skipped oracle is loud | Done | `pytest_sessionfinish` prints prominent FAIL banner with test list |
| 4. Document how to build the gate env | Done | `groop/README.md` §Gate environment |
| 5. No behavior change to groop itself | Done | Only pyproject.toml, conftest.py, docs touched |

### Oracle mapping (numbered, adversarial)

| Oracle | Assertion | Status |
|---|---|---|
| 1. Gate env runs zstd oracles | `pip install -e 'groop[dev]'` + `pytest groop/tests -q` — assert test IDs include zstd oracles | ✅ (requires zstd; tested below in no-zstd env) |
...
| 4. `zstandard` is not a hard runtime dependency | `pip install groop` without extras imports and runs `groop report` on plain `.jsonl` | ✅ (unchanged — no source changes) |
...
```bash
$ cd /workspaces/vbpub/.worktrees/groop-p84-pin-gate-environment
$ PYTHONPATH=groop/src python3 -m pytest groop/tests/test_report.py -q -k "oracle_1 or oracle_2b or oracle_5" --no-header
# output:
# s
...
```bash
$ timeout 900 env PYTHONPATH=groop/src python3 -m pytest groop/tests -q --no-header
...
!!! GATE FAILED: zstandard extra not installed !!!
...
```bash
$ python3 -m py_compile groop/tests/conftest.py && echo OK
OK
```
...
re-validate this by building a fresh venv with `pip install -e 'groop[dev]'`.
