...
Date: 2026-07-13. Branch: `feat/groop-p58-daemon-mcp-frontend-v4` (worktree
`.worktrees/groop-p58-daemon-mcp-frontend-v4`, base `27e0a6a`).
...
| B2 descriptions/CONTRACTS promise absent behavior | **Fixed** | Discovered descriptions now state only enforced bounds (16 components, 1..50 rows, 1..100 points, 128 metrics/64 findings, 4 MiB). `groop_history` has a real `limit`. The "1000-point" claim is gone and a test asserts `"1000" not in` every description. `test_textual_boundary.py` is genuinely extended. |
...
install (no `groop[mcp]`), that is a **collection error, and pytest aborts the
entire run** - not a skip:

```
ERROR groop/tests/test_mcp_server.py
!!!!!!! Interrupted: 1 error during collection !!!!!!!
no tests collected, 1 error in 0.20s
```

So the standing gate `pytest groop/tests` was broken for anyone without the
...
### R2 - `groop_history` accepted any string as a metric name
...
```
groop_history(metric='nonexistent_typo_metric')
...
### R4 - exact-key `groop_entity` did two daemon round trips
...
`groop/src/groop/mcp/__init__.py` shipped two em dashes. The self-review explicitly
...
```text
PYTHONPATH=groop/src <venv>/python -m pytest \
...
PYTHONPATH=/tmp/blockmcp:groop/src <venv>/python -m pytest \
  groop/tests/test_mcp_server.py groop/tests/test_textual_boundary.py -q -W error
2 passed, 1 skipped         # base install without groop[mcp]: skips, does not abort
...
|---|---|---|
| R1 | Optional extra not optional: suite aborts at collection without `groop[mcp]` | **no** |
| R2 | `groop_history` accepts any string as a registry metric name | **no** |
| R3 | Empty history window reported as `invalid-selector` | **no** |
| R4 | Exact-key `groop_entity` costs two daemon RPCs | **no** |
| R5 | Non-ASCII em dashes in `groop/src/groop/mcp/__init__.py` | **no** (pass #1 fixed the em dashes in the LOG/REPORT and missed the source file) |
...
optional *for the CLI* (both absent-SDK tests target `groop mcp serve`) but never
...
- The `groop_health` 16-component and `groop_entity` 128-metric/64-finding caps
