...
- Branch: feat/groop-p85-flaky-ui-timing-gate
- Worktree: .worktrees/groop-p85-flaky-ui-timing-gate
...
- Commands: read_file groop/handoff/P85-flaky-ui-timing-gate.md, read_file groop/tests/test_ui_app.py, read_file groop/tests/test_record.py, read_file groop/src/groop/ui/app.py
...
- Commands: for i in $(seq 1 10); do python3 -m pytest groop/tests/test_ui_app.py::test_pilot_snapshot_running_status_appears_immediately -x -q; done
...
- Files changed: groop/tests/test_ui_app.py (add _wait_or_timeout helper, use it), groop/src/groop/ui/app.py (fix _run_ui_smoke)
...
- Commands: timeout 900 python3 -m pytest groop/tests -q (3 times)
...
for i in $(seq 1 20); do
  python3 -m pytest groop/tests/test_ui_app.py::test_pilot_snapshot_running_status_appears_immediately -x -q 2>&1 | tail -1
done
# 20/20 passed

# Test 2: 20x isolation stress
for i in $(seq 1 20); do
  python3 -m pytest groop/tests/test_record.py::test_record_cli_runs_ui_and_writes_frames -x -q 2>&1 | tail -1
done
# 20/20 passed

# Full suite × 3
timeout 900 python3 -m pytest groop/tests -q
# Run 1: 1331 passed, 8 skipped
# Run 2: 1331 passed, 8 skipped
# Run 3: 1339 passed

# Mutation 1: snapshot running: status never set
sed -i '431s/self._refresh_status/# self._refresh_status/' groop/src/groop/ui/app.py
python3 -m pytest groop/tests/test_ui_app.py::test_pilot_snapshot_running_status_appears_immediately -x -q
# FAILED: AssertionError: 'snapshot running:' not in status text
sed -i '431s/# self._refresh_status/self._refresh_status/' groop/src/groop/ui/app.py

# Mutation 2: _snapshot_in_progress never reset
sed -i '469s/self._snapshot_in_progress = False/# self._snapshot_in_progress = False/' groop/src/groop/ui/app.py
timeout 15 python3 -m pytest groop/tests/test_ui_app.py::test_pilot_snapshot_running_status_appears_immediately -x -q
# FAILED: AssertionError: predicate not satisfied within 10.0s
sed -i '469s/# self._snapshot_in_progress = False/self._snapshot_in_progress = False/' groop/src/groop/ui/app.py

# Mutation 3: frame never arrives in _run_ui_smoke
sed -i '636,640s/if app.frames_received:/if False:/' groop/src/groop/ui/app.py
timeout 20 python3 -m pytest groop/tests/test_record.py::test_record_cli_runs_ui_and_writes_frames -x -q
# FAILED: AssertionError on stdout mismatch
git checkout groop/src/groop/ui/app.py  # revert — re-applied fix afterward
```
