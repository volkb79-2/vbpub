...
- Added `groop.providers` with the frozen P3 provider contract in
  `src/groop/providers/base.py`.
- Implemented `src/groop/providers/net_host.py`:
...
- Implemented `src/groop/providers/net_netns.py`:
...
```text
$ PYTHONPATH=/tmp/groop-pytest:/tmp/vbpub-groop-p3-network/groop/src python3 -m pytest groop/tests -q
25 passed in 2.07s
```

```text
$ PYTHONPATH=/tmp/vbpub-groop-p3-network/groop/src python3 -m py_compile $(find groop/src/groop -name '*.py' | sort)
# exit 0, no output
```

```text
$ PYTHONPATH=/tmp/vbpub-groop-p3-network/groop/src python3 -m groop.cli --once --json --cgroup-root groop/tests/fixtures/cgroupfs/gstammtisch
