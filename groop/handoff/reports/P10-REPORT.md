...
- Added `groop.snapshot.bundle` for incident snapshot bundles:
...
- Added `[snapshots]` config:
  ...
  - `frames`
  - `redact`
- Added `groop snapshot inspect FILE`.
...
```bash
PYTHONPATH=/tmp/groop-pytest:/tmp/vbpub-groop-p10-snapshots/groop/src python3 -m pytest groop/tests -q
# 74 passed in 10.59s
```

```bash
PYTHONPATH=/tmp/vbpub-groop-p10-snapshots/groop/src python3 -m py_compile $(find groop/src/groop -name '*.py' | sort)
# clean
```

```bash
PYTHONPATH=/tmp/vbpub-groop-p10-snapshots/groop/src python3 -m groop.cli --once --json --cgroup-root groop/tests/fixtures/cgroupfs/gstammtisch
# once 1 8
```

```bash
PYTHONPATH=/tmp/groop-pytest:/tmp/vbpub-groop-p10-snapshots/groop/src python3 -m groop.cli --replay groop/tests/fixtures/frames/gstammtisch-once.jsonl --step --ui-smoke
