...
`feat/groop-p14-damon-modal`. Follow `groop/README.md` workflow protocol.
...
2. Textual typed-confirmation modal for paddr host session:
   - shows planned sysfs writes;
   - requires exact `START`;
   - refuses duplicate groop-owned paddr session.
3. Stop control surface:
   - explicit cleanup for groop-owned sessions;
