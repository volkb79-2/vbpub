...
3. Canonical JSON helpers in `model.py` per CONTRACTS §4. `groop --once --json`,
...
11. `cli.py`: `groop --once --json` (pretty and compact), `--cgroup-root PATH`
...
- `python3 -m pytest groop/tests -q` green; `python3 -m py_compile` clean for
