...
Make `groop report --window auto` fast enough to run on real recordings, **without
...
`detect_steady_window` (`src/groop/report.py`) scans candidate trailing suffixes
...
- `src/groop/report.py`: `detect_steady_window`, `_finite_gauge_value`,
  `_validate_stability_options`, `select_report_window`, `WindowRange`.
- `groop/handoff/P62-report-steady-state-autodetect.md` — the pinned criterion.
  **You are not allowed to change it.**
- `groop/handoff/reports/P62-REVIEW.md` — why this is a separate package.
- `groop/tests/test_report.py::TestAutoSteadyWindow` — the selections you must
  reproduce exactly.
