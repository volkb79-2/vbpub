"""W8's v12 acceptance suite: the DISCARDED-MUTANTS cut (B070).

**One change, one bump.** v12 exists for exactly one field:
...
1. `schema_version` 11 -> 12, in all nine;
2. in the one INGESTED document (`ingested-r2-v12-template.json`):
   ...
   legal v12 spelling. Its `mutation.candidate_count` is unchanged for the
...
than implying a run behind them -- `multi-target-r3-v12-template.json` (the
plural-canary shape) and `r4-red-first-v12-template.json` (F015's phase-3
...
What is NEW at v12, and what this suite exists to pin:
...
It also carries forward, unchanged, W6's own pins that v12 does not touch:
...
V12_TEMPLATES = [
    "missing-tool-v12-template.json",
    "sql-r2-v12-template.json",
    "ca1-r3-no-base-v12-template.json",
    "ca4-all-equivalent-v12-template.json",
    ...
    # at v12 this is the ONLY template the cut actually touches.
    "ingested-r2-v12-template.json",
    ...
    "multi-target-r3-v12-template.json",
    "r4-red-first-v12-template.json",
    # NEW at v12 (B070), and the only new document this cut adds -- a REAL
    # high-discard ingested verdict. See HIGH_DISCARD_TEMPLATE below.
    "high-discard-r2-v12-template.json",
]
...
#: proves v12's new arithmetic is a re-derivation and not an upper-bound clamp
...
HIGH_DISCARD_TEMPLATE = "high-discard-r2-v12-template.json"
...
MULTI_TARGET_V12_TEMPLATE = "multi-target-r3-v12-template.json"
RED_FIRST_V12_TEMPLATE = "r4-red-first-v12-template.json"

#: The frozen ingested document, by name, for the tests that are about it
#: specifically rather than about "every v12 template verifies".
INGESTED_V12_TEMPLATE = "ingested-r2-v12-template.json"

P25_V12_TEMPLATES = ["p25-pass-v12-template.json", "p25-missing-v12-template.json"]
...
def refuses_only_the_defect(verify_document, clean: dict, broken: dict, why: str):
    assert verify_document(clean) == [], (
        f"{why}: control document must verify clean under v12"
    )
...
def test_schema_identity_is_internally_consistent_under_v12():
    from assay import verdict as V

    schema = load(ROOT / "src" / "assay" / "schemas" / "verdict.schema.json")
    assert schema["$id"] == "urn:assay:schema:verdict:12"
    assert schema["properties"]["schema_version"]["const"] == 12
    assert V.VERDICT_SCHEMA_VERSION == 12


def test_shipped_schema_is_byte_identical_to_the_locked_v12_asset():
    ...
    locked = (HERE / "verdict.schema.v12.json").read_bytes()
...
def test_every_earlier_frozen_template_is_rejected_under_v12(verify_document, frozen):
    """A-170's hard cut, over ALL FIVE earlier generations at once: v12
    ...
    assert "is not this verifier's version 12" in failures[0], failures
...
def test_the_v10_refusal_is_worded_exactly_as_the_v9_and_v8_ones_are(verify_document):
    ...
    v12 in the SAME shape v9 and v8 are, so the hard cut is one rule and not a
...
@pytest.mark.parametrize("name", V12_TEMPLATES)
def test_locked_v12_template_is_accepted(verify_document, name):
    assert verify_document(load(HERE / "expected" / name)) == []


def test_b079_discard_reason_is_closed_and_distinguishes_compile_runtime(
    verify_document,
):
    document = load(HERE / "expected" / HIGH_DISCARD_TEMPLATE)
    discarded = document["judgment"]["r2"]["discarded"]
    assert {entry["discard_reason"] for entry in discarded} == {"compile_error"}

    runtime = copy.deepcopy(document)
    runtime["judgment"]["r2"]["discarded"][0]["discard_reason"] = "runtime_error"
    assert verify_document(runtime) == []

    invalid = copy.deepcopy(document)
    invalid["judgment"]["r2"]["discarded"][0]["discard_reason"] = "other"
    assert verify_document(invalid)

    missing = copy.deepcopy(document)
    del missing["judgment"]["r2"]["discarded"][0]["discard_reason"]
    assert verify_document(missing)


@pytest.mark.parametrize("name", P25_V12_TEMPLATES)
def test_p25_v12_siblings_validate(verify_document, name):
    assert verify_document(load(HERE / "expected" / name)) == []


# --- the frozen INGESTED document (fix round 1) -----------------------------
#
# `ingested-r2-v12-template.json` is a REAL verdict: a real run over the
...
def test_the_frozen_ingested_template_carries_the_WHOLE_ingested_record(
    ...
    """The point of freezing it. `test_locked_v12_template_is_accepted` proves
    ...
    document = load(HERE / "expected" / INGESTED_V12_TEMPLATE)
    ...
    # B070/v12: an ARRAY, empty here because the artifact behind this document
...
def test_p26_attestation_shapes_survive_v12(verify_document, name):
...
def test_an_r2_document_must_declare_its_producer(verify_document):
    ...
    clean = load(HERE / "expected" / "sql-r2-v12-template.json")
...
def test_a_native_r2_document_may_not_carry_the_ingested_record(verify_document):
    ...
    for field, value in (
        ...
        # B070/v12: an ARRAY now, and an EMPTY one is still forbidden on a
...
def test_the_locked_v12_schema_admits_the_ingested_namespace_and_keeps_the_rest_closed():
    ...
    schema = json.loads((HERE / "verdict.schema.v12.json").read_text())
...
def test_a_verdict_may_name_the_directory_its_command_ran_in(verify_document):
    ...
    clean = load(HERE / "expected" / "p25-pass-v12-template.json")
...
# ============================================================================
# Inherited from v10, and re-run here against v12 bytes. Every negative below
...
def test_an_ingested_r2_must_state_the_floor_it_judged_against(verify_document):
    ...
    clean = load(HERE / "expected" / INGESTED_V12_TEMPLATE)
...
def test_a_non_pass_claim_may_carry_the_refusing_sentence(verify_document):
    clean = load(HERE / "expected" / "missing-tool-v12-template.json")
    assert verify_document(clean) == []
...
def test_a_pass_claim_carries_no_detail(verify_document):
    ...
    clean = load(HERE / "expected" / "ca1-r3-no-base-v12-template.json")
    broken = copy.deepcopy(clean)
...
def test_the_multi_target_template_is_the_plural_branchs_only_frozen_witness(
    ...
    clean = load(HERE / "expected" / MULTI_TARGET_V12_TEMPLATE)
    assert verify_document(clean) == []
...
def test_the_red_first_template_pins_the_shape_before_its_producer_exists(
    ...
    clean = load(HERE / "expected" / RED_FIRST_V12_TEMPLATE)
    assert verify_document(clean) == []
...
# ============================================================================
# What is NEW at v12 (B070): judgment.r2.discarded LISTS the invalid mutants,
...
def test_the_A437_reproduction_now_refuses_BY_NAME(verify_document):
    ...
    document verified clean. The v12 spelling of that exact forgery is a
...
def test_an_empty_discarded_list_and_an_absent_one_say_different_things(
    ...
    spelling."""
    ingested = load(HERE / "expected" / INGESTED_V12_TEMPLATE)
    assert ingested["judgment"]["r2"]["discarded"] == []
    native = load(HERE / "expected" / "sql-r2-v12-template.json")
...
def test_the_locked_v12_schema_types_discarded_as_a_mutant_outcome_array():
    """Asserted against the LOCKED artifact, at the level the claim is made
    ...
    # `test_the_locked_v12_schema_bounds_both_at_the_document_ceiling` for why
...
def test_the_locked_v12_schema_still_forks_discarded_on_the_producer():
...
def test_the_locked_v12_schema_bounds_both_at_the_document_ceiling():
