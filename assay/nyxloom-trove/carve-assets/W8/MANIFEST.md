# W8 — the verdict schema v12 successors (B070)
...
| **W8** | **`verdict.schema.v12.json`** | **`test_acceptance_v12.py`** | **`p25-*-v12-template.json`** |
...
when it proved it; rewriting one to v12 would claim a document was accepted
...
`test_acceptance_v12.py`'s hard-cut sweep is what proves W6's nine documents
...
| file | what it is |
|---|---|
| `verdict.schema.v12.json` | a byte copy of `src/assay/schemas/verdict.schema.json` at the v12 cut, verified with `cmp`, not trusted from a paste |
| `test_acceptance_v12.py` | the locked v12 acceptance suite (104 nodes), run from the installed wheel by the registered gate |
| `expected/*-v12-template.json` | **ten** committed v12 documents: W6's nine migrated in place, plus one NEW real high-discard verdict this cut introduces |
...
| file | what it is |
|---|---|
| `high-discard-r2-v12-template.json` | a REAL ingested verdict with **40 discarded mutants** out of 88 candidates |
...
as for `ingested-r2-v12-template.json`.
...
v12 shipped a re-derivation and not a clamp: 40 discarded mutants standing
beside 48 attempted, and `verify_document` returns an empty failure list.

Like `ingested-r2-v12-template.json` it carries no `judge_provenance`, for the
...
* `test_shipped_schema_is_byte_identical_to_the_locked_v12_asset` — the check
  this project has been bitten by twice. Whatever moves in the shipped schema
  must move in the copy here, in the same commit.
* `test_every_earlier_frozen_template_is_rejected_under_v12` — A-170's hard
...
* `test_the_A437_reproduction_now_refuses_BY_NAME` — the v12 spelling of
...
* `test_the_locked_v12_schema_types_discarded_as_a_mutant_outcome_array` and
  `test_the_locked_v12_schema_still_forks_discarded_on_the_producer` — the
