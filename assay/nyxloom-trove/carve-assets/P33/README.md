...
Re-carved by C-sol-1 after **two** NOT READY pre-dispatch adversarial reviews
(`../../reports/assay-P33-pre-dispatch-adversarial-review.md` and
`...-round2.md`).
...
**A-226**/**A-227**/**A-228** (the round-2 repair set). Report: `../../reports/assay-P33-JIT-CARVE.md`.
...
| `test_acceptance_v5.py` | **the locked v5 acceptance suite**, 30 tests. Runs alongside P26's module, which is deselected down by five tests rather than retired (A-226). **26 failed / 4 passed** pre-implementation; must reach all green. Every negative is differential |
...
# the locked acceptance suite (26 failed / 4 passed before implementation)
...
17da7c0818e1dd95c2fb5f8657228182093cbdc85d0ab77542c58927d96b208f  sweep_v4_consumers.py
80d5f031333eef9c132a8998e42c6163c927dec1897e72ff4be2b2663bb0d818  test_acceptance_v5.py
...
| asset | what it fixes |
|---|---|
| `sweep_v4_consumers.py` | **the inventory.** Walks the registered gate's transitive execution path and reports every consumer of a frozen expectation. Found a third instance neither review named |
| `expected/p25-{pass,missing}-v5-template.json` | the v5 siblings `qualify_topos.py` and `tests/test_python_qualification.py` compare against. P25's originals stay frozen; `test_p25_v5_siblings_are_the_declared_projection` audits the difference |

The suite is now **30 tests, 26 failed / 4 passed** pre-implementation.
