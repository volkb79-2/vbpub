...
### 40. `feat(assay): B004, the whole carve -- adjudicated image provenance (A-442)`

**Generation 13, the one item BRIEF-12 carried forward.** B004 is DONE: all
eight of the carve's boundaries (parser, lane surface, adjudicator,
producer, docs, tripwire discharge, decisions row, gate) land in this one
commit, `d9fc22eb`, on top of `78a786fc` (R-2's SF-1 fix, landed directly on
the shared branch by the reviewer — non-overlapping, see below).

- **`src/assay/adjudication.py` (new).** `evaluate_provenance`, the ONE
  registered `"image-provenance"` adjudicator: total over a document's
  bytes, never raises. DA-R12's `{1, 2}` `schema_version` set through one
  parser; an unrecognised `overall` is refused, not guessed; all five
  non-green states collapse to one `NO_MEASUREMENT`/`PROVENANCE_UNVERIFIED`
  terminal; the green path additionally binds `commit_under_test` as a hex
  prefix of HEAD. Nothing is asserted about `containers`/`status`/
  `labelled_revision`/`image`/`tree_state` — §3.3's closed vocabulary,
  every tolerance backed by a real measured shape in `carve-assets/W2/`.
  `load_adjudicated_evidence` mirrors `attestation.load_attested_evidence`'s
  one-result-per-declaration contract over the descriptor-safe read
  boundary, `remaining()` checked at entry and after every item.
- **`src/assay/vocabulary.py`**: `ADJUDICATED_EVIDENCE_KEYS` (new), the leaf
  both `config.py` and `adjudication.py` import — a direct
  `config -> adjudication` import would be a real cycle (`verdict.py`
  imports FROM `config.py`; `adjudication.py` imports FROM `verdict.py`),
  not the DA-R1/A-406 precedent's original reason but the same shape,
  cross-checked by its own drift-equality test.
- **`config.py`**: `EVIDENCE_SOURCES` (was private) now public, widened to
  `{"attested", "adjudicated"}`; the old `has_attestation_dir ==
  has_evidence` rule becomes per-source (`attestation_dir` iff an attested
  entry is declared, `adjudication_dir` iff an adjudicated one is — a lane
  may declare both, one, or neither); an adjudicated key is checked against
  the registered set at load. `LANE_SCHEMA_VERSION` untouched (stays 2,
  additive-optional, DA-D7) — no second `!` commit on this branch.
  `_validate_attestation_dir` generalised to `_validate_evidence_dir`
  WITHIN `config.py` only; `adjudication.py` and `attestation.py` stay
  independent readers of the closed grammar, per the pre-existing design
  comment already in `config.py` — a recorded deviation from the carve's
  own (pre-dating that comment) W3 suggestion to share the validator across
  modules.
- **`cli.py`**: the carve's own adversarial review (F4) in full. A lane
  declaring both sources needs one call to EACH loader over its own subset
  (the attested loader refuses any non-attested declaration); the two
  result tuples are merged back into the lane's full DECLARED order — plain
  concatenation breaks an interleaved `[attested, adjudicated, attested]`
  lane, and `runner._require_evidence_bound_to_lane` requires exact ordered
  equality. One `try`/`except` still wraps both loader calls, so a timeout
  from EITHER discards whichever already resolved and renders every
  declared identity, both sources, the same payload-free
  `BUDGET_EXCEEDED`/`LANE_TIMEOUT` pair — proved by a real first-loader PASS
  discarded when the second times out
  (`test_an_adjudication_timeout_after_a_completed_attestation_pass_is_atomic`).
- **The conformance tripwire is discharged.**
  `tests/test_verdict_conformance.py`'s `EXCLUDED_ENTIRELY` drops
  `("NO_MEASUREMENT", "PROVENANCE_UNVERIFIED")`; the new fixture
  `tests/fixtures/verdicts/evidence_adjudicated_unverified.json` (hand-written,
  matching the sibling fixtures' own convention) discharges the obligation.
  All 177 tests in the file pass.
- **Docs corrected, not just extended**: README/DESIGN-GUIDE's Tier-2 table
  said assay "invokes" a declared tool — false for B004 (A-030; the docker
  socket it would need is unreachable from inside an S3/S4 lane) — now
  "invokes, or consumes the declared structured output of,". CONSUMERS.md's
  B004 subsection is AMENDED (DA-R31; the `#migration-notes-v9--v10` anchor
  is generated by the parent heading, confirmed by grep before renaming this
  subsection's own heading), gaining a loadable lane example, F6's
  delete-first capture pattern, and a freshness-bound caveat.
- **Tests**: `test_adjudication_registry.py` (9, new), `test_adjudication_
  provenance_parse.py` (21, new), `test_adjudication_pipeline_integration.py`
  (5, new — the only green witness anywhere in this estate per A-334, run
  against the real frozen ciu 6.0.3/7.10.1 captures, since `overall` is
  pinned at `mismatch` on every live host); additions to
  `test_config_accept.py` (2), `test_config_reject.py` (4), `test_cli_run.py`
  (2), `test_docs_examples_and_vocabulary.py` (1).
- **`nyxloom-trove/decisions.md`**: **A-442** (the implementation row, all
  six points above plus the recorded W3 deviation and an explicit "what is
  NOT done" close: GATE 2/O8 out of scope, schema untouched); **A-443**
  (append-only correction to A-430's stale `name` — the shipped field is
  `key`, matching `EvidenceDeclaration.key` and CONSUMERS.md's own example;
  finding attributed to R-2's round-1 review, relayed by the controller;
  A-430 itself left untouched per A-408's append-only convention).
- **R-2's `78a786fc`** (`verify.py` + `tests/test_canary_multi_target.py`,
  a real gap in `_check_r3_rederivation`'s `budget_exhausted` validation
  branch) sits beneath this commit on the shared branch — landed directly by
  the reviewer, confirmed non-overlapping with every file this commit
  touches before proceeding.
- **Gate GREEN**, verdict read in a separate step: `gate-gen13.log` on
  **`d9fc22eb`**. `GATE_EXIT=0` exactly once, one
  `ASSAY_REGISTERED_GATE_COMPLETE=1`, zero `FAILED|DIRTY_TREE|Traceback`,
  wheel `assay-4.1.1.dev45+gd9fc22eb-py3-none-any.whl` carrying the exact
  commit, all twelve `ASSAY_GATE_PHASE` markers present including
  `verdict-v10-successors-verified`. Full local suite also run once, per the
  host rule: **4178 passed, 20 skipped**, 0 failed, 379.08s.
- **B004 is the last item BRIEF-12 named.** All eight post-cut items from
  the wave prompt are now done; the wave's own remaining steps are R-2's
  round 2, merge, and the 5.0.0 release — none of them this generation's to
  do.
- Ids used this generation: **A-442**, **A-443** (next free **A-444**); no
  B-id allocated (next free **B071**, unchanged).
