...
- feat(assay): `judge.language = "go"` resolves at **R1**. The Go adapter is registered in the CLI's built-in registry, deliberately as the last step after the statement-attribution chain above, so a Go lane could never be runnable while the parser still read block extents as statement truth. R2 and R3 stay unregistered (`generate_mutation_sites` is unconditionally `UNSUPPORTED`). A judging environment with no Go toolchain refuses `NO_MEASUREMENT`/`MISSING_EXTERNAL_TOOL` before the lane's command runs (A-394)
- feat(assay): `judge.coverage.producer` accepts `go-test` and `covdata` on a `go-cover` lane. The key is optional for this format because its two producers agree — `go tool covdata textfmt` emits the same instrumenter's same text format as `go test -coverprofile` — so the name records the evidence's provenance (integration vs unit) rather than changing how the artifact is read (B047 item 3, A-398)
- feat(assay): `helpers[]` gets its first PRODUCER. A Go R1 lane's verdict now records the toolchain that derived its statement positions — `role: "statement-positions"`, `tool: "go"`, the resolved path, and an `identity` the toolchain reported about itself. Lanes for every other language still OMIT the key (A-230a), and a helper whose claim is voided by a cleanup-only failure is dropped with it rather than left describing work that judged nothing. The correspondence rule now has exactly one definition, `verdict.supported_helper_roles`, read by the schema's own validation and by the producer (B047 item 5)

- feat(assay): `LanguageAdapter.for_project(repo_top, project_root)` — a new protocol member, called once per lane by the core before anything reads the coverage profile, returning the adapter to use for that project. Every adapter but Go returns itself; the Go one returns a copy carrying the module path read from the project's own `go.mod` (A-404)

### Fixed
- fix(assay): **`assay run` can judge a real Go module.** A Go cover profile keys records by import path while `git diff` uses repo-relative paths, and nothing set `GoAdapter.module_path` through the CLI — so every Go lane reachable from the shipped binary refused `ERROR`/`UNREADABLE_ARTIFACT`, with a message blaming a revision mismatch for what was an unstripped prefix. The module path is now DERIVED from the nearest `go.mod` at or above the lane's project root, parsing only its `module` directive; there is no lane key and no flag for it, because it is the project's fact and not a literal to restate (B059, A-404, DESIGN-GUIDE §5). A project root in no Go module refuses `ERROR`/`BAD_LANE_CONFIG`; a profile key outside the derived module refuses naming the key, the module path and the `go.mod` it came from. One Go module per lane; no `go.work` support
- fix(assay): the CLI's own `--help` and module documentation said `judge.language = "go"` was refused at any level, three releases of prose behind the registry itself
- fix(assay): `assay.registry`'s module documentation stated two facts that are no longer true — that the CLI registers "Python at `R1` only, and no entry at all for Go" (stale since SQL and JavaScript registered, before this wave), and that "every adapter a built-in registry can advertise today … declares `external_tools = ()`, so no real entry here ever exercises one" (A-087's premise, falsified by this wave's own `external_tools = ("go",)` plus A-394's registration). Both are rewritten to keep A-087's reasoning while stating what is now the case; the entry list is not restated at all, because a second copy of `cli._built_in_registry` is the thing that went stale twice. `assay.cli`'s own module docstring carried a third copy of the same list, still saying "JavaScript at R1 only" three releases after B046 admitted it at R2; corrected in the same pass
...
- fix(assay): the Go statement-position oracle is reachable from the shipped **zipapp**. `go run .` needs a real working directory, and inside a `.pyz` the helper's `stmtpos.go`/`go.mod` are zip members, so the adapter refused every Go lane with "assay's Go statement-position oracle is missing from the installation" for an oracle the artifact carried all along. The files are now read through `importlib.resources` — identical from a source tree, a wheel and a zip — and staged into a temporary directory for the duration of the call. This matters because the zipapp is the only install path into `tester-unified-go`, which inherits an interpreter but has no pip and no ensurepip (A-402, A-403)
...
- fix(assay): **a Go lane judged covered code as uncovered whenever its profile carried repeated records for one block** (B061). `go test -coverpkg=./...` instruments every package into every test binary and concatenates each binary's own profile section, so one block gets one record per binary and only the binary that ran it carries a non-zero count — srdm's real profile carries 20 records per block. The statement-position join keyed a dict by extent, so whichever record came LAST decided the block, and a covered block whose non-zero record was not final was attributed to `missing`. On srdm's own tree that was 255 lines reported uncovered where the same profile's own line-level fold says 45. Repeated records now fold executed-wins before any count is read — the rule the parser already applies one layer down — and the invariant that a correction can never downgrade a line the parser called executed is asserted directly. Found by F008-A5's qualification, on its first run
- fix(assay): **the committed Go coverage fixtures are real toolchain output, and the expectations read from them are the oracle's** (F008-A4, B057). `tests/fixtures/go/hello/hello.out` and both canary profiles were hand-authored and wrong in both coordinates (A-234); they are now `go test -coverprofile` output for the exact committed source bytes, produced together with a real statement-position oracle document by one re-runnable in-image script (`nyxloom-trove/carve-assets/P27-recarve/regenerate-fixtures.sh`). Every asserted line set is re-derived from that oracle in the same change, because real bytes read as statement truth would have been worse than the hand-authored ones — the real block `32.32,34.2` expands naively to three lines where Go has one statement. The tests join the two documents with the production `attribute_statements`, which refuses on any extent disagreement, so an edit to a fixture source that skips the regeneration turns the suite red instead of silently attributing stale lines
- fix(assay): **a real `go test -coverprofile` artifact is no longer refused for containing a zero column.** A `//line file:line` directive with no column makes `go/token` report `Column == 0` — the case `cmd/cover`'s own comment names (`cover.go:1055-1060`, Go issues #27530/#30746) and its `dedup` helper exists for — and assay refused the WHOLE artifact with "column number 0 … is not positive", a guessed fact about `cmd/cover` that `cmd/cover` disproves. Columns are now `>= 0` (lines stay `>= 1`) at all three sites, each citing the toolchain. Such a FILE is instead flagged as `//line`-remapped, and the disposition is per file: with no line in the judged set it is IGNORED, so one generated file no longer takes down a lane; inside the judged set the lane refuses `ERROR`/`BAD_LANE_CONFIG`, naming `//line`, the file and the remedy (generated sources belong outside `judge.source_roots`). Never a silent 0/0. Witness: Go's own `TestLineDup` corpus, run through the real toolchain and committed with its recipe (A-405)
- fix(assay): **a lane whose language is judged from block extents can no longer declare a coverage format that carries none.** `judge.language` and `judge.coverage.format` are independent by design, so a Go lane could declare `format = "lcov"` — and lcov converted from a Go coverprofile carries exactly the naive block expansion A-392 exists to refuse, arriving with no block extents, so the runner marked it statement-attributed with no oracle and no `helpers` entry and the guard passed it. Such a lane is now refused at CONFIG LOAD, `ERROR`/`BAD_LANE_CONFIG`, naming the language, the declared format and `go-cover` with its producers; the runner keeps a second independent refusal for library callers. The empty profile still terminates where it always did, `NO_MEASUREMENT`/`EMPTY_COVERAGE` (A-406)
- fix(assay): **a Go lane that refuses AFTER its coverage was measured now reports what is wrong with the artifact, and writes a verdict.** The statement-position helper is recorded the instant the oracle returns, so any judge refusal downstream of it — a stale profile whose extents no longer match the source, or a `//line`-remapped file the lane judges (A-405) — left that `helpers[]` entry with no claim to support, and `assemble_verdict`'s internal wiring guard fired instead: the operator saw `ERROR/BAD_LANE_CONFIG: lane 'unit' recorded helper role(s) ['statement-positions'] …`, a sentence about assay's own bookkeeping, and **no verdict document was written at all**. Both refusals now reach the consumer with their own reason code (`UNREADABLE_ARTIFACT`, `BAD_LANE_CONFIG`) and their own verdict artifact. The entry is dropped where the payload was taken away, using the one `supported_helper_roles` rule the schema validates with — the same move the cleanup-failure path already made — so the wiring guard stays a wiring guard (A-407)
- fix(assay): a malformed Go coverage record that ends before it starts *in the column* (`m/x.go:5.10,5.2 1 1`) escaped the parser as a bare `ValueError` rather than an `AssayError`, so the run ended in an uncaught traceback: no verdict document written at all, and the coverage reservation never released. It is now `ERROR`/`UNREADABLE_ARTIFACT` with both positions named, like every other malformed record
- fix(assay): repeated records for one block that DISAGREE about `numStmts` are refused rather than silently folded. Go's own profile reader refuses them (`inconsistent NumStmt: changed from %d to %d`, `x/tools/cover/profile.go`), and B061's fold transcribed that loop's merge rule without its guard; the fold now carries both, and cites the source for the merge rule it had been justifying by internal precedent
- fix(assay): `go.mod` parsing matches the real toolchain on three inputs it used to accept. A trailing slash (`module example.com/x/`, and `module /`) is REFUSED rather than stripped — `go list -m` says `malformed module path "...": trailing slash`, and `module /` previously escaped as an empty module path and surfaced as `ERROR`/`UNREADABLE_ARTIFACT`, blaming the coverage artifact for a lane-configuration fault. An unquoted argument containing `"` or a backquote is refused, the rule the module's own docstring already claimed (`invalid quoted string: unquoted string cannot contain quote`). All three messages measured against the toolchain in-image
- fix(assay): no Go test in the suite judges a downgraded adapter any more. `tests/test_canary_go_pipeline.py`'s `_PreOracleGoAdapter` — the real adapter with `requires_statement_attribution` flipped to `False` — is deleted: with genuinely statement-attributed fixtures the SHIPPED adapter judges them, so Go's cause-sensitive canary is now proven at statement granularity (the appended never-called function's two body statements, not its four-line block extent). `conftest.as_pre_oracle_attributed` is gone with it; its replacement, `as_statement_attributed`, refuses a multi-line block extent instead of trusting the caller (B057, all three boxes)
