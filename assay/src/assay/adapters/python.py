...
# B034/A-326: `_is_uuid_expression`, `_is_enum_member_expression` and
# `_semantic_comparison_sites` are DELETED here, not repaired. Their two
# operators produced Eq<->NotEq flips at the same byte spans, with the same
# replacement bytes, as `_compare_swap_sites` already produces -- measured
# over `src/assay/**.py`, 87 sites, zero that `compare-swap` did not already
# produce identically -- so co-selecting them emitted every shared site
# twice and added no mutation coverage of any kind. The vocabulary spellings
# survive for artifact compatibility only (see `assay.vocabulary.
# WITHDRAWN_MUTATION_OPERATORS`); no lane may declare them and no adapter
# produces them.
