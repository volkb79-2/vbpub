...
### Claude Code capability recipe (validated by P121–P123)

Do not use `--bare`: on this host it reports an unauthenticated state despite a
valid normal Claude Max login. Invoke the normal CLI with `--permission-mode
dontAsk --tools Read Edit`; path-scope every `Read` using the **real absolute
worktree path**, not a virtual `/topos/...` alias. This CLI rejects a
path-scoped `Edit(...)` rule under `dontAsk`, so allow `Edit` only after the
tool list has already been reduced to `Read Edit`, and keep the handoff's
single-file scope and controller diff audit mandatory. A clean BLOCKED before
an edit is route-health evidence, not a failure to be prompted through; rotate
the session when it replays stale handoff names or workspace identity.
...
| 3 | `claude/haiku` at `high` via Claude Code | User-requested low-cost Claude implementation floor. | **Qualified 2026-07-26** for narrow, controller-preflighted test packages; use the capability recipe recorded below, not the historical route's permissive dispatch flags. |
...
| `claude/haiku` at `high` | P121, P122 complete; P123 follow-on | **QUALIFIED** | The earlier `AUTH_REQUIRED` result was a harness false negative: `claude --bare` suppresses the normal authenticated credential integration. A normal `dontAsk` probe resolved `claude-haiku-4-5-20251001` and returned `ROUTE_READY`. P121 (`dbe4232e`) added three causal BPF fallback/report tests; P122 (`4aa4b567`) added three causal mounted-UI/defensive MouseTable tests. A stale resumed session first guessed the obsolete BPF handoff name and cleanly BLOCKED with no edit; a fresh session then met both packages after two controller harness fixes: use real absolute worktree paths for path-scoped `Read`, and allow the already tool-limited `Edit` capability because this CLI rejects path-scoped `Edit(...)`. The isolated gate passed at 2,181 tests in 50.63s with changed-line coverage 100%; DeepSeek Pro Max independently reviewed P121/P122 and returned **APPROVE** (one non-blocking unused-fixture nit, removed by controller). P123 (`d53dffeb`) reused the clean Haiku session for the remaining negative-cursor branch; the gate passed at 2,182 tests in 47.69s and `data_table.py` reached 100% statements/branches. This establishes a viable low-cost implementation route for frozen, one-file test packages; it is not evidence that arbitrary broad implementation work can use Haiku unsupervised. |
