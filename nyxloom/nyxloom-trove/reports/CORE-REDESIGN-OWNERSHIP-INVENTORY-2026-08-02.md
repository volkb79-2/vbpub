...
| --- | ---: | --- | --- | --- |
| `src/nyxloom/daemon.py` | 8,515 | Effect execution, process/gate/git/HTTP orchestration, shared mutable daemon state | CR-05, CR-13a, CR-15, CR-16 | Frontier implementation and review; split by effect boundary, never by arbitrary line ranges |
| `src/nyxloom/effects.py` | 576 | CR-05 (added 2026-08-03, CR-05a): the effect boundary itself — the injected ports (clock, processes, git, filesystem, background work, event log, journal), the per-action context, and the handler registry that gives every action type exactly one owner. Imports no other control-plane module, which is what lets an effector be exercised without constructing the shell. CR-05b lowers `LEGACY_HANDLER_BUDGET` to zero as it moves the remaining families; raising it is a review-rejectable change, not a merge-conflict resolution |
| `src/nyxloom/effects_gates.py` | 460 | CR-05 (added 2026-08-03, CR-05a): the gate-verify cadence and post-merge validation, including BOTH background-work registries the amendment's §3.3 requires to leave the shell. CR-12 owns making the gate result typed product evidence; CR-16 adds liveness on the same drain seam |
