...
Re-measured 2026-09-02 (nyxloom-P98) for the rows nyxloom-P98 moved past
their size tolerance or whose responsibility text it made stale, retiring
GA1 (`nyxloom gate verify`, canary-based external gate-trustworthiness
verification) and GA4 (the daemon's periodic gate-verify cadence) in favor
of Assay's own R2/R3 mechanisms once a project declares assay/run-gate
lanes (`nyxloom-trove/decisions.md`, 2026-09-02). Removed
`src/nyxloom/gate_canary.py`'s row entirely: the module is deleted outright,
so its path no longer exists. Re-measured `src/nyxloom/effects_gates.py`
(473 -> 354) and `src/nyxloom/cli.py` (2,469 -> 2,220), both past their own
declared tolerance after this package's deletions (the GA4 cadence's
background-work methods/state, and the GA1 CLI verb, respectively); trimmed
`effects_gates.py`'s responsibility text to drop the now-deleted cadence
half. Re-measured `src/nyxloom/rules_attention.py` (118 -> 83, still inside
its own tolerance today) and trimmed its responsibility text's gate-verify-
...
| `src/nyxloom/effects_gates.py` | 354 | CR-05 (added 2026-08-03, CR-05a): post-merge validation, including its background-work registry the amendment's §3.3 requires to leave the shell. CR-12 owns making the gate result typed product evidence; CR-16 adds liveness on the same drain seam. Re-measured 2026-09-02 (nyxloom-P98): the GA4 gate-verify cadence and its background-work registry are retired, leaving post-merge validation as the module's sole responsibility |
...
| `src/nyxloom/effects_exit.py` | 837 | CR-05 (added 2026-08-03, CR-05e): what an agent leg's EXIT means. The LAST effect family to move, because what an exit means depends on the attempt's ROLE rather than on which effector dispatched it -- so it composes the carve, carver, review and lifecycle effectors instead of duplicating any. Owns the receipts-are-process-facts rule: a clean exit is permission to LOOK for a typed judgement, never a verdict. CR-09 extends its decline handling; CR-11 the stale-premise admission; CR-07c (2026-08-04) moved `implement.done`'s target resolution to `stages.effective_exit_map` |
| `src/nyxloom/effects_dispatch.py` | 330 | CR-05 (re-measured 2026-08-04, CR-11a — it gained `route_currently_paused` and a route-health check inside `admissible()`: a route can be paused by ANOTHER task's LIMIT receipt mid-pass, mutating the SAME `ProviderPauseRegistry` `admissible()` now reads fresh via `ctx.provider_pause`, rather than the `provider_ok` snapshot baked once before the pass's action loop — closes the gap CR-08 Slice 2 named as CR-11's charter, sharpest for `ResumeAttempt`'s ordinary branch, which the planner never route-health-gates at all): the launch primitives the four agent-dispatching families share — execute-time admission, pause mode, budget, route health, gate hint, handoff frontmatter, mutex leases, approved scope amendments. Plain FUNCTIONS over the effect context, deliberately not a base class: inheritance would put "may this launch happen" on something an effector could override. CR-05c and CR-05d consume it as they move. CR-13a (2026-08-03) added the containment precondition as an optional `route` argument rather than the minted admission token this row previously planned: the token would have made the wrapper's gate a check of the DAEMON's decision, and the wrapper's gate is stronger derived from the route it was already given — an effector that mints nothing still cannot launch uncontained. The token remains the right shape for the non-daemon launch sites (intake, decision, onboarding) this predicate still does not cover |
...
| `src/nyxloom/effects_lifecycle.py` | 396 | CR-05 (added 2026-08-03, CR-05a; re-measured 2026-09-09, nyxloom-P105): task/attempt lifecycle bookkeeping, wave opening, spec-attention escalation, and the provider backoff registry — owned by the effector that writes it and injected into the input builder that reads it. B29 adds `mark_review_stalled`, the one handler here that appends an AUDIT event beside its state event: the typed review-stall reason and its compact controller summary |
...
| `src/nyxloom/rules_attention.py` | 83 | CR-06 (added 2026-08-03, CR-06a): the rules that claim nothing — the progress ratchet and the three spec-health signals. Every attention branch is deduped against an input flag because its condition is PERSISTENT; that is what stopped the 2026-07-16 notification storm and it is not an optimisation. Re-measured 2026-09-02 (nyxloom-P98): the third rule, the GA4 gate-verify-cadence-overdue probe that was deliberately outside the carve mutex, is retired |
...
| `src/nyxloom/projection.py` | 330 | CR-04b (added 2026-08-03): the pure event validation and projection both the public store API and the store implementation depend on. Extracting it is what removed the import cycle the CR-04 contract names; it does no I/O, which is what lets the store call it INSIDE a transaction against committed state. CR-07 changes the transition rules it encodes and must keep it pure. CR-07e (2026-08-04) added the versioned-event upcaster skeleton (`upcast_event_payload`/`upcast_state_dict`/`UpcastError`/`EVENT_UPCASTERS`/`STATE_UPCASTERS`) the plan assigned to CR-04 but never landed |
...
| `src/nyxloom/stages.py` | 414 | Seven-stage menu, stage ownership, preset closure. CR-06a (2026-08-03) made `effective_concurrency` log-free: it is the one helper the pure planner reaches, and its DEBUG record made the planner's advertised purity false transitively — the resolver must stay log-free or `tests/test_planning.py`'s purity oracle fails. CR-07c (2026-08-04) added `effective_exit_map`, the single function `validate_pipeline`, `effects_exit.py` and the shadow-compile projection all read to resolve `implement.done`'s composition-conditional target, closing the declared/actual divergence CR-07a found | CR-07 | Replace menu composition with compiled workflow IR only after semantic parity proof |
...
| `src/nyxloom/notify.py` | 819 | CR-16 (re-measured 2026-08-03): health alarms need an independent escape path, not only this transport. Adds `probe_transport` -- an active reachability probe distinct from `send()`, never publishing a real notification -- that `doctor.liveness_findings` drives as the second, transport-independent alarm path. Re-measured 2026-09-08 (NL-17 / nyxloom-P106, 532 -> 788): `send()`'s two hardcoded transports became a `NotifyBackend` seam (ntfy / webhook / mattermost), each backend owning its own payload translation |
...
| `src/nyxloom/decision_chat.py` | 676 | CR-09: the human-decision escape path a band-5 decline lands on; its transport must stay independent of route health |
...
| `src/nyxloom/commands.py` | 467 | CR-05, CR-16: operator chat-ops are effects; they must route through the same effect boundary and health alarm. CR-15 (2026-08-03) made its mutating verbs an authenticated ingress: they resolve a named channel operator before any project lookup and refuse otherwise |
...
| `src/nyxloom/review_progress.py` | new | B29 (added 2026-09-09, nyxloom-P105, LESSONS PL11): the review-leg progress detector. Deliberately NOT part of `watchdog.py` — that module detects repeating PATTERNS over a project's recent event window and remedies them project-wide (escalate/suppress/auto-pause); this one scores ONE in-flight leg against a budget from signals that are not in the event log at all (transcript extent, branch head), and its remedy is a per-attempt mark that hands off to the existing ladder. Same purity contract as `watchdog.py` for the same reason: the daemon measures, this decides |
...
| `tests/test_stages.py` | 362 | Registry/closure invariants for the stage menu CR-07 replaces, plus `effective_exit_map`'s own tests (CR-07c, 2026-08-04) | Structure mirror of a mechanism being replaced. Its closure invariants (no dead-end, single ownership, terminal reachable) must be re-expressed against the compiled workflow IR before the menu is deleted; the file itself retires with the menu, named in the CR-07 report. |
| `tests/test_types.py` | 245 | Transition-graph and serde invariants, plus `TaskState._missing_`'s DRAFT read-compat tests (CR-07d, 2026-08-04) | Behavior oracle. The lifecycle/node split (CR-07) may move members, never delete a proven invariant: every transition legality and serde round-trip assertion migrates with the type it covers. |
...
| `tests/test_workflow_shadow.py` | new | The shipped presets compiled and proven edge-for-edge equivalent to `stages.py`, and the §5.4 stop-loss measurement | Behavior oracle owned by CR-07. CR-07a pinned a real defect (the DECLARED form of every shipped preset was unreachable, because `stages.py` said `implement.done -> AWAITING_REVIEW` while `effects_exit.py` routed to `SELF_REVIEWING`, invisible to `validate_pipeline` rule 3). CR-07c (2026-08-04) fixed it at the source (`stages.effective_exit_map`) and replaced the pin with a test that the declared form now compiles clean. |
