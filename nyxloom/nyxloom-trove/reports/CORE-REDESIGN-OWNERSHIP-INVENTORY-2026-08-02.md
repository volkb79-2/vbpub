...
| `src/nyxloom/reconcile.py` | 1,928 | Pure planning: the action vocabulary, the input snapshot, the eligibility predicates, and the `plan_project` DRIVER. Re-measured 2026-08-03 (CR-06a): the lifecycle, review and attention concerns moved to rule modules and the pass machinery to `planning.py`; what is left of the monolith is the three concerns CR-06b (dispatch, attempt ladder) and CR-06c (carve authority) own, registered as legacy rules and counted by `planning.LEGACY_RULE_BUDGET` | CR-06, CR-07, CR-09, CR-11 | Differential old/new planner comparison; classify touched tests before retirement |
