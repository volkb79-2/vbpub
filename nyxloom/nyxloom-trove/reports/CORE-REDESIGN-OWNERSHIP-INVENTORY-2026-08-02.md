...
| --- | ---: | --- | --- | --- |
| `src/nyxloom/daemon.py` | 5,275 | Effect execution, process/gate/git/HTTP orchestration, shared mutable daemon state | CR-05, CR-13a, CR-15, CR-16 | Frontier implementation and review; split by effect boundary, never by arbitrary line ranges |
...
| `src/nyxloom/effects_carve.py` | 1,470 | CR-05 (added 2026-08-03, CR-05f): carve dispatch and proposal admission — the effect whose OUTPUT is a planner input, which is why every guard around it is about authority rather than correctness. Most of it is packet assembly: the entire interface between the factory's state and the agent that extends it, assembled from committed facts. Owns the carve outcome vocabulary; `daemon.py` re-exports it for the exit consumer CR-05e still holds |
| `src/nyxloom/effects_carver.py` | 1,019 | CR-05 (added 2026-08-03, CR-05d): the persistent strategic carver session — its three launch verbs, the snapshot reader whose None is the feature-off gate, and the proposal validators the repair turn depends on. The central rule is that the provider session is a CACHE and never an evidence identity: every turn mints a fresh attempt id and the route is pinned to the generation. CR-05f moves the carve-dispatch families that share its sequence counter and packet builders; CR-05e's exit consumer reads its turn envelopes |
