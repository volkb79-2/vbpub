...
| `src/nyxloom/notify.py` | 788 | CR-16 (re-measured 2026-08-03): health alarms need an independent escape path, not only this transport. Adds `probe_transport` -- an active reachability probe distinct from `send()`, never publishing a real notification -- that `doctor.liveness_findings` drives as the second, transport-independent alarm path. Re-measured 2026-09-08 (NL-17 / nyxloom-P106, 532 -> 788): `send()`'s two hardcoded transports became a `NotifyBackend` seam (ntfy / webhook / mattermost), each backend owning its own payload translation |
