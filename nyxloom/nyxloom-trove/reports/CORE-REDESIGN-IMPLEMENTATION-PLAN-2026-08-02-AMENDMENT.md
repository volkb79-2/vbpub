...
| CR-07e | pending, narrowed | The versioned-event upcaster skeleton: a no-op-everywhere upcast dispatch table (keyed `(schema_version, EventType)` or similar) inserted at the seam `storage_sqlite.py`'s `_row_to_event`/state-row equivalent already has (between `json.loads(payload)` and `Event.from_dict`/`TaskStateFile.from_dict`), gated on `row.schema_version < projection.SCHEMA_VERSION`. Proves the mechanism the plan assumed CR-04 delivered (`"schema and projection version tables plus upcasters"`) with a synthetic test (bump a private test-only schema version, hand-write a v1 row, register one upcaster, assert replay produces the v2 shape) plus a negative test (an unregistered upcast fails loudly rather than silently, per the acceptance-criterion wording `tests/test_storage_sqlite.py:618` already quotes: "failed upcast produce no authorizing effect"). Does not bump `projection.SCHEMA_VERSION` for real, does not decide the lifecycle/node schema's shape -- both stay out of scope. Zero production behavioral risk: every persisted row today is schema 1, so every real read path is a no-op through the new code. |
