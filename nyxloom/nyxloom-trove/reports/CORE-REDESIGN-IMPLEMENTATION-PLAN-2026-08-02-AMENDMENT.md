...
| CR-07d | pending | `DRAFT` read-compatibility via `TaskState._missing_` (mirroring `Role._missing_` at `types.py:151-159`, the one existing precedent) and removal of `TaskState.DRAFT` as a constructible, executable state -- pinned as inert since 2026-07-17 by `test_invariants.py:426-445`'s `xfail(strict=True)` (`_DRAFT_GAP_REASON`: "no code anywhere ever assigns DRAFT as a task's live state"). Requires proving the recounted frozen graph (51 edges / 35 kernel / 16 compiler, down from 55/38/17) with the same rigor CR-07a applied to 17, updating the checked-by-regex prerequisite inventory document, `KERNEL_OWNED_STATES` (`workflow_ir.py:190-192`), `stages.LIFECYCLE_STATES` (`stages.py:36-38`), `workflow_shadow.WAIT_NODE_IDS` (`workflow_shadow.py:109-114`), `render.py`'s DRAFT legend/color (`render.py:205,230,328`), and retiring the xfail plus its `KNOWN_STATE_GAPS` entry (`test_invariants.py:300-302`) and `tests/planner_synthetic.py`'s `"draft"` scenario (`planner_synthetic.py:176-181`). |
