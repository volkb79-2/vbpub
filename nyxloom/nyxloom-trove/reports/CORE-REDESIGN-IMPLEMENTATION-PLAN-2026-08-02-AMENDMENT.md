...
| CR-05d | next | Carve and the persistent carver session: `CarveDispatch`, `AdmitCarveProposal`, and the three session verbs, plus the ~1,500 lines of packet builders behind them. The largest remaining family. Budget 6 -> 1. Its consumers (`_consume_carve_exit`, `_consume_carver_session_exit`) are what the exit router delegates into, which is why it precedes CR-05e. |
