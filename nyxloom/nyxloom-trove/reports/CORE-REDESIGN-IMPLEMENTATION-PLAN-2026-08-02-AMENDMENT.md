...
| CR-07e | pending | The compiler-IR-to-dispatch wiring (the 17 compiler edges "onto compiled nodes") plus the lifecycle/node schema and the versioned-event upcaster machinery it requires -- discovered this session to not exist despite being assigned to CR-04 (see the CR-07c-split row). Largest and highest-risk of the three: touches live dispatch (`effects_exit.py`, `rules_lifecycle.py`, `reconcile.py`) for every task transition, and invents an upcast mechanism from nothing rather than plugging into one. Needs its own scoping/design pass -- likely a further split -- before implementation starts. Also inherits CR-07b's named residual (extending the factorization property across the acquisition boundary) and its three missing fact producers (`touches_security_boundary`, `gate_rigor`, `effective_band`). |
