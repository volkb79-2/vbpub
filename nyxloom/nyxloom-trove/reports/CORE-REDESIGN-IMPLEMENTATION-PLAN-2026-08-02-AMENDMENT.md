...
| CR-07f | pending | The compile-and-cache mechanism (compile a project's `cfg.pipeline` to a `WorkflowIR` once, keyed/invalidated by pipeline digest -- nothing today calls `compile_workflow` outside tests) plus the ~5-6 genuinely mechanical edge-lookup dispatch sites it unblocks: `effects_exit.py`'s self_review-verdict routing (`QUEUED`/`AWAITING_REVIEW`, currently hardcoded despite `stages.py`'s `self_review.exit_map` already declaring it) and review_independent-reject routing (`REVIEW_REJECTED`), `rules_lifecycle.py`'s `queue_admission` (`CARVED -> QUEUED`) and `decision_hold`'s return-to-QUEUED half. Same shape as CR-07c's `implement.done` repair, generalized from `stages.py`'s declarative record to the compiled IR. |
