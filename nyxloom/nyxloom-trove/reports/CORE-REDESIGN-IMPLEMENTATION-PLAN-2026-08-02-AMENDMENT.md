...
| CR-05c | next | The attempt lifecycle: `DispatchImplementer`, `ResumeAttempt`, `LaunchSelfReview`, and `EmitAttemptExit`. The exit consumer routes by attempt ROLE, so it belongs with the launch families rather than with whichever effector dispatched the leg -- and its CARVER branch delegates into carve, which is why CR-05d follows rather than precedes. Budget 9 -> 5. Everything it needs already exists: `effects_dispatch` for admission and leases, `effects_review.parse_reject_class` for the triage hint. |
| CR-05d | pending | Carve and the persistent carver session: `CarveDispatch`, `AdmitCarveProposal`, and the three session verbs, plus the ~1,500 lines of packet builders behind them. The largest remaining family. Budget 5 -> 0, at which point the readiness audit's final structural oracle applies: reject effector imports or effector-owned mutable state on `Daemon`, not merely a long `_execute`. |
