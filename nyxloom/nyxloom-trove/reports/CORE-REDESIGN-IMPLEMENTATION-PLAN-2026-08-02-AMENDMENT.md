...
| CR-04b | pending | The rest of CR-04: make SQLite unconditional and delete the `NYXLOOM_STATE_BACKEND` selector, the file backend and the dual-backend tests (17 refs in `src/`, the audit's 119 test call sites); extract pure validation/projection; storage and projection version metadata in a dedicated table (NOT the event envelope's `SCHEMA_VERSION`); upcasters; backup, restore, JSONL export and a TESTED re-import; and the fault matrix (disk-full, corruption, locked database, invalid event, failed upcast produce no authorizing effect). Split on the CR-02a/CR-02b precedent: the projection API was independently gateable and is the item with cross-package consequences. |
| CR-05 through CR-14, CR-16 | pending | Dependency order in section 7 remains authoritative. |
