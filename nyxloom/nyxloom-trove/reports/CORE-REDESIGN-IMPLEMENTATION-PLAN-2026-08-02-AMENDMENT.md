...
| CR-05b | next | The remaining 12 effect families: attempt dispatch/resume and exit consumption, review launch and verdict consumption, carve dispatch and admission, the three carver-session verbs, and auto-merge. They are already registered — `_execute_legacy` is reached through the SAME registry lookup, so there is no second dispatch path to hide in — and the work is to move each into an effector module, lower `LEGACY_HANDLER_BUDGET` in the same commit, declare its `emits`, and CONSUME its idempotency key (which is where the parent's "never a duplicate launch/merge" acceptance actually bites; CR-05a delivered the mechanism and the two background families, not the launch half). Extend the differential scenario list as each family moves. |
