...
| CR-07c | pending | The lifecycle/node migration, split out of CR-07b per the carving principle: the 17 compiler edges onto compiled nodes, the lifecycle/node schema through a CR-04 upcaster, `DRAFT` read-compatibility via enum `_missing_` rather than an executable workflow state, and repairing `stages.py`'s declared `implement.done` exit, which CR-07a pinned as disagreeing with `effects_exit.py` for every shipped preset including the default since B5 (2026-07-20). It also inherits CR-07b's named residual (extending the factorization property across the acquisition boundary) and its three missing producers. |
