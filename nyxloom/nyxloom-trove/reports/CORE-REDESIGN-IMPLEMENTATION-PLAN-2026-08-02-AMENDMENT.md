...
| CR-07c | pending, narrowed | Repair `stages.py`'s declared `implement.done` exit (`stages.py:100`, unconditional `-> AWAITING_REVIEW`) to agree with `effects_exit.py:738-749`'s actual routing (`-> SELF_REVIEWING` whenever `self_review` is composed, true for `full`/`gated`/`lean`/`DEFAULT_PIPELINE` since B5, 2026-07-20). The CR-07a pin at `tests/test_workflow_shadow.py:166-183` (`test_the_declared_form_of_every_self_reviewing_preset_is_unreachable`) asserts BOTH that `validate_pipeline` accepts the stale declaration AND that `compile_workflow(declared_source(preset))` raises `UNREACHABLE_NODE` -- post-fix it should assert the opposite (compiles clean) for all three presets. `Stage.exit_map` (`stages.py:53`) is a flat, pipeline-composition-blind tuple, so the repair is in tension with the dataclass shape as it stands today; at minimum the stale declaration must stop asserting a target the engine never produces for a shipped preset. Also retires or simplifies, once declared and actual agree: `DECLARED_DIVERGENCES` (`workflow_shadow.py:90-96`, currently a singular `"implement.done"` entry), `declared_source` (`workflow_shadow.py:233-245`), `_successor_state` (`workflow_shadow.py:143-153`), and `test_the_divergence_is_declared_and_singular` / `test_declared_source_is_a_no_op_without_self_review`. |
