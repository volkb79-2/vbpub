...
| CR-05d | next | Carve and the persistent carver session. **Sized before starting, and the shape is not what the earlier rows assumed** -- recorded here so the next session does not re-derive it. The family totals ~899 executable lines: carve dispatch and its exit consumer 137, the packet builders 134, the verdict-audit section 64, proposal admission 202, and the carver session 362. Carve dispatch and the session verbs CANNOT be split apart: `_execute_carve_dispatch` delegates into `_execute_carve_via_session_resume` when the session feature is on, and both sides share `_build_carver_carve_prompt`, `_rescope_context`, `_next_carve_seq` and `_ensure_worktree`. That matches the parent plan's own family list, which names "carve/session operations" as ONE family -- so the size is a property of the code, not of a bad cut. Proposal admission looks separable and is not: `_execute_admit_carve_proposal` needs `_carver_session` and `_validated_carve_proposals`, both INPUT-BUILDER readers called from `_build_input` and therefore CR-06's surface, not an effector's. Moving them is a CR-06 decision; injecting the daemon to reach them is forbidden. So CR-05d is either one ~700-line package (carve + session, leaving admission behind with a declared reader dependency) or it waits for CR-06 to settle where the carve-snapshot readers live. Budget 6 -> 1 either way. |
