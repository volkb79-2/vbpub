...
- `src/nyxloom/types.py` — the single authorized frozen-core edit:
...
- `src/nyxloom/adapters.py` — module docstring's `capture_session`
  ...
  non-claude routes.
- `src/nyxloom/wrapper.py` — module docstring's step 5 updated to note
...
- `src/nyxloom/cli.py` — module docstring's subcommand list gained
...
```
cd /workspaces/vbpub/nyxloom && /workspaces/vbpub/.venv/bin/python -m pytest tests/test_adapters.py tests/test_wrapper.py tests/test_cli.py tests/test_properties.py -v
...
```
cd /workspaces/vbpub/nyxloom && /workspaces/vbpub/.venv/bin/python -m pytest tests/ -v
...
- **The `merge` CLI verb did not exist anywhere before this change — I
  ...
  `reconcile.py`, `docs/SPEC.md`, this repo's sibling `nyxloom` v1 tree —
