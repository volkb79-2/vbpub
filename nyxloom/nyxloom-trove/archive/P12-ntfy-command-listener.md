...
- The listener reads the topic via the READ-ONLY identity: token from env
  ...
  ntfy user `cmd-reader`, read-only on `nyxloom-cmd`).
...
- `src/nyxloom/commands.py` (new module — the listener + verb handlers)
- `tests/test_commands.py` (new)
- `src/nyxloom/daemon.py` — NARROW EXCEPTION to the frozen rule: you may
...
5. Transport: fake server streams one command message then blocks; listener
   ...
   Authorization uses the PUBLISHER token env, tag nyxloomd-reply present)
...
7. Full suite still green: run the complete gate
   (`.../python -m pytest tests/ -q` from nyxloom) and report the tail.
