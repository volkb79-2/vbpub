# TUI-SPEC — `topos`: a host pressure inspector and cgroup forensics TUI
...
name topos; v0/v1/v1.5/v2 release cut; v1 read-only; Python+Textual; metric
...
Project name is **`topos`**. The package name, binary name, config directory,
daemon socket group, and BPF pin directory all use `topos` unless a later spec
...
`topos` is a host pressure inspector and cgroup forensics TUI, fast enough to be
...
- zero-argument `topos` prefers a compatible daemon and visibly falls back to
...
- React and pwmcp remain decided for the web client. Topos owns same-origin
  static assets and projected read/history routes over loopback HTTP; P81 shared
  redaction is a prerequisite. TLS, if needed, terminates outside Topos. The
  ...
  Topos features. A future shared mode requires an external authenticated TLS
...
`topos` is scoped to solve exactly these five things for one operator on one host,
...
toolkit and it is exactly what `topos`'s DAMON columns/detail-page generalize:
...
custom multiplexer; `topos`'s control stage (§3.6c) just needs to track which
...
atexit). `topos` must do the same (§3.6c) — and must NOT do it silently in the
...
- **Borrow:** the record daemon and the live viewer share one data model, so
  ...
  this is exactly `topos`'s collector/model/UI split (§6.1) and its
...
- **Borrow:** cgroup-native navigation (rows keyed by cgroup, not by container)
  ...
  primary key. (A data-model statement, not a UI-priority one: `topos`'s tree
...
- **atop**: C, curses. Per-process AND per-cgroup-ish resource accounting with
  ...
  second confirmation this is the right shape for `topos --replay`.
...
- **lazydocker**: Go + `gocui`, multi-pane keybinding-driven layout (containers /
  ...
  template for `topos`'s v2 `--admin`-gated actions (§4) — show the exact command,
...
opening a help screen — `topos` adopts this as a persistent one-line key-hint
...
submit step) — `topos`'s `/` filter behaves identically (incremental). btop's
...
legacy and not mounted here). `topos` uses a three-tier model:
...
so operators moving between `damon_cli.py` and `topos` get identical
...
6. **No shell-out to `damo` and no dependency on `../damon-analysis`'s venv.**
   `topos` vendors its own minimal sysfs writer (a slimmed reimplementation of
...
- Same `damon_stat` conflict handling and idx-allocation discipline as (c),
  ...
  the one topos-owned session it starts or adopts. A session created by the
  current daemon run is stopped on that run's shutdown; a verified topos-owned
  ...
  uses `topos damon stop --all-mine`. Interactive TUI starts remain behind the
...
One file, XDG-conventional location (`$XDG_CONFIG_HOME/topos/config.toml`,
falling back to `~/.config/topos/config.toml`; `--config <path>` override). Full
...
- `[hotkeys]` — profile selection (`topos` native / `htop` / `top` / `custom`)
...
Full table in §8. Summary of the design: a **`topos` native default** keymap
...
Compose-versus-reconstructed-`docker run` design. Topos discovers an owner chain
...
`phase_10`). None of this is currently visible to `topos` as anything other
...
there — flag for the `ciu` maintainer; this is `topos` asking `ciu` for a
small favor, not `topos` inventing its own parallel labeling scheme):**
...
when invoked via `ciu-deploy`, the active phase name), not a `topos`-side
reverse-engineering exercise. Until `ciu` ships it, `topos` uses the
...
- Runs as a systemd unit (`topos-daemon.service`) with one instance per host.
- Listens on a local Unix socket, not TCP, for v2. The socket group is `topos`,
  mode `0660`.
- Read-only API is available to members of the `topos` group.
...
| eBPF per-cgroup network accounting (v2, Appendix B) | custom `cgroup_skb` BPF program + per-CPU maps pinned under `/sys/fs/bpf/topos/` | provider emits same shape as v1 network providers; not in v1 |
...
`topos` is an observability tool first. Privilege boundaries are explicit because
...
- **Docker group implications:** membership in `docker` is already root-equivalent
  for Docker operations. `topos` does not make that worse by exposing v1 mutating
...
- **BPF pinned object lifecycle:** v2 BPF programs/maps are owned by the daemon or
  root helper, not an ephemeral TUI. Objects are pinned under `/sys/fs/bpf/topos/`
...
- **Daemon socket model:** v2 uses a local Unix socket with group `topos`, mode
...
```toml
# topos config — $XDG_CONFIG_HOME/topos/config.toml
...
[hotkeys]
profile = "topos"       # "topos" | "htop" | "top" | "custom"
...
| Action | `topos` (default) | `htop` profile | `top` profile |
|---|---|---|---|
| Move selection | `↑`/`↓`, `j`/`k` | `↑`/`↓` | `↑`/`↓` |
...
1. **Performance:** renders 30 discovered cgroups (mixed tree depth, ~10 with a
   ...
   over a 5-minute steady-state run (`pidstat -p $(pgrep -f topos) 5` or
...
11. **Packaging:** `pipx install topos` (from a local sdist/wheel in absence of a
   published package) succeeds cleanly and registers a `topos` console-script
...
9. **RESOLVED — naming:** the chosen name is `topos`; see Appendix A. Decision
...
Chosen name: **`topos`**. Use it for the package name, console-script/binary,
config path (`$XDG_CONFIG_HOME/topos/config.toml`), daemon socket group
(`topos`, mode `0660`), and BPF pin path (`/sys/fs/bpf/topos/`).
...
- Pin programs and maps under `/sys/fs/bpf/topos/`.
