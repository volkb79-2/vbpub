...
```bash
unzip -p topos/dist/topos-0.1.0-py3-none-any.whl topos-0.1.0.dist-info/METADATA
...
```bash
PYTHONPATH=topos/src python3 -m pytest topos/tests/test_packaging_metadata.py -q
# 2 passed in 0.03s
```
...
Installed topos version verification and replay smoke:
...
PYTHONPATH=topos/src python3 -m topos.acceptance smoke --json
```
...
- Command:
  - `PYTHONPATH=topos/src /tmp/p25-venv/bin/python -m topos.acceptance smoke --cgroup-root topos/tests/fixtures/cgroupfs/gstammtisch --replay topos/tests/fixtures/frames/gstammtisch-once.jsonl --json`
- Result:
  - exit `0`, `ok: true`
  - entities: `8`
...
PYTHONPATH=topos/src python3 -m topos.acceptance steady --json
```
...
- Command:
  - `PYTHONPATH=topos/src /tmp/p25-venv/bin/python -m topos.acceptance steady --cgroup-root topos/tests/fixtures/cgroupfs/gstammtisch --samples 2 --interval-s 0 --json`
- Result:
  - exit `0`, `ok: true`
  - samples: `2/2`
...
```text
topos acceptance tui-smoke  v0.1.0
...
- Command:
  - `PYTHONPATH=topos/src python3 -m topos.acceptance tui-smoke --replay topos/tests/fixtures/frames/gstammtisch-once.jsonl --json`
- Result:
  - exit `0`, `ok: true`
...
```bash
ps -o pid,rss,cmd -p "$(pgrep -f 'topos')"
```
...
pipx install ./topos/dist/topos-*.whl --force
topos --version
topos --replay topos/tests/fixtures/frames/gstammtisch-once.jsonl --step --ui-smoke
```
...
- TUI paddr modal requires exact `START`, starts only through
  `start_planned_paddr_session`, and reports duplicate topos-owned paddr
  sessions.
...
1. Baseline game/server session without topos-controlled DAMON.
2. Passive read-only topos TUI.
3. Controlled vaddr session against one entity.
4. Manual paddr host session.
5. Stop all topos-owned sessions and verify foreign sessions remain untouched.
...
```bash
/tmp/vbpub-topos-p17-venv/bin/python -m pytest topos/tests/test_network_providers.py -v --no-header 2>&1 | grep bpf
...
PYTHONPATH=topos/src /home/vscode/.venv/bin/python -m pytest \
  topos/tests/test_daemon_bpf_snapshot.py -q
...
- ``/sys/fs/bpf/topos`` is not writable
- No pinned BPF maps exist under ``/sys/fs/bpf/topos``
...
```bash
PYTHONPATH=topos/src python3 -m pytest topos/tests/test_inspect_files.py -v
...
```bash
python3 -m py_compile topos/src/topos/inspect_files/reader.py
# clean, exit 0
```
...
```bash
PYTHONPATH=topos/src python3 - <<'PY'
...
r = build_inspect_read(
    ...
    fixture_root=Path("topos/tests/fixtures/inspect_files/docker"),
    is_root=lambda: True,
)
...
All Python files under `topos/src/topos` and `topos/tests` compile cleanly:
...
```bash
PYTHONPATH=topos/src python3 -m pytest topos/tests/test_inspect_files.py -q
# 132 passed in 0.85s
```
...
```bash
PYTHONPATH=topos/src python3 -m pytest \
  topos/tests/test_daemon_paddr_lifecycle.py -q
# 13 passed in 0.22s
```
...
- Idempotent adoption: existing topos-owned marker is adopted without duplicate.
- Foreign session: non-topos markers and foreign kdamond slots untouched.
...
```bash
cd topos && python3 -m pytest tests/ -q
...
protocol op, and ``topos daemon health [--json]`` CLI. It models collector, BPF
...
```bash
PYTHONPATH=topos/src python3 -m pytest \
  topos/tests/test_daemon_component_health.py -q
# 49 passed in 3.47s
```
...
- CLI ``topos daemon health --json`` and ``--pretty-json``.
...
PYTHONPATH=topos/src /tmp/p43-clean-venv/bin/python -m pytest \
  topos/tests/test_daemon_p51.py topos/tests/test_daemon_broker.py \
  topos/tests/test_daemon_client.py topos/tests/test_daemon_component_health.py \
  topos/tests/test_record.py -q -W error
# 90 passed in 16.84s

PYTHONPATH=topos/src /tmp/p43-clean-venv/bin/python -m pytest topos/tests -q -W error
