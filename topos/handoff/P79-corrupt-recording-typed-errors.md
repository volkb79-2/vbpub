...
`topos report` **crash with an unhandled exception traceback** and exit 1:

```
$ topos report corrupt.jsonl.zst --json
...
- `src/topos/record.py` (or wherever `RecordReader` lives) - the read/decompress path,
  and how it currently reports an unknown format or a missing `zstandard`.
- `src/topos/cli.py` - `_main_report`, and how it turns reader errors into exit codes.
...
- `topos/CONTRACTS.md` - input trust, error disclosure.
...
2. **No raw exception text crosses the boundary.** The message names the file and the
   failure class in topos's own words. It does not paste `ZstdError`, a library
...
6. **A healthy recording still reports identically** - byte-compare the JSON output of
   `topos report` on `tests/fixtures/frames/gstammtisch-once.jsonl` before and after.
...
- `topos --replay` / snapshot-inspect error paths, unless the same reader function is
...
`topos/README.md` (work-package row), `docs/OPERATIONS.md` (one line: what a damaged
...
path `topos/src` and fail spuriously otherwise. The full suite must be **fully green**
