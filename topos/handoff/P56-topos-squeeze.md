# P56 - `topos squeeze` (Guided Working-Set Measurement)
...
Add `topos squeeze --target CGROUP_PATH [options]`: a guided, stepped
`memory.high` squeeze that measures a cgroup's real (hot) working set under
pressure, absorbing natively into topos the workflow already proven live by
...
(`src/topos/collect/cgroup.py`: `read_int`, `read_flat_kv`, `read_pressure`,
`read_text`) and the v2 admin action framework (`src/topos/actions/`: `P21`
...
roadmap, not blocking it). If P55 lands first, `topos squeeze` MAY reuse its
...
`inspect-files --target` flags (`src/topos/cli.py` lines ~203, ~209, ~489,
~495).
...
- Branch: `feat/topos-p56-topos-squeeze`
- Worktree: `.worktrees/-topos-p56-topos-squeeze`
- Touch only `topos/**`; write P56-LOG.md/P56-REPORT.md; commit, do not merge.

## Requirements

- Add `topos squeeze --target CGROUP_PATH --admin --confirm TEXT [options]`,
  dispatched like the existing `topos action`/`topos daemon`/`topos snapshot`
  ...
  gating pattern (`src/topos/actions/execute.py`: root-in-production,
...
- Options, defaults drawn directly from `container-mempress.sh`'s proven
  ...
  (default a topos-namespaced path, e.g. under the existing topos state/log
  convention — cite whatever convention `src/topos/actions/audit.py` or
  `docs/OPERATIONS.md` already establishes for topos-owned log locations
...
- Protocol (mirrors the script step-for-step): read current
  ...
  (`src/topos/collect/cgroup.py`); refuse targets with `memory.min > 0`
...
- Write a headered JSONL log — one `header` record (target, cgroup path,
  ...
  make this log schema-compatible with topos's existing record schema
  (`src/topos/record/writer.py`/`RecordWriter`, the same P2 header+frame
  JSONL convention used by `--record`) so `topos report` (P54, whenever it
...
- Audit the session (not each step) via the existing `AuditLog`
  (`src/topos/actions/audit.py`): one audit record at session start (target,
