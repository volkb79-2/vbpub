...
### B1. Check 3 tested `isError`, which is never true for a topos error
...
(`mcp/server/lowlevel/server.py:473-480`). topos's tools *return* their typed
failures as an ordinary `{"error": {"code": ...}}` payload (`mcp/server.py:81`
`_tool_error`). So `isError` is `False` for **every** typed topos error, and the
leg reported *"All 4 tools returned successful results"* even if the daemon
rejected every single call. `topos_overview` was additionally never folded into
...
interpreters: in **four of six**, `topos_entity`/`topos_history` ran *before*
`topos_overview`, so they were called with an empty selector and got a typed
...
the live `topos_overview` response" was being satisfied only by luck, and the two
...
| 6 | Teardown unlinked the socket unconditionally: `mcp-smoke --socket /run/topos/topos.sock` would **delete the packaged system daemon's socket**. Only a socket this process created is removed now. The tmpdir was created unconditionally but removed only on the default path, leaking a directory per `--socket` run. | no |
