...
- Added **`topos/src/topos/inspect_files/reader.py`** — bounded read API with:
...
- Extended **`topos inspect-files read`** CLI subcommand:
  - `topos inspect-files read --kind --target --inspect-files --admin [--json] [--max-bytes N] [--max-lines N]`
