...
- Focused: `PYTHONPATH=topos/src /tmp/p43-clean-venv/bin/python -m pytest topos/tests/test_report.py -q` → `91 passed in 3.02s`.
- Full suite: `PYTHONPATH=topos/src timeout 400 /tmp/p43-clean-venv/bin/python -m pytest topos/tests -q -W error` → `1037 passed, 2 skipped in 122.79s`.
...
- Scope: all 8 files under `topos/**`. Docs (README, OPERATIONS) updated with the gate example and the new exit-1 semantics.
