# P56 — `topos squeeze` (Guided Working-Set Measurement) — Implementation Report

## Summary

Implemented `topos squeeze --target CGROUP_PATH --admin --confirm SQUEEZE`: a
guided, stepped `memory.high` squeeze that measures a cgroup's real (hot+warm)
working set under pressure, absorbing into topos natively the workflow proven
...
### New module: `topos/src/topos/actions/squeeze.py`
...
- `_default_squeeze_log_path(target)` — default log under `/var/log/topos/squeeze/`
...
### Tests: `topos/tests/test_squeeze.py` — 31 tests
...
1. ✅ `topos squeeze --target CGROUP_PATH --admin --confirm TEXT [options]`
...
None. The squeeze module is additive and package-private (`topos/actions/`).
It reuses `read_int`/`read_flat_kv`/`read_pressure` from `topos.collect.cgroup`
and `AuditLog` from `topos.actions.audit`.

## Test evidence

```bash
PYTHONPATH=topos/src python3 -m pytest topos/tests/test_squeeze.py -q
31 passed in 0.32s
...
PYTHONPATH=topos/src python3 -m py_compile \
  topos/src/topos/actions/squeeze.py \
  topos/src/topos/actions/__init__.py \
  topos/src/topos/cli.py \
  topos/tests/test_squeeze.py
...
- The default `--audit-path` points at `/var/log/topos/actions.jsonl` but
