...
**Base:** `fba1d89` (docs(topos): carve P42 daemon BPF snapshot bridge)
**Date:** 2026-07-10 (updated after controller review)

## What Was Built

### `topos/src/topos/daemon/bpf_snapshot.py` - `BpfSnapshotBridge`
...
1. **Path confinement.** The map pin path is validated to be beneath the
   configured topos BPF root. Uses ``Path.is_relative_to`` (not string prefix
...
7. **Atomic write to separate state_dir.** Writes via a private
   ...
   ``/run/topos/bpf``, separate from the bpffs pin root), flushes and fsyncs
...
### Integration into `topos daemon serve`

The bridge is integrated into `topos daemon serve` via:
- `--bpf-root PATH` CLI argument (disabled by default, default `None`)
- `--bpf-interval SECONDS` CLI argument (default `30.0`, minimum `5.0`)
- `--bpf-state-dir PATH` CLI argument (default `/run/topos/bpf`)
...
### Configuration (`src/topos/config.py`)

Added `BpfSnapshotConfig` dataclass:
- `enabled: bool = False`
- `root: Path | None = None`
- `interval: float = 30.0`
- `map_name: str = "topos_cgroup_skb"`
- `state_dir: Path = Path("/run/topos/bpf")`

Integrated into `ToposConfig.to_primitive()` and `load()`.

### Tests - 44 test functions (was 29)

All in `topos/tests/test_daemon_bpf_snapshot.py`:
...
- **Snapshot written to separate state_dir, not bpffs pin root.** The bridge
  writes ``snapshot.json`` to ``state_dir`` (default ``/run/topos/bpf``)
...
1. Separate snapshot ``state_dir`` (``/run/topos/bpf``) distinct from bpffs pin root
...
```bash
cd /workspaces/vbpub/.worktrees/-topos-p42-daemon-bpf-snapshot-bridge
...
PYTHONPATH=topos/src /home/vscode/.venv/bin/python -m topos.acceptance \
  tui-smoke --replay topos/tests/fixtures/frames/gstammtisch-once.jsonl --json
...
1. **Live `bpftool` execution not tested.** This host has no `bpftool`
   installed, no writable `/sys/fs/bpf/topos`, and no pinned BPF maps. All
...
```
M topos/README.md                             (P42 row: Planned -> Done)
M topos/MEASUREMENTS.md                       (P42 evidence section)
M topos/docs/BPF-NETWORK-ACCOUNTING.md        (P42 implementation status)
M topos/docs/OPERATIONS.md                    (bpf_snapshot config example)
M topos/docs/ROADMAP.md                       (P42 status: done)
M topos/docs/STATUS.md                        (P42 added, test counts updated)
M topos/src/topos/daemon/__init__.py           (BpfSnapshotBridge/BpfSnapshotError exports)
M topos/src/topos/config.py                    (BpfSnapshotConfig with state_dir)
M topos/src/topos/cli.py                       (--bpf-root/--bpf-interval/--bpf-state-dir,
                                                 BpfProvider integration, immediate refresh)
A topos/src/topos/daemon/bpf_snapshot.py       (BpfSnapshotBridge module, ~500 lines
                                                 with controller review fixes)
M topos/src/topos/providers/net_bpf.py         (state_dir parameter, fallback to bpf_root)
A topos/tests/test_daemon_bpf_snapshot.py      (44 focused tests, +15 for review fixes)
A topos/handoff/reports/P42-LOG.md             (work log, updated)
A topos/handoff/reports/P42-REPORT.md          (this file, updated)
```
