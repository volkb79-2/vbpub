...
`topos mcp serve` is a stdio-only, read-only MCP server in the optional
`topos[mcp]` extra (`mcp>=1.28.0`; tested with 1.28.1). It exposes exactly:

| Tool | Enforced bounds |
|---|---|
| `topos_health` | <=16 component summaries; 4 MiB aggregate response |
| `topos_overview(sort_by, limit)` | closed sort keys; integer limit 1..50; 4 MiB aggregate response |
| `topos_entity(selector)` | exact EntityKey or P57 docker name/prefix; <=128 metrics and <=64 findings; 4 MiB aggregate response |
| `topos_history(selector, metric, window, limit)` | exact EntityKey or P57 docker name/prefix; 1..100 points; `last:Ns` max seven days; 4 MiB aggregate response |
...
```text
PYTHONPATH=topos/src /usr/local/py-utils/venvs/pytest/bin/python -m pytest \
...
```text
PYTHONPATH=topos/src python3 -m py_compile \
  topos/src/topos/cli.py topos/src/topos/mcp/__init__.py \
  topos/src/topos/mcp/server.py topos/tests/test_mcp_server.py \
  topos/tests/test_textual_boundary.py
# exit 0
git diff --check
# exit 0
```
...
```text
timeout 900 env PYTHONPATH=topos/src /usr/local/py-utils/venvs/pytest/bin/python -m pytest topos/tests -q -W error
