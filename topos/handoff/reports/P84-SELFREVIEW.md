...
| Gate | Status | Notes |
|---|---|---|
| `pytest topos/tests/test_report.py -q -k "oracle_1"` | ✅ Banner fires, exit 0 | zstandard absent — banner primary mechanism |
| `pytest topos/tests/test_report.py -q -k "oracle_3"` | ✅ No false-positive banner | Correctly silent for non-zstd tests |
| `pytest topos/tests/test_report.py -q` | ✅ 117 passed, 6 skipped, banner fires | Full test_report gate |
| `pytest topos/tests -q` (with zstandard installed) | ✅ 1337 passed, 0 skipped, 2 pre-existing failures | P85 UI flakes only; banner correctly absent |
| `python3 -m py_compile topos/tests/conftest.py` | ✅ OK | |
...
| 4. Document how to build gate env | `topos/README.md`, `docs/STATUS.md` | §Gate environment section |
| 5. No behavior change to topos itself | N/A (negative) | No source code edits |

All 6 files in the diff are inside `topos/`. No other vbpub areas touched.
...
|---|---|---|---|
| 1. Gate env runs zstd oracles | `pip install -e 'topos[dev]'` + test IDs include zstd oracles | Test IDs printed in pytest output | N/A (requires controller's [dev] env) |
...
| 4. zstandard not hard runtime dep | `topos report` on plain `.jsonl` works | No source changes; existing behavior | Regression risk if pyproject.toml changed incorrectly — caught by test suite |
...
- All paths resolve correctly: `topos/tests/conftest.py`, `topos/pyproject.toml`, etc.

### 5. LOG, REPORT present; ASCII; no dead code/scaffolding in the diff

**Pass.**

- LOG present: `topos/handoff/reports/P84-LOG.md` (96 lines)
- REPORT present: `topos/handoff/reports/P84-REPORT.md` (117 lines)
...
Installed `zstandard` (0.25.0) and ran `pytest topos/tests/test_report.py -q -k "oracle_1"`:
