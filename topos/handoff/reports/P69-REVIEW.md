...
Docs-only, as required: the diff touches `topos/docs/**` and
`topos/handoff/reports/**` and not one line under `src/`.

## The DECISIONS-INBOX disposition (the thing this review had to decide)

Pass #1 loudly flagged that the implementer created `topos/docs/DECISIONS-INBOX.md`
...
- **D-001** (v1 web stack): a genuine product call. Its load-bearing technical
  claim -- that a `topos[web]` extra *cannot* make same-wheel package data
  conditional, so static assets grow the plain `pip install topos` wheel -- is
...
2. **`flagged-by-pass-1: no` - the carve bug is still live.** The prior review
   ...
   to the handoff-authoring guide in `topos/README.md`.
...
| Socket is `0660 root:topos` | `DAEMON.md:15-21` | confirmed |
...
daemon whose current boundary is a `0660 root:topos` Unix socket. It specifies four
...
diff scope                   topos/docs/** + topos/handoff/reports/** only, 0 source files
git diff --check             OK
```
