...
A read-only ZFS ARC host provider that teaches topos about ARC memory. On a ZFS
host, the ARC can hold many GB of RAM that `MemAvailable` counts as unavailable,
so topos's host memory banner would report severe memory pressure on a host that
...
```bash
$ PYTHONPATH=topos/src python3 -W ignore -m pytest \
  topos/tests/test_host_swap.py \
  topos/tests/test_p23_zram_drilldown.py \
  topos/tests/test_ui_banner.py \
  topos/tests/test_zfs_arc.py \
  topos/tests/test_collector.py -q -W ignore
...............................................                          [100%]
47 passed in 0.68s
```
...
```bash
$ PYTHONPATH=topos/src python3 -m topos.cli --once --json | python3 -c "import sys,json; d=json.load(sys.stdin); print({k:d['host'][k] for k in d['host'] if 'zfs' in k})"
