...
P84 documents `pip install -e 'topos[dev]'` as *the* way to build the gate env.
I built exactly that venv and ran the suite:

```
3 failed, 1320 passed, 1 skipped
SKIPPED [1] topos/tests/test_mcp_server.py:17: topos[mcp] extra not installed
GATE FAILED banner:  ABSENT
exit code: 1 (from test failures, not from P84's gate)
```

The `[dev]` extra pinned `topos[zstandard]` and `pytest` — but **not** `mcp`. So
...
Adding `topos[mcp]` to the extra makes that venv green: **1337 passed**, the 3
...
procedure, `pytest topos/tests -q` runs the zstd oracles — assert on the test IDs
...
two report files — all in scope, all in `topos/`. Contract 5 ("no behavior change
to `topos` itself") holds: no `src/` file is touched.
...
python3 -m venv /tmp/p84-devenv && /tmp/p84-devenv/bin/pip install -e 'topos[dev]'
  -> installs topos, zstandard 0.25.0, textual 8.2.8, pytest, mcp
timeout 900 pytest topos/tests -q          -> 1337 passed  (2 P85 flakes, fixed by P85)
pytest topos/tests/test_gate_environment.py -> 2 passed
