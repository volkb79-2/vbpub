...
Extended `python -m topos.acceptance` with a `tui-smoke` subcommand that
...
3. **Core logic** (`run_tui_smoke()`) — uses `subprocess.run()` to invoke
   `python -m topos.cli --replay PATH --step --ui-smoke` with optional
...
6. **Import contract preserved** — no `textual` or `topos.ui.*` imports in the
...
```bash
PYTHONPATH=topos/src python3 -m pytest topos/tests/test_acceptance.py -q --tb=short
# 40 passed in 7.21s
```
...
```bash
PYTHONPATH=topos/src python3 -m pytest topos/tests -q --tb=short
...
```bash
python3 -m py_compile topos/src/topos/acceptance.py topos/tests/test_acceptance.py
# (no output — clean)
```
...
| `test_subprocess_tui_smoke_json` | `python -m topos.acceptance tui-smoke --json` exit 0 |
...
```
$ python3 -c "import topos.acceptance; import sys; print('textual' in sys.modules)"
False
```
...
- The `tui-smoke` command requires Textual to be installed in the Python
  environment where the child process runs, but `topos.acceptance` itself does
