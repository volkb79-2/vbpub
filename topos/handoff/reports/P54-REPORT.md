...
1. **`topos/src/topos/report.py`** — New module containing:
...
2. **CLI additions** in `topos/src/topos/cli.py`:
   - `parse_report_args()` — Parses `topos report FILE [--window last:Ns|all] [--group-by slice|entity] --json`; `--json` is required (argparse `required=True`).
...
3. **Tests** — 57 tests in `topos/tests/test_report.py` covering:
...
4. **Documentation updates:**
   - `README.md`: quickstart examples for `topos report`, description paragraph referencing P2 reader reuse and gstammtisch stack measurement program; P54 marked Done.
...
None. The report module is additive and package‑private within `topos/`. No shared interfaces in `CONTRACTS.md` were modified.
...
```bash
$ python3 -m py_compile topos/src/topos/report.py
$ python3 -m py_compile topos/src/topos/cli.py
$ python3 -m py_compile topos/tests/test_report.py
```
...
```bash
$ PYTHONPATH=topos/src python3 -m topos.cli report topos/tests/fixtures/frames/gstammtisch-once.jsonl --json
{"metrics_version":1,"profiles":[{"gauges":{...}},...]}
```

```bash
$ PYTHONPATH=topos/src python3 -m topos.cli report topos/tests/fixtures/frames/gstammtisch-once.jsonl --json --group-by slice
...
```
M topos/README.md
M topos/docs/ARCHITECTURE.md
M topos/docs/OPERATIONS.md
M topos/src/topos/cli.py
A topos/src/topos/report.py
A topos/tests/test_report.py
A topos/handoff/reports/P54-LOG.md
A topos/handoff/reports/P54-REPORT.md
```
