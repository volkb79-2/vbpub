...
- Branch: feat/topos-p88-unified-frame-query-core
- Worktree: /workspaces/vbpub/.worktrees/topos-p88-unified-frame-query-core
...
- Follow-up: implement topos.query package (errors, source, semantics, engine,
  serialize) + `topos query` CLI + tests.
```

```text
2026-07-15 (later)
- Action: implemented topos.query (errors, source, semantics, engine,
  __init__) + `topos query` CLI subcommand; wrote topos/tests/test_query.py
...
- Commands:
  PYTHONPATH=topos/src .venv/bin/python -m pytest topos/tests/test_query.py -q -W error -p no:schemathesis  -> 62 passed
  focused query+report+daemon(P63/P52)                                                                     -> 262 passed
  timeout 900 ... pytest topos/tests -q -W error -p no:schemathesis                                        -> 1512 passed (zero skips), then re-run after cleanups
