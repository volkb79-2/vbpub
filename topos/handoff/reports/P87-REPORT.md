...
Added a narrow, fail-closed owner / protected-ID safety gate on Topos's raw
...
New file `src/topos/actions/owner_safety.py`:
...
`src/topos/cli.py`: the `action execute` path wires `owner_inspect=
...
Environment: fresh worktree venv, `pip install -e './topos[dev]'` (exit 0);
`PYTHONPATH=topos/src` from the worktree root. This is agent-env evidence; the
