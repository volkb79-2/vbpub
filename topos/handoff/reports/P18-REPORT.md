...
**Base:** `6a9ebab` (docs(topos): record P21 merge evidence)
**Date:** 2026-07-09

## What was built

### `topos/src/topos/providers/net_bpf.py` - `BpfProvider`
...
2. **Map format.** The snapshot contains:
   - `maps["topos_cgroup_skb"]`: list of entries with `cgroup_id` (int),
...
All in `topos/tests/test_network_providers.py`:
...
| `test_bpf_provider_is_publicly_exported` | `topos.providers.BpfProvider` public export works |

### Fixtures

- `topos/tests/fixtures/bpf/working/snapshot.json` - valid BPF snapshot with 14
  map entries across 4 cgroup IDs
- `topos/tests/fixtures/bpf/corrupt/snapshot.json` - deliberately malformed JSON
- `topos/tests/fixtures/bpf/unavailable/` - empty directory (no snapshot file)
...
```bash
# Full suite
PYTHONPATH=topos/src /tmp/vbpub-topos-p17-venv/bin/python -m pytest topos/tests -q
........................................................................ [ 48%]
........................................................................ [ 97%]
...                                                                      [100%]
...
PYTHONPATH=topos/src /tmp/vbpub-topos-p17-venv/bin/python -m pytest \
  topos/tests/test_network_providers.py -q
15 passed in 0.16s
...
```bash
cd /home/vb/volkb79-2/vbpub/.worktrees/-topos-p18-bpf-provider
/tmp/vbpub-topos-p17-venv/bin/python -m py_compile \
  topos/src/topos/providers/net_bpf.py topos/src/topos/providers/__init__.py \
  topos/src/topos/collect/collector.py topos/src/topos/model.py \
  topos/src/topos/registry.py topos/tests/test_network_providers.py
# (exit 0, no output)
```

### `topos --once --json` smoke using fixture root

```bash
PYTHONPATH=topos/src /tmp/vbpub-topos-p17-venv/bin/python -m topos.cli \
...
1. **Live BPF overhead not measured.** This host has no `bpftool`, the running
   uid is not root, and `/sys/fs/bpf/topos` is not writable. The P17 gate
...
```
M topos/README.md                           (P18 row Done)
M topos/MEASUREMENTS.md                     (P18 evidence section)
M topos/docs/STATUS.md                      (BPF provider: Partially Implemented)
M topos/src/topos/collect/collector.py       (BPF metric source label)
M topos/src/topos/model.py                   (additive bpf MetricSource)
M topos/src/topos/providers/__init__.py      (BpfProvider export)
M topos/src/topos/registry.py                (provider:network-bpf references)
M topos/tests/test_network_providers.py      (9 BPF tests + imports)
A topos/src/topos/providers/net_bpf.py       (BpfProvider class, ~200 lines)
A topos/tests/fixtures/bpf/working/snapshot.json
A topos/tests/fixtures/bpf/corrupt/snapshot.json
A topos/tests/fixtures/bpf/unavailable/
A topos/handoff/reports/P18-REPORT.md        (this file)
A topos/handoff/reports/P18-LOG.md           (work log)
```
