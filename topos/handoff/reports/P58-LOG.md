...
- No out-of-scope file: every implementation and self-review change is under
  `topos/**`. No raw socket/envelope parser is present in `topos.mcp`.
...
timeout 900 env PYTHONPATH=topos/src \
  /usr/local/py-utils/venvs/pytest/bin/python -m pytest topos/tests -q -W error
1115 passed, 2 skipped in 138.58s

PYTHONPATH=topos/src python3 -m py_compile <all changed/new Python files>
# exit 0
git diff --check
# exit 0
```
