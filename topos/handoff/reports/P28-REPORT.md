...
- Added `read_io_max_caps()` parser to `topos/src/topos/collect/cgroup.py`:
...
- Added `io_cap_saturation_pct` derivation in `topos/src/topos/collect/collector.py`:
...
- Added registry entry in `topos/src/topos/registry.py`:
  - `io_cap_saturation_pct` with unit `%`, branch_policy `local_only`, sources `io.max` + `io.stat`
- Added table support in `topos/src/topos/ui/table.py`:
  - Label: `IO_CAP%`
...
- Feature branch merged to main with `git merge --no-ff feat/topos-p28-io-cap-saturation`.
- Merge commit: `177c370 Merge topos P28 io cap saturation`.
- Pre-merge validation:
  - `python3 -m pytest topos/tests/test_io_cap_saturation.py -v` → `15 passed in 0.10s`
  - `python3 -m pytest topos/tests -q` → `216 passed in 29.28s`
