...
already established, and it answers a question topos currently answers WRONG on
any ZFS host — see below.
-->

## Goal

Teach topos that ARC memory exists. On a ZFS host, the ARC can hold many GB of
RAM that `MemAvailable` counts as *unavailable*, so topos's host memory banner
...
- **The exemplar to imitate is P19 + P23**: `src/topos/collect/host.py` (how host
  facts are read and turned into `host_*` MetricValues), `src/topos/registry.py`
...
- `topos/CONTRACTS.md` §4 (`Frame.host`, `Frame.host_meta`, `MetricValue.src`),
  §8 (degradation: every read wraps errors into `unavail_*`, never fabricates 0).
- `src/topos/ui/banner.py` — where host facts are rendered.
- `topos/docs/COMPRESSED-SWAP.md` — the precedent for documenting a memory-backend
...
5. **Banner annotation, no new UI surface.** The host banner gains an ARC segment
   ...
   hotkey, no layout redesign. Textual stays confined to `src/topos/ui/`.
...
`topos/README.md` (work-package table row), `docs/ROADMAP.md` (mark the ZFS item
...
```bash
PYTHONPATH=topos/src python3 -m pytest topos/tests/<new zfs test file> -q -W error
timeout 900 env PYTHONPATH=topos/src python3 -m pytest topos/tests -q -W error
PYTHONPATH=topos/src python3 -m topos.cli --once --json    # must still work on a non-ZFS host (this one)
