...
Extend P52's typed `DaemonClient` (`topos/src/topos/daemon/client.py`) with
...
(P52, `topos/src/topos/daemon/api.py`) serves `hello`/`current`/`history`/
...
`topos/handoff/reports/P58-REVIEW.md` (B3 blocker) and the terra-med v2
BLOCKED reasoning in `topos/handoff/reports/P58-REPORT.md` /
`P58-LOG.md` on branch `feat/topos-p58-daemon-mcp-frontend-v2`.
...
- Branch: `feat/topos-p63-daemon-client-versioned-read-methods`
- Worktree: `.worktrees/-topos-p63-daemon-client-versioned-read-methods`
- Touch only `topos/**`; write P63-LOG.md/P63-REPORT.md; commit, do not merge.

## Context To Read First (bounded — do not survey the whole tree)

`topos/README.md` (Workflow protocol), this handoff, `topos/CONTRACTS.md` §10
(P52 envelope, error-code enum, sensitivity enum), `topos/src/topos/daemon/
client.py` (the existing typed-client pattern you are extending),
`topos/src/topos/daemon/api.py` (the server whose envelope you decode — read
...
`topos/tests/test_daemon_client.py` + P52 tests for the transport/decoding
...
```bash
PYTHONPATH=topos/src python3 -m pytest <focused P63/client tests> -q -W error -p no:schemathesis
