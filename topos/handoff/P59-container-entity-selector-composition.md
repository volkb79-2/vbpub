...
> **Session-hint:** fresh
> **Serialize-with:** P60 (shared file: `src/topos/cli.py` argument parsing / `_filter_kwargs`)
...
`src/topos/cli.py` and P57-REPORT "P55/P56 Composition Notes"). Both are now
merged, so this wiring is unblocked.

## Context To Read First

- `src/topos/cli.py`: the top-level `parse_args()` (`--entities`/`--slice`/
...
- `src/topos/collect/dockerjoin.py`: `resolve_container_key()`,
  `ContainerResolveError` (P57).
- `src/topos/collect/collector.py`: `Collector` filtering params
  (`entities_globs`, `slice_names`) and `collect_once()` entity-filter step.
- `src/topos/collect/cgroup.py`: `build_entity_predicate`, `add_entity_ancestors`.
