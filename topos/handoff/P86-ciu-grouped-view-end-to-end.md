...
- `src/topos/ui/app.py` - `action_toggle_view`, `_refresh_view`, `VIEW_MODES`,
  `action_open_drill`, and the `self.selected_key not in frame.entities` guard.
- `src/topos/ui/table.py` - `render_data_table_container_grouped`,
...
1. **Drive the real app.** Tests construct a `ToposApp` over a frame containing
...
```bash
PYTHONPATH=topos/src python3 -m pytest <focused P86 tests> -q -W error -p no:schemathesis
...
Build the gate environment with `pip install -e 'topos[dev]'` (P84). A run that
