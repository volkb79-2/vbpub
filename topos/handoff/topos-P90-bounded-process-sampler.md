---
schema_version: 1
id: topos-P90-bounded-process-sampler
project: topos
...
- **Worktree:** create a git worktree for branch `feat/topos-p90-bounded-process-sampler`
  at `.worktrees/topos-p90-bounded-process-sampler` (repo-root-relative, per
  `worktree_root` in `topos/.nyxloom/project.toml`) from `main`; do all
  implementation work there, never in the primary checkout.
- **Branch:** `feat/topos-p90-bounded-process-sampler`
