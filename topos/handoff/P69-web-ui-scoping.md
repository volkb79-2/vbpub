...
Answer, with evidence from the merged code, **what a topos web UI should be and
...
## Deliverables (all under `topos/docs/` or `topos/handoff/`)

1. **`topos/docs/WEB-UI-SCOPING.md`** — the analysis. Sections:
   ...
   - **Page inventory.** The smallest set of screens that makes the product
     useful to someone who is not already a topos CLI user. Ground each page in
   ...
   - **Trust boundary.** The daemon socket is `0660 root:topos` on localhost.
   ...
   - **Framework/stack options.** At most three candidates, each with: build/
     ...
     packaging story (does `pip install topos[web]` still work?), and how it is
...
3. **`DECISIONS-INBOX.md` entries** for every question that is a *product* call
   ...
   prompt. If `topos/docs/DECISIONS-INBOX.md` does not exist, create it using the
...
`topos/CONTRACTS.md` §10 (read API envelope, sensitivity enum),
`topos/src/topos/daemon/api.py`, `topos/src/topos/daemon/client.py` (the P63 typed
methods), `topos/docs/DAEMON.md`, `topos/handoff/P67-versioned-read-http-gateway.md`
and `topos/handoff/P68-versioned-current-subscribe-client.md` (queued, not merged
— you are analyzing what they promise), `topos/docs/ROADMAP.md` §P69, and the TUI
surfaces you are proposing to mirror (`src/topos/ui/table.py`, `banner.py`,
...
- **No source changes.** Not one line under `src/topos/`. If you find a bug while
...
5. The framework section names the packaging consequence for `pip install topos`
...
```bash
git diff --check
timeout 900 env PYTHONPATH=topos/src python3 -m pytest topos/tests -q -W error   # must still be green: proves you changed no source
```

The REPORT states which environment the suite ran in and confirms the diff touches
only `topos/docs/**` and `topos/handoff/**`.
