...
**Cut:** v1.5. **Depends:** P8 merged. Branch: `feat/topos-p9-damon-control`.
Follow `topos/README.md` workflow protocol.

## Goal

The ONE mutating feature of v1.5: start/stop a topos-owned vaddr DAMON session
...
1. `damon/control.py`: allocate a FREE kdamond slot (never touch a busy one —
   ...
   ownership via a marker (topos-owned context naming/state file under
   $XDG_STATE_HOME/topos/) so topos NEVER stops a session it didn't start.
2. Lifecycle safety: TUI exit leaves the session running with a status
   ...
   `topos damon stop --all-mine` CLI to clean up; document this in the
...
4. Audit log: every start/stop appended to $XDG_STATE_HOME/topos/actions.log
...
- On the reference host as root: start on a test container, P8 columns
  populate from the topos-owned session, stop tears down cleanly, foreign
