...
Teach topos that GPU memory exists. On a host with a discrete GPU, VRAM pressure
and GPU utilization are invisible to every metric topos currently collects -- an
...
- `src/topos/collect/host.py` - `_zfs_arc_metrics()` / `_parse_arcstats()`: how a
...
- `src/topos/collect/collector.py` - `_apply_zfs_arc_rate()`: how a *derived*
...
- `src/topos/registry.py` - the `host_zfs_arc_*` block: name, unit, kind,
  ...
  ASCII).
- `src/topos/ui/banner.py` - `_zfs_arc_line()`: a conditional segment, appended
  only when the hardware is present. No new panel, no new hotkey.
- `topos/tests/test_zfs_arc.py` - the six-oracle test shape, and in particular how
...
`docs/ARCHITECTURE.md` (module map), `topos/README.md` (work-package row),
...
PYTHONPATH=topos/src python3 -m topos.cli --once --json    # must still work on this GPU-less host
