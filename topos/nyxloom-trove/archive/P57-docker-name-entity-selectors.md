...
Add `--container NAME_OR_PREFIX`, resolved via topos's existing Docker
metadata join, anywhere topos currently takes a raw cgroup-path/entity
...
docker-join module `src/topos/collect/dockerjoin.py`, which the collector
already calls every sweep (`Collector.collect_once()` →
`enrich_entities(entities, self.docker_inspect)`,
`src/topos/collect/collector.py` line 60) to populate `Entity.docker`
...
`topos squeeze --target` already exist when this is implemented, wire
...
container identity; and routine topos inspection (`inspect-files --target`,
`action --target`) already accepts "container id/name" as one of several
target shapes per their existing `--help` text (`src/topos/cli.py` lines
...
- Branch: `feat/topos-p57-docker-name-entity-selectors`
- Worktree: `.worktrees/-topos-p57-docker-name-entity-selectors`
- Touch only `topos/**`; write P57-LOG.md/P57-REPORT.md; commit, do not merge.
...
- Add a resolver function, e.g. `resolve_container_key(name_or_prefix: str,
  ...
  `dockerjoin.py`, e.g. `src/topos/collect/dockerjoin.py` or a new
  `src/topos/collect/container_resolve.py` if keeping join-population and
...
- Ambiguity/no-match handling: exact name match wins over prefix match when
  ...
  precedent (`src/topos/actions/catalog.py validate_target` rejects
...
- Wire `--container NAME_OR_PREFIX` into every CLI surface that already
  ...
  (`src/topos/cli.py` ~203/~209) and `action preview/execute --target`
...
- Composition with P56 (if merged): accept `--container NAME_OR_PREFIX` as
  an alternative to `topos squeeze --target CGROUP_PATH`, resolving before
...
- DAMON vaddr session target selection — `TUI-SPEC.md`'s DAMON default-target
  ...
  session start (`topos damon paddr start` et al.) is future work once that
