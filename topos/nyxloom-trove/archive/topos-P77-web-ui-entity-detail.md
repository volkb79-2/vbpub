---
schema_version: 1
id: topos-P77-web-ui-entity-detail
project: topos
...
stack: none
depends_on: [topos-P73-web-ui-read-only-shell]
scope:
  touch: ["topos/**"]
  forbid: []
oracles:
...
- **Worktree:** create a git worktree for branch `feat/topos-p77-web-ui-entity-detail`
  at `.worktrees/topos-p77-web-ui-entity-detail` (repo-root-relative, per
  `worktree_root` in `topos/.nyxloom/project.toml`) from `main`; do all
  implementation work there, never in the primary checkout.
- **Branch:** `feat/topos-p77-web-ui-entity-detail`
