...
`topos/tests/test_ui_app.py`. Every failure reads `Static.renderable`, an API
...
- Touch only `topos/**`
- Keep `topos/handoff/reports/P40-LOG.md` current
- Finish with `topos/handoff/reports/P40-REPORT.md` and focused commits
...
- Focused `topos/tests/test_ui_app.py` under the managed environment.
- Full `topos/tests` suite with `PYTHONPATH=topos/src`.
