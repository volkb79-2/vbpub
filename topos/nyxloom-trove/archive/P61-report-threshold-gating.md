...
Add threshold gating to `topos report`: let an operator assert bounds on the
already-computed steady-state profile figures and get a non-zero exit when a
bound is breached, so `topos report` can run as a pass/fail gate in the
...
- Branch: `feat/topos-p61-report-threshold-gating`
- Worktree: `.worktrees/topos-p61-report-threshold-gating`
- Touch only `topos/**`; write P61-LOG.md/P61-REPORT.md; commit, do not merge.

## Requirements

- Add repeatable `--assert GROUP:METRIC:STAT<=VALUE` (and `>=`) options to the
  existing `topos report` subcommand (`parse_report_args`/`_main_report` in
...
- Tests (fixture-recording based, asserting observable outcomes):
  ...
  return value.
- Update `README.md` (CLI docs / the existing `topos report` paragraph) and
...
- Run focused tests (`topos/tests/test_report.py`) and the full suite with
