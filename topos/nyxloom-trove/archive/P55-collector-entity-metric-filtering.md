...
`Collector(...)` call sites in `src/topos/cli.py` — currently lines 376, 400,
and 416 — go through `main()`'s single top-level `parse_args()`). Because of
that shared plumbing, once P55 lands, `topos --record FILE --headless` (P53,
...
- Worktree: `.worktrees/-topos-p55-collector-entity-metric-filtering`
- Touch only `topos/**`; write P55-LOG.md/P55-REPORT.md; commit, do not merge.

## Requirements

- Add `--entities GLOB` to the top-level parser in `parse_args()`
  (`src/topos/cli.py`), `action="append"`, default `None` (unset = no entity
  ...
  elsewhere in the codebase (audit `src/topos` for an existing glob helper
...
- Add `--slice NAME` as a subtree selector: shorthand for "include this
  ...
  existing target validators, e.g. `src/topos/actions/catalog.py
  validate_target` or `src/topos/inspect_files` target checks — cite and
...
- Add `--metrics compact`, a closed enum (`full` default, `compact`
  ...
  `src/topos/registry.py`'s existing metric-name/group metadata as the
...
- **Collection-time filtering, not just output pruning:** modify
  `walk_entities()` (`src/topos/collect/cgroup.py`) — or add a filtering
...
- **Ancestor resolution:** governance/drift math (`effective_memory_min`,
  origin/drift classification in `src/topos/drift/origin.py`) and slice
  ...
  `src/topos/collect/cgroup.py`), even though those ancestors themselves did
...
- Filtering already-recorded/replayed frames after the fact (a `report`
  ...
  already specifies `topos report` tolerating filtered recordings).
