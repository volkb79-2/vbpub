...
- **`src/topos/ui/app.py`** — Replaced the synchronous `action_create_snapshot()` body with an
...
```bash
git merge --no-ff feat/topos-p26-snapshot-progress-ui -m "Merge topos P26 snapshot progress UI"
```
