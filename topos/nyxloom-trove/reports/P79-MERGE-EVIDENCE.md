...
$ PYTHONPATH=topos/src /tmp/p79-venv/bin/python -m pytest topos/tests -q
...
| Defect | Consequence |
|---|---|
| Truncated `.zst` decoded to its surviving prefix | `topos report` returned a **believable, wrong** report at exit 0. On a multi-block recording a half-file leaves ~786KB of valid frames behind the cut. Worse than the traceback it replaced. |
