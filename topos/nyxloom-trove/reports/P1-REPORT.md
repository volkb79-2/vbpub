...
- Added the `topos` Python package skeleton under `topos/src/topos`, with `pyproject.toml` and console script metadata.
...
- Implemented stdlib-only CLI path: `topos --once --json --cgroup-root PATH` via `python3 -m topos.cli`.
...
- `pytest` was not installed in the system Python. For validation only, pytest was installed into `/tmp/topos-pytest` and invoked through `PYTHONPATH`; no repository or system package files were modified.
...
`PYTHONPATH=/tmp/topos-pytest:/tmp/vbpub-topos-p1-collector/topos/src python3 -m pytest topos/tests -q`
...
`PYTHONPATH=/tmp/vbpub-topos-p1-collector/topos/src python3 -m py_compile $(find topos/src/topos -name '*.py' | sort)`

```text
# no output; exit 0
```

`PYTHONPATH=/tmp/vbpub-topos-p1-collector/topos/src python3 -m topos.cli --once --json --cgroup-root topos/tests/fixtures/cgroupfs/gstammtisch`
