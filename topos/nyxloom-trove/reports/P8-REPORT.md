...
- Added passive DAMON collection in `topos/src/topos/damon/passive.py` with a read-only scanner over a configurable DAMON sysfs root (`/sys/kernel/mm/damon/admin/kdamonds` by default).
- Wired passive DAMON annotation into the collector after entity collection and before governance/diagnostics/UI use.
- Added `[damon]` config defaults in `topos.config`:
  - `hot_rate = 50.0`
  - `warm_rate = 5.0`
  - `cold_age = 30.0`
  - `idle_age = 120.0`
...
- `topos/src/topos/damon/passive.py` only uses read-side filesystem access (`Path.read_text()`, `Path.stat()`, directory iteration).
- No DAMON control paths are written.
- Source audit command produced no matches:

```bash
rg -n "write_text|\\.write\\(|open\\([^)]*,\\s*['\\\"][^'\\\"]*[wa+][^'\\\"]*['\\\"]|commit" topos/src/topos/damon/passive.py
```
...
- Implemented an additive serialized `EntityFrame.damon` metadata block in `topos.model` so replayed frames can render the DAMON drill-down panel. Existing frame fields and metric serialization remain unchanged.
- Controller review documented this additive block in `topos/CONTRACTS.md` and
  added round-trip serialization coverage.

## Validation

```bash
PYTHONPATH=/tmp/topos-pytest:/tmp/vbpub-topos-p8-damon-passive/topos/src python3 -m pytest topos/tests -q
# 63 passed in 10.17s
```

```bash
PYTHONPATH=/tmp/vbpub-topos-p8-damon-passive/topos/src python3 -m py_compile $(find topos/src/topos -name '*.py' | sort)
# clean
```

```bash
PYTHONPATH=/tmp/vbpub-topos-p8-damon-passive/topos/src python3 -m topos.cli --once --json --cgroup-root topos/tests/fixtures/cgroupfs/gstammtisch
# emitted one JSON frame; live DAMON fields degraded to unavail_perm on this host
```

```bash
PYTHONPATH=/tmp/topos-pytest:/tmp/vbpub-topos-p8-damon-passive/topos/src python3 -m topos.cli --replay topos/tests/fixtures/frames/gstammtisch-once.jsonl --step --ui-smoke
