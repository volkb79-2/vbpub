...
- **No Textual import** - uses only stdlib + topos packages below `ui/`.
...
- `topos/MEASUREMENTS.md` - added P33 smoke harness as the preferred rootless
  evidence path, with example commands and output.
- `topos/docs/OPERATIONS.md` - added release-smoke command example.

## Deviations from the Handoff

None. The handoff was followed exactly.

- Single-file `topos/src/topos/acceptance.py` with `if __name__ == "__main__"`
...
PYTHONPATH=topos/src /tmp/p25-venv/bin/python -m topos.acceptance smoke \
