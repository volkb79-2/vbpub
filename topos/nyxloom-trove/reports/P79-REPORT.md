...
Every corrupt/truncated/invalid recording input to `topos report` now produces a
...
| File | Change |
|---|---|
| `src/topos/record/reader.py` | Catch `_ZstdError` in `_open_text` (decompressor setup) and `iter_frames` (lazy decompression), converting to `ValueError` with bounded message. Wrap header `KeyError` and `frame_from_jsonable` `KeyError`/`TypeError`/`ValueError`. Guard `except _ZstdError` with `isinstance` so it does not crash when `zstandard` is not installed. |
| `src/topos/cli.py` | Add `except OSError` and `except Exception` catch-all in `_main_report` so no unexpected exception produces a raw traceback across the CLI boundary. |
...
| File | Change |
|---|---|
| `topos/README.md` | P79 row: Queued → Done with report link. |
| `topos/docs/OPERATIONS.md` | Add line: "A damaged recording produces a typed error on stderr and exits 2 — never a raw traceback." |
...
1. **Truncated recordings silently produced a partial report (exit 0).** Oracle 2's
   ...
   as a clean EOF rather than an error, so `topos report` decoded the surviving
...
- The zstd-specific oracles still `skip` when the extra is absent — you cannot
  ...
  Carved as **P84**.
- `topos --replay` and `topos snapshot inspect` share the same `RecordReader` and
