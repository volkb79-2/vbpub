...
- Branch: feat/topos-p23-zram-device-drilldown
- Worktree: .worktrees/-topos-p23-zram-device-drilldown
- Base commit: 8266fb9 docs(topos): carve P23 zram drilldown handoff
...
- Commands: git worktree add -b feat/topos-p23-zram-device-drilldown .worktrees/-topos-p23-zram-device-drilldown main
...
- Files changed: topos/src/topos/ui/hostmem.py
...
- Files changed: topos/tests/test_p23_zram_drilldown.py
...
- Commands: pytest topos/tests -q, py_compile, topos --once --json
...
- Commands: focused pytest, full pytest, py_compile, compact `topos --once --json` smoke.
...
- Commands: `git merge --no-ff feat/topos-p23-zram-device-drilldown -m "Merge topos P23 zram device drilldown"`, py_compile, full pytest, compact fixture JSON smoke.
- Result: Merge succeeded; `topos/tests` passed with 161 tests; fixture smoke reported schema 1, `host_meta=["zram_devices"]`, 0 zram devices, and 8 entities.
```
...
# Python compile
python3 -m py_compile topos/src/topos/model.py
python3 -m py_compile topos/src/topos/collect/host.py
python3 -m py_compile topos/src/topos/collect/collector.py
python3 -m py_compile topos/src/topos/ui/hostmem.py
python3 -m py_compile topos/tests/test_p23_zram_drilldown.py
