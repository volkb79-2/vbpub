...
| Branch | `feat/topos-p55-collector-entity-metric-filtering` |
| Status | **Done** |
| Base | main |
...
```bash
PYTHONPATH=topos/src python -m pytest topos/tests/test_p55_filtering.py -q
# 32 passed in 0.36s

PYTHONPATH=topos/src python -m pytest topos/tests/test_collector.py -q
# 6 passed in 0.24s

PYTHONPATH=topos/src python -m py_compile topos/src/topos/collect/cgroup.py topos/src/topos/registry.py topos/src/topos/collect/collector.py topos/src/topos/cli.py
# All compile OK

# CLI smoke: topos --once --json --slice system.slice --metrics compact exits 0
PYTHONPATH=topos/src python -c "
...
| File | Change |
|---|---|
| `topos/src/topos/registry.py` | Added `METRIC_GROUPS`, `COMPACT_GROUPS` for `--metrics compact` |
| `topos/src/topos/collect/cgroup.py` | Added `_validate_slice_name`, `build_entity_predicate`, `add_entity_ancestors` |
| `topos/src/topos/collect/collector.py` | Added `entities_globs`, `slice_names`, `metrics_mode` params to `Collector`; entity/metric filtering in `collect_once()` |
| `topos/src/topos/cli.py` | Added `--entities`, `--slice`, `--metrics` args; `_filter_kwargs` helper; validation in `main()`; wiring into 3 `Collector()` calls |
| `topos/tests/test_p55_filtering.py` | 32 new tests covering all filtering behaviors |
| `topos/README.md` | Updated CLI quickstart |
| `topos/CONTRACTS.md` | Added filtered-recording note to §5 |
| `topos/docs/ROADMAP.md` | Marked P55 done |
| `topos/docs/STATUS.md` | Moved P55 to Implemented |
| `topos/handoff/reports/P55-LOG.md` | Work log |
| `topos/handoff/reports/P55-REPORT.md` | This report |
