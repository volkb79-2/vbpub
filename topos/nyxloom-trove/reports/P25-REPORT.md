...
- Added install-plan data model and helper to `topos/src/topos/daemon/deploy.py`:
...
- Added CLI subcommand `topos daemon install-plan` to `topos/src/topos/cli.py`:
...
```bash
python3 -m py_compile topos/src/topos/daemon/deploy.py topos/src/topos/cli.py topos/tests/test_daemon_deploy.py
# (no output — clean)

python3 -m pytest topos/tests/test_daemon_deploy.py -v
# 11 passed in 1.60s

python3 -m pytest topos/tests -q
# 177 passed in 27.01s after merge

PYTHONPATH=topos/src /tmp/vbpub-topos-p17-venv/bin/python -m topos.cli daemon install-plan
# topos daemon install plan
# ============================================================
# socket path : /run/topos/topos.sock
# daemon group : topos
# service unit : /etc/systemd/system/topos.service
# tmpfiles conf: /etc/tmpfiles.d/topos.conf
# --- plan steps (read-only; no host mutation) ---
# Step 1: Create the system group 'topos' ...
# ... (7 steps, warnings, PLAN disclaimer)
# This is a PLAN only. No files were written and no system state was changed.

PYTHONPATH=topos/src /tmp/vbpub-topos-p17-venv/bin/python -m topos.cli daemon install-plan --json > /dev/null
# exit 0; valid JSON with plan, steps, warnings, service_content, tmpfiles_content

PYTHONPATH=topos/src /tmp/vbpub-topos-p17-venv/bin/python -m topos.cli daemon install-plan --group custom --socket /tmp/custom/topos.sock --json
# rendered service_content uses Group=custom and --socket /tmp/custom/topos.sock;
...
- Step 2 (`usermod -aG topos <username>`) uses a placeholder; no user-discovery or batch-add mechanism is provided.
...
- Custom socket paths outside `/run/topos` are rendered into the service and
...
```bash
git merge --no-ff feat/topos-p25-daemon-install-plan -m "Merge topos P25 daemon install plan"
```

Post-merge validation from the main checkout: py_compile clean,
`topos/tests/test_daemon_deploy.py` passed with 11 tests, full suite
`177 passed in 27.01s`, and `topos daemon install-plan --json` parsed with
`json.tool`.

## Controller Merge Review

- Feature commit(s) on `feat/topos-p25-daemon-install-plan`.
- Pre-merge validation:
  - `python3 -m pytest topos/tests/test_daemon_deploy.py -v` -> `11 passed in 1.60s`
  - `python3 -m pytest topos/tests -q` -> `177 passed in 27.93s`
  - `python3 -m py_compile topos/src/topos/daemon/deploy.py topos/src/topos/cli.py topos/tests/test_daemon_deploy.py` -> clean
  - `python3 -m topos.cli daemon install-plan --json | python3 -m json.tool > /dev/null` -> exit 0, valid JSON
