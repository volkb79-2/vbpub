...
- Made execution audit mandatory. The CLI has no `--audit-log`; it uses the
  fixed absolute default `/var/log/topos/actions.jsonl`. An absolute injected
...
`topos/tests/test_actions.py` now has 129 passing tests. The correction tests
...
- The fixed production audit path must be provisioned root-owned and mode
  `0600` (the execution code creates `/var/log/topos` privately when running
...
| File | Change |
|---|---|
| `topos/src/topos/actions/catalog.py` | Fixed absolute executables and shared strict target validator |
| `topos/src/topos/actions/execute.py` | Root-gated execution, secure mandatory audit, bounded runner, typed failure handling, immutable-plan revalidation |
| `topos/src/topos/actions/preview.py` | Uses the shared catalog validator |
| `topos/src/topos/actions/__init__.py` | Exports corrected execution contract symbols |
| `topos/src/topos/cli.py` | Removes arbitrary execute audit-path option |
| `topos/tests/test_actions.py` | Updates absolute-argv expectations and adds adversarial correction coverage |
| `topos/README.md` and `topos/docs/*` | Corrected operational and readiness claims |
| `topos/handoff/reports/P46-LOG.md` | Resumability log for the correction |
