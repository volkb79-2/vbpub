...
`topos report --window auto` now selects the longest stable trailing suffix of
...
- `src/topos/report.py`: pure detector, resolved-window/result types, safe
  option validation, auto metadata serialization, and additive load helper.
- `src/topos/cli.py`: auto-window and stability option parsing; resolved
...
additive only to `topos report --window auto` output.
...
its own deprecation warning fail all tests before topos code executes. With the
...
- Full-suite failure with the global Schemathesis plugin under `-W error` is an
  existing environment dependency warning, not a topos test failure.
