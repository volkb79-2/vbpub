...
- Base commit: 80f5583 docs(topos): carve P30 daemon default client
- Package: P30 Daemon default client UX (v1.5/v2 daemon usability)
- Current objective: Default-socket attach and topos daemon current command

## Timeline

```text
2026-07-09 UTC
- Action: Created git worktree on branch feat/topos-p30-daemon-default-client from main
- Commands: git worktree add -b feat/topos-p30-daemon-default-client .worktrees/-topos-p30-daemon-default-client main
- Result: Worktree ready at 80f5583

- Action: Set up venv, verified 261 tests pass on main
- Commands: python3 -m venv /tmp/vbpub-topos-p30-venv && pip install -e topos/ pytest
...
- Result: --attach (bare) defaults to /run/topos/topos.sock; --attach /path works as before; no --attach means None

- Action: Added daemon current subcommand to parse_daemon_args and _main_daemon
- Files changed: topos/src/topos/cli.py
- Result: topos daemon current --socket PATH [--pretty-json] prints canonical frame JSON
...
- Files changed: topos/tests/test_daemon_deploy.py
...
- Files changed: topos/src/topos/cli.py, topos/tests/test_attach_cli.py,
  topos/tests/test_daemon_deploy.py, topos/docs/DAEMON.md,
  topos/handoff/reports/P30-LOG.md, topos/handoff/reports/P30-REPORT.md
- Result: Help text now shows the concrete default socket path, `topos daemon
...
```bash
git merge --no-ff feat/topos-p30-daemon-default-client
