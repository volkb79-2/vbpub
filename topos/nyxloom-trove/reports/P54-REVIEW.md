...
- Focused: `PYTHONPATH=topos/src python3 -m pytest topos/tests/test_report.py -q -W error::RuntimeWarning` → `57 passed`.
- Full suite: `PYTHONPATH=topos/src timeout 300 python3 -m pytest topos/tests/ -q -p no:asyncio -p no:schemathesis -W error` → `971 passed, 2 skipped in 122.60s` (the 2 skips require the `zstandard` extra, absent in this env — same baseline as P53).
...
- Reuses `RecordReader`/parent-chain ancestry — no new file parsing, no cgroup-path reimplementation. Scope: all 8 files under `topos/**`.
