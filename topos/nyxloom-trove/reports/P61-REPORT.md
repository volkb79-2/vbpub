...
Added threshold gating to `topos report`: the new repeatable `--assert` flag
...
`topos report` to run as a pass/fail gate in CI-style scripts and the
...
| File | Change |
|------|--------|
| `topos/src/topos/report.py` | Added `Assertion`, `AssertionResult` dataclasses; `parse_assert_spec` regex parser; `evaluate_assertions` pure function; `assertion_result_to_jsonable`; `_find_profile_metric` helper. Updated `report_to_jsonable` and `format_report` to accept optional assertions list. Updated module docstring. |
| `topos/src/topos/cli.py` | Added `--assert` (action="append", dest="assert_specs") to `parse_report_args`. Wired parsing and evaluation into `_main_report` with exit codes: 0 = all pass, 1 = breach, 2 = malformed/unknown. |
| `topos/tests/test_report.py` | Added `TestParseAssertSpec` (12 tests), `TestEvaluateAssertions` (11 tests), `TestReportAssertionCLI` (11 tests). Updated imports. |
| `topos/README.md` | Added `--assert` documentation and examples to the `topos report` paragraph. |
| `topos/docs/OPERATIONS.md` | Added threshold-gating examples to the report command section. |
| `topos/handoff/reports/P61-LOG.md` | New — work log. |
...
```
PYTHONPATH=topos/src python3 -m pytest topos/tests/test_report.py -v --tb=short
→ 91 passed in 2.96s
```
...
```
timeout 300 python3 -m pytest topos/tests/ -q --tb=short
...
```
python3 -m py_compile topos/src/topos/report.py    # OK
python3 -m py_compile topos/src/topos/cli.py        # OK
```
