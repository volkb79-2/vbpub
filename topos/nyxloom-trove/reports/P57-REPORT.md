...
### `resolve_container_key()` in `topos/src/topos/collect/dockerjoin.py`
...
Neither P55 (`--entities`/`--slice`) nor P56 (`topos squeeze --target`) are
...
# 2 passed (mutual-exclusion tests)
$ python3 -m pytest topos/tests -q --ignore=topos/tests/test_ui_app.py \
  --ignore=topos/tests/test_damon_paddr.py \
  --ignore=topos/tests/test_damon_passive.py \
...
- TUI-side container-name jump/search is explicitly out of scope (handoff §Out
  Of Scope).
- `--container` on `topos squeeze --target` (P56) and `--entities`/`--slice`
