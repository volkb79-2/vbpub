...
```bash
timeout 900 env PYTHONPATH=topos/src /workspaces/vbpub/.venv/bin/python \
...
```bash
timeout 900 env PYTHONPATH=topos/src /tmp/p43-clean-venv/bin/python \
  -m pytest topos/tests -q -W error
...
to `topos/docs/**` and `topos/handoff/reports/**`.
