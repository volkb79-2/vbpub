...
project: topos
title: "Close query engine and semantic coverage gaps"
tier: sonnet5-high
...
oracles:
  - id: O1
    observable: "src/topos/query/engine.py and src/topos/query/semantics.py each have empty missing_lines and missing_branches in branch-aware JSON from the complete xdist gate"
    negative: "completion is claimed from aggregate, serial, focused, rounded, or partial target coverage"
    gate: topos-suite
  - id: O2
    observable: "tests drive real Query, FrameSource, frame, selector, window, semantic, projection, ordering, and cap inputs and assert exact rows, cells, summaries, metadata, errors, ordering, resets, gaps, and truncation"
...
escalate_if:
  ...
  - "a real defect requires source outside the two named query modules"
  - "either clean full-gate run fails or target executed/missing sets differ"
advances: []
---

# P101 — Close query engine and semantic coverage gaps

## Goal

Bring `query/engine.py` and `query/semantics.py` to exact 100% statement and
branch coverage in the full xdist gate. Preserve every previously exact target.
...
5. `topos/nyxloom-trove/reports/P96-COVERAGE-GAPS.md`, only:
   - `query/engine.py`: 39 missing lines, 39 missing branch pairs.
   - `query/semantics.py`: 12 missing lines, 12 missing branch pairs.
6. `topos/tests/test_query.py` and the two target modules.
...
2. Run the full branch-aware gate once and mechanically print the current gap
   slice for both targets.
3. For each residual, bind the JSON line/arc to `nl -ba` output from this
   exact commit, then record the Query/FrameSource/domain input selecting it
   and the exact public result or typed error that changes.
...
5. Exercise validation, selector ancestry, tree projection, semantic
   aggregation, reset/gap handling, visibility, ordering, row/point/byte caps,
   and empty/degraded inputs without host state or time dependence.
...
8. Run the exact gate twice, require `2/2 exact`, and compare target
