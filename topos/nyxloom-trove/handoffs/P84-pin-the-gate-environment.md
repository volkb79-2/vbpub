...
`topos report` on a corrupt `.jsonl.zst` is only *reachable* when the optional
...
5. **No behavior change to `topos` itself.** This package touches packaging, test
   config, and docs. If it needs a source change, say why in the REPORT first.

## Acceptance Oracles (numbered, adversarial)

1. From a venv built by the documented procedure, `pytest topos/tests -q` runs the
...
4. `zstandard` is still not a hard runtime dependency: `pip install topos` without
   extras still imports and runs `topos report` on a plain `.jsonl`.
...
`topos/README.md` (how to build the gate env), `docs/STATUS.md`.
