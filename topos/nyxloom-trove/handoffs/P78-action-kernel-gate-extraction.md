...
> **Session-hint:** fresh
> **Serialize-with:** none   <!-- but it OWNS src/topos/actions/ for its duration: do not run any other actions-area package concurrently -->
...
`src/topos/actions/execute.py` now contains **four** near-identical executors —
...
- `src/topos/actions/execute.py` — all four executors, end to end. The differences
  between them are the specification of this package; enumerate them before writing
  anything.
- `src/topos/actions/kill_ops.py`, `update_ops.py`, `governance.py` — the per-verb
...
- `topos/CONTRACTS.md` standing contracts.
...
```bash
PYTHONPATH=topos/src python3 -m pytest topos/tests/test_actions.py topos/tests/test_p72_kill_update.py -q -W error
...
path `topos/src` and fail spuriously otherwise. State in the REPORT which environment
