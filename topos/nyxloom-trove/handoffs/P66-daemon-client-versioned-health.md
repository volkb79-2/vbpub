...
- Touch only `topos/**`; write P66-LOG.md/P66-REPORT.md; commit, do not merge.

## Context To Read First (bounded)

`topos/README.md`, this handoff, `topos/src/topos/daemon/client.py` (P63's
`_request_envelope`, `request_current`, and result dataclasses — mirror them),
`topos/src/topos/daemon/api.py` (`_op_health` only — the result shape you
decode), `topos/src/topos/daemon/component_health.py` (existing health models
and `HealthSnapshot` — decode into the established model where it already fits,
do not re-derive), and `topos/tests/test_daemon_client_p63.py` for the test
...
- Export the new result type through `topos/src/topos/daemon/__init__.py`.
...
```bash
PYTHONPATH=topos/src python3 -m pytest <focused P66 tests> -q -W error -p no:schemathesis
