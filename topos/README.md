# topos — host pressure inspector and cgroup forensics TUI
...
Stack: Python; Textual allowed ONLY under `src/topos/ui/` (spec §6.1, §6.4).
...
- `docs/OPERATIONS.md` — runbook for using topos safely today.
...
- `docs/RELEASE-READINESS.md` — release-candidate readiness surface mapping
  ...
  explicit non-claims.
- `pwmcp/README.md` — pinned Topos-owned browser-test service, CIU lifecycle and
...
topos report topos/tests/fixtures/frames/gstammtisch-once.jsonl
topos report topos/tests/fixtures/frames/gstammtisch-once.jsonl --json --window last:60s --group-by slice
topos report recording.jsonl --json --window auto
topos query recording.jsonl --shape summary --metric ram --metric psi_mem_some_avg10 --sort ram:p95:desc
topos query recording.jsonl --shape raw --metric io_r_bps --window last:60s --json
topos squeeze --target /sys/fs/cgroup/system.slice/app.service --admin --confirm SQUEEZE
...
Use `--container NAME_OR_PREFIX` on `topos inspect-files plan/read --target` or
`topos action preview/execute --target` to resolve a Docker container name or
prefix to its cgroup path automatically instead of manually specifying
`--target`.

For AI CLI access to a running local daemon, install `topos[mcp]` and register
the stdio frontend with `claude mcp add topos -- topos mcp serve`. The four
read-only tools have stated item limits and an enforced 4 MiB aggregate result
cap; use `topos mcp serve --redact-above operational` when sensitive metric
values should be replaced by typed redaction markers.

Use `topos report FILE` to compute a steady-state profile from a P2-format
...
By default `topos report` and `topos query` print a deterministic, pipe-safe
...
::
    ...
    topos report recording.jsonl --json --assert ':ram:max<=4294967296'
...
```bash
# From the repo root:
pip install -e 'topos[dev]'
```

That installs ``topos``, its runtime dependency ``textual``, the ``zstandard``
...
Neither extra becomes a hard runtime dependency: ``pip install topos`` without
extras still imports and runs ``topos report`` on a plain ``.jsonl`` file.
...
| P20 | Done | Daemon attach mode for non-root clients | v1.5/v2 foundation | Consume P16 daemon frames via `topos --attach SOCKET`; `--once --json` and UI smoke are supported. Report: `handoff/reports/P20-REPORT.md`. |
...
| P32 | Done | Daemon status command | v1.5/v2 daemon usability | Add read-only `topos daemon status` combining deployment preflight and current-frame protocol checks. Handoff: `handoff/P32-daemon-status-command.md`. Report: `handoff/reports/P32-REPORT.md`. |
| P33 | Done | Release smoke harness | v1/v1.5 release confidence | Add rootless `python -m topos.acceptance smoke` for deterministic safe-path evidence. Handoff: `handoff/P33-release-smoke-harness.md`. Report: `handoff/reports/P33-REPORT.md`. |
...
| P35 | Done | Acceptance steady harness | v1/v1.5 release confidence | Adds rootless multi-sample collector CPU/RSS evidence via `python -m topos.acceptance steady`; live Textual TUI measurement remains separate release evidence. Handoff: `handoff/P35-acceptance-steady-harness.md`. Report: `handoff/reports/P35-REPORT.md`. |
...
| P38 | Done | TUI smoke evidence harness | v1/v1.5 release confidence | Adds rootless `python -m topos.acceptance tui-smoke` evidence over the existing Textual `--ui-smoke` path, with child wall/CPU/RSS measurements and import-contract coverage. Handoff: `handoff/P38-tui-smoke-evidence.md`. Report: `handoff/reports/P38-REPORT.md`. |
...
| P46 | Done | Admin action execution kernel | v2 actions | Execute only validated Docker/systemd start/stop/restart plans behind root, admin, typed-confirmation, strict timeout/argv, and mandatory fail-closed audit gates. Production audit is fixed at `/var/log/topos/actions.jsonl`; API-only fixture paths and injected runner/clock/identity seams are test-only. Handoff: `handoff/P46-admin-action-execution-kernel.md`. Report: `handoff/reports/P46-REPORT.md`. |
| P47 | Done | Daemon component health | v2 daemon | Thread-safe bounded collector/BPF/paddr health, strict `health-v1` protocol validation, and `topos daemon health [--json]`. Handoff: `handoff/P47-daemon-component-health.md`. Report: `handoff/reports/P47-REPORT.md`. |
...
| P54 | **Done** | Steady-state report command | v1.5 recording | Add `topos report FILE [--window last:Ns\|all] [--group-by slice\|entity] --json` computing per-entity p50/p95/max for key gauges and deriving `_per_s` rates from embedded raw counters when the recorded live rate is `None`. Report: `handoff/reports/P54-REPORT.md`. |
...
| P56 | **Done** | `topos squeeze` guided memory measurement | v2 actions | Add a guided, stepped `memory.high` squeeze that measures a cgroup's hot working set, with mandatory memory.high restore on exit/SIGINT and a topos-record-compatible JSONL log. Handoff: `handoff/P56-topos-squeeze.md`. Report: `handoff/reports/P56-REPORT.md`. |
| P57 | Done | Docker-name entity selectors | v1.5/v2 ergonomics | Add `--container NAME_OR_PREFIX`, resolved via the existing docker metadata join, wherever topos takes a cgroup-path/entity identifier. Handoff: `handoff/P57-docker-name-entity-selectors.md`. Report: `handoff/reports/P57-REPORT.md`. |
| P58 | Done | Daemon MCP frontend | v2/v3 API | `topos mcp serve` is an optional stdio MCP frontend over the P52 API via P63's typed client. It exposes exactly health, bounded overview/entity/history reads, P57 docker selectors, sensitivity redaction, and an enforced 4 MiB response cap. Handoff: `handoff/P58-daemon-mcp-frontend.md`. Report: `handoff/reports/P58-REPORT.md`. |
...
| P61 | Done | Steady-state report threshold gating | v1.5 recording | Add repeatable `--assert GROUP:METRIC:STAT<=VALUE` to `topos report` (exit 1 on breach), evaluated over the already-computed P54 profile without recomputing it; absent group/metric and null STAT are breaches. Handoff: `handoff/P61-report-threshold-gating.md`. Report: `handoff/reports/P61-REPORT.md`. |
| P62 | Done | Steady-state window auto-detection | v1.5 recording | Add `--window auto` to `topos report`: select the longest trailing window whose primary gauge coefficient-of-variation is within a pinned bound, then profile it via the existing P54 math. Serialize-with P61. Handoff: `handoff/P62-report-steady-state-autodetect.md`. Report: `handoff/reports/P62-REPORT.md`. |
...
| P65 | Done | Human-readable query/report rendering | operator-console tooling | `src/topos/render.py`: a pure renderer over the already-computed P88 `Result.to_jsonable()` / `report_to_jsonable()` dicts — `topos query`/`topos report` emit a deterministic ASCII table by default now, with `--json` kept for the machine contract (`--json`/`--table` mutually exclusive, exit 2 on conflict). Closed vocabulary (`missing`/`redacted`/`warming`/`stale`/`permission-denied`/`truncated`) covers every typed value state; zero always renders as `0`; hierarchy output preserves ancestry/sibling order and `flat` is always labelled explicitly. Handoff: `handoff/topos-P65-report-human-readable-render.md`. Report: `handoff/reports/P65-REPORT.md`. |
...
| P88 | Done | Unified frame query core | operator-console core | `src/topos/query/`: one typed `FrameSource` over recordings and P63 daemon history (zero P2/P52 wire change), strict `Query` object, six declared value semantics reusing P54/P70 math with differential oracles, registry-policy subtree aggregation, coverage/gap/eviction/reset truth in every result, pre-materialization row/point/byte bounds, byte-identical payloads across sources, and the `topos query --json` CLI. Handoff: `handoff/P88-unified-frame-query-core.md`. Report: `handoff/reports/P88-REPORT.md`. |
| P89 | Ready (P88 merged) | Visible source auto-selection/backfill | operator-console core | Prefer daemon, visibly fall back to local, and immediately backfill available history. Handoff: `handoff/P89-source-auto-backfill.md`. |
| P90 | Ready (P88 merged) | CPU-hot + I/O-hot process projection | operator-console core | Identity-safe bounded process union with `pidstat`-class history and ownership. Handoff: `handoff/P90-bounded-process-sampler.md`. |
| P91 | Ready (P88 merged) | Persistent capped history | operator-console core | Five-minute RAM tier plus recoverable 24-hour/256-MiB simultaneous disk caps. Handoff: `handoff/P91-persistent-capped-history.md`. |
| P92 | Blocked on P66/P81/P88/P91 | Loopback web transport | operator-console web security | Per-start capability token, same-origin bounded routes and Topos-owned PWMCP fixture. Handoff: `handoff/P92-loopback-web-transport.md`. |
| P93 | Ready (P87 merged) | Lifecycle owner-chain protocol | lifecycle safety | Side-effect-free discovery/plan, centralized authorization, no raw-runtime fallback and migration of existing actions. Handoff: `handoff/P93-lifecycle-owner-protocol.md`. |
...
- **Worktree + branch**: work in a dedicated git worktree on a feature branch
  named `feat/topos-<pkg>-<slug>`, e.g.
  `git worktree add -b feat/topos-p1-collector .worktrees/-topos-p1-collector main`.
  The worktree MUST live under the repo-root `.worktrees/` directory, using a
  path like `.worktrees/-topos-<pkg>-<slug>`, and MUST branch from local
  `main`. `.worktrees/` is gitignored; do not edit or commit package work
  directly in the main checkout.
- **Scope**: touch only `topos/**`. No edits to other vbpub areas, no host
...
- **Quality gates before handover**: `python3 -m pytest topos/tests -q` green;
  `python3 -m py_compile` clean on all new files; `topos --once --json`
  (or the package's own entry point) demonstrably runs.
- **Engineering bar**: keep package code modern, typed where it clarifies
  contracts, and DRY. Shared behavior belongs in `src/topos/` helpers, not in
...
- **Handover**: finish with (a) focused commits on the feature branch, the
  ...
  `topos/handoff/reports/<PKG>-REPORT.md` containing: what was built, deviations
...
- **Resumability log**: every package must also keep
  `topos/handoff/reports/<PKG>-LOG.md` updated while working. Use
