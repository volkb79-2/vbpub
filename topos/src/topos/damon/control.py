...
from topos.collect.cgroup import parse_int_text, read_text
...
def confirmation_text(plan: DamonStartPlan) -> str:
    ...
    return (
        ...
        "The session is topos-owned and will keep running after the viewer exits.\n"
...
def plan_start_session(
    ...
    *,
    ...
    if len(owned) >= config.max_concurrent_targets:
        raise NoFreeKdamond("topos-owned DAMON target limit reached")
...
def start_planned_session(
    ...
    *,
    ...
    if marker.exists():
        raise OwnershipError(f"kdamond {plan.kdamond_idx} already has a topos marker")
    ...
    *,
    ...
    for marker in markers:
        ...
        if payload.get("owner") != "topos":
            raise OwnershipError(f"refusing non-topos marker {marker}")
...
def owned_markers(state_dir: Path) -> list[Path]:
    """Return sorted topos-owned marker paths under *state_dir*.
