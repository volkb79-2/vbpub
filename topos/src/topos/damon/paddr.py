...
def paddr_confirmation_text(plan: PaddrStartPlan) -> str:
    ...
    return (
        ...
        "The session is topos-owned, host-wide, and will keep running after the viewer exits.\n"
...
def plan_start_paddr_session(
    *,
    ...
    _refuse_existing_topos_paddr(root, damon_root)
...
def _refuse_existing_topos_paddr(state_dir: Path, damon_root: Path) -> None:
    for marker in _owned_markers(state_dir):
        ...
        if payload.get("mode") == "paddr" and Path(str(payload.get("damon_root", damon_root))) == damon_root:
            raise NoFreeKdamond("topos-owned paddr DAMON session already exists")
