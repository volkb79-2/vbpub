...
from topos.collect.cgroup import _validate_slice_name
from topos.collect.dockerjoin import ContainerResolveError, resolve_container_key
from topos.config import BpfSnapshotConfig, load
from topos.damon.control import APPROVAL_TEXT, DamonControlError, RootRequired, stop_owned_sessions
from topos.damon.passive import DEFAULT_DAMON_ROOT
from topos.damon.paddr import paddr_confirmation_text, plan_start_paddr_session, start_planned_paddr_session
from topos.daemon.client import DaemonClientError, DaemonConnectError, DaemonProtocolError, DaemonResponseError, current_frame
...
def parse_damon_args(argv: list[str]) -> argparse.Namespace:
    ...
    stop_parser.add_argument("--state-dir", type=Path, default=None, help="topos state dir containing DAMON ownership markers")
    ...
    paddr_start = paddr_subparsers.add_parser("start", help="start a topos-owned host paddr session")
    paddr_start.add_argument("--damon-root", type=Path, default=DEFAULT_DAMON_ROOT, help="DAMON kdamonds sysfs root")
    paddr_start.add_argument("--state-dir", type=Path, default=None, help="topos state dir containing DAMON ownership markers")
...
def parse_daemon_args(argv: list[str]) -> argparse.Namespace:
    ...
    serve.add_argument("--bpf-state-dir", type=Path, default=None, help="BPF snapshot output directory (default: /run/topos/bpf)")
...
def parse_bpf_args(argv: list[str]) -> argparse.Namespace:
    ...
    gate.add_argument("--pin-root", type=Path, default=Path("/sys/fs/bpf/topos"), help="expected topos BPF pin root")
...
def _format_daemon_error(exc: DaemonClientError, socket_path: Path) -> str:
    ...
    if isinstance(exc, DaemonConnectError):
        ...
        if socket_path == DEFAULT_DAEMON_SOCKET:
            lines.append("Try: topos daemon preflight")
            lines.append("If the daemon is not installed: topos daemon install-plan")
        else:
            lines.append(f"Try: topos daemon preflight --socket {socket_path}")
    elif isinstance(exc, (DaemonProtocolError, DaemonResponseError)):
        lines.append("")
        lines.append("Check that the process at the socket is a compatible topos daemon")
...
def _run_ui(
    ...
    *,
    ...
    try:
        from topos.ui.app import run_ui
...
def _default_squeeze_log_path(target: str) -> Path:
    """Build a default squeeze log path under ``/var/log/topos/squeeze/``.

    Uses a timestamp and the target cgroup path's last component for a
    descriptive filename, following the convention of topos audit log
    locations (``/var/log/topos/``).
    """
    import datetime
    ...
    return Path("/var/log/topos/squeeze") / f"squeeze-{leaf}-{ts}.jsonl"


def parse_squeeze_args(argv: list[str]) -> argparse.Namespace:
    """Parse arguments for ``topos squeeze``.
    ...
    parser.add_argument("--audit-path", type=Path, default=Path("/var/log/topos/actions.jsonl"), help="audit log path")
    return parser.parse_args(argv)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.once or not args.json:
        print("topos implements --once --json for live collection and --replay for frame playback", file=sys.stderr)
        return 2
    try:
...
def _main_damon(argv: list[str]) -> int:
    ...
    if args.command == "stop":
        if not args.all_mine:
            print("topos damon stop requires --all-mine", file=sys.stderr)
            return 2
        try:
            ...
            print(str(exc), file=sys.stderr)
            return 1
        print(f"stopped {stopped} topos-owned DAMON session(s)")
        return 0
    if args.command == "paddr" and args.paddr_command == "start":
        ...
        try:
            ...
            print(str(exc), file=sys.stderr)
            return 1
        print(f"started topos-owned paddr DAMON session on kdamond {session.kdamond_idx}")
        return 0
...
def _main_action(argv: list[str]) -> int:
    from topos.actions.audit import AuditLog
    from topos.actions.execute import ExecuteResult, execute_plan
    ...
    from topos.actions.preview import ActionPlan, DisabledPlan, build_admin_preview
    ...
    if args.command == "execute":
        ...
        if args.json:
            from topos.actions.execute import result_to_jsonable
            print(json.dumps(result_to_jsonable(result), sort_keys=True))
        else:
            from topos.actions.execute import render_result_text
...
def parse_report_args(argv: list[str]) -> argparse.Namespace:
    """Parse topos report [--window last:Ns|all|auto] [--group-by slice|entity] FILE.
...
def _main_report(argv: list[str]) -> int:
    """Implement topos report — load recording, compute profile, emit JSON."""
    try:
        args = parse_report_args(argv)
    except SystemExit as exc:
        return int(str(exc.code)) if exc.code is not None else 2

    from topos.render import render_report
...
def parse_query_args(argv: list[str]) -> argparse.Namespace:
    """Parse ``topos query FILE`` — the executable surface over the P88 engine.
...
def _main_query(argv: list[str]) -> int:
    """Implement topos query — load a recording, run the P88 engine, emit a result."""
    try:
        args = parse_query_args(argv)
    except SystemExit as exc:
        return int(str(exc.code)) if exc.code is not None else 2

    from topos.render import render_query
...
def parse_compare_args(argv: list[str]) -> argparse.Namespace:
    """Parse ``topos compare CURRENT BASELINE`` — informational P88 summary diff (P64).

    Reads two ``topos query --shape summary --json`` result files and computes
...
def _main_compare(argv: list[str]) -> int:
    """Implement topos compare — diff two P88 summary results (P64, informational)."""
    try:
        args = parse_compare_args(argv)
    except SystemExit as exc:
        return int(str(exc.code)) if exc.code is not None else 2

    from topos.compare import (
        CompareError,
        compare_exit_code,
        compare_summaries,
...
def _resolve_mutual_exclusive_target(target: str | None, container: str | None, command: str) -> str:
    ...
    .. note::
        ...
        **P56 composition:** If/when P56 (``topos squeeze --target``) is
...
def _main_mcp(argv: list[str]) -> int:
    ...
    from topos.mcp.server import run_server
...
def _main_gateway(argv: list[str]) -> int:
    ...
    print(f"serving authenticated topos reads on http://{host}:{port}", flush=True)
    try:
...
def _main_daemon(argv: list[str]) -> int:
    ...
    if args.command == "serve":
        ...
        print(f"serving read-only topos frames on {args.socket}", flush=True)
        ...
        if bpf_enabled and bpf_root is not None:
            ...
            try:
                map_name = config.bpf_snapshot.map_name if not args.bpf_root else "topos_cgroup_skb"
                state_dir: Path = (
                    args.bpf_state_dir
                ...
                from topos.providers.net_bpf import BpfProvider as BpfProv
