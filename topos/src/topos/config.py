...
class BpfSnapshotConfig:
    ...
    enabled: bool = False
    root: Path | None = None  # BPF pin root, e.g. /sys/fs/bpf/topos
    interval: float = 30.0  # refresh interval in seconds
    map_name: str = "topos_cgroup_skb"  # pinned map name under root
    state_dir: Path = Path("/run/topos/bpf")  # directory for snapshot.json output
...
def _default_path() -> Path:
    ...
    return (Path(base) if base else Path.home() / ".config") / "topos" / "config.toml"
...
def load(path: Path | None = None) -> ToposConfig:
    data: dict[str, Any] = {}
    try:
    ...
    return ToposConfig(
        ...
        bpf_snapshot=BpfSnapshotConfig(
            ...
            map_name=str(bpf_snapshot_data.get("map_name", "topos_cgroup_skb")),
