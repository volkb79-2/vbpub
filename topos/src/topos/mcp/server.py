...
from topos.daemon.api import DEFAULT_MAX_RESPONSE_BYTES, Sensitivity
from topos.daemon.redaction import PayloadShape, classify_metric
...
from topos.daemon.component_health import HealthSnapshot
from topos.daemon.deploy import DEFAULT_DAEMON_SOCKET
from topos.model import Entity, EntityFrame, EntityKey
...
class McpServer:
    ...
    def run(self) -> int:
        ...
        try:
            ...
            if exc.name and exc.name.startswith("mcp"):
                print(
                    "topos mcp serve requires the topos[mcp] extra; install it with: pip install 'topos[mcp]'",
                    file=sys.stderr,
                )
                return 2
            raise
        ...
        if _is_error(startup):
            print("topos mcp serve could not reach a compatible daemon", file=sys.stderr)
            return 1
    ...
    def build_mcp_server(self, fast_mcp: Any) -> Any:
        ...
        def topos_health() -> dict[str, object]:
        ...
        def topos_overview(sort_by: str = "ram", limit: object = 20) -> dict[str, object]:
        ...
        def topos_entity(selector: str) -> dict[str, object]:
