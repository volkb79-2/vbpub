...
def default_snapshot_dir() -> Path:
    ...
    return (Path(base) if base else Path.home() / ".local" / "state") / "topos" / "incidents"
...
def create(
    ...
    *,
    ...
    bundle_path = _unique_bundle_path(out_dir, f"topos-incident-{int(ts)}-{slug}", suffix)
    with tempfile.TemporaryDirectory(prefix="topos-snapshot-") as tmp:
        root = Path(tmp)
...
def inspect_bundle(path: Path) -> str:
    with tempfile.TemporaryDirectory(prefix="topos-inspect-") as tmp:
        root = Path(tmp)
...
def _write_frames(path: Path, frames: list[Frame], config: ToposConfig) -> None:
...
def _manifest(root: Path, *, entity_key: str, frame: Frame, config: ToposConfig, privacy_redacted: bool) -> dict[str, object]:
    files = []
