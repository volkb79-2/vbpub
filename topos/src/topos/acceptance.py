"""topos acceptance smoke harness for rootless release-confidence checks.

Usage:
    python -m topos.acceptance smoke [--cgroup-root PATH] [--replay PATH] [--json] [--pretty-json]
    python -m topos.acceptance steady [--cgroup-root PATH] [--samples N] [--interval-s SECONDS]
                         [--max-cpu-pct FLOAT] [--max-rss-kb INT] [--json] [--pretty-json]
    python -m topos.acceptance tui-smoke [--replay PATH] [--config PATH] [--profile NAME]
                               [--timeout-s FLOAT] [--json] [--pretty-json]
    python -m topos.acceptance mcp-smoke [--socket PATH] [--timeout-s FLOAT]
                                [--json] [--pretty-json]
...
_MCP_SMOKE_TOOLS = {"topos_health", "topos_overview", "topos_entity", "topos_history"}
# Call order is load-bearing, not cosmetic: topos_entity and topos_history need
# the entity key that topos_overview discovers, so iterating the set above (whose
# order varies with PYTHONHASHSEED) would call them with an empty selector in
# most interpreters.
_MCP_SMOKE_CALL_ORDER = ("topos_health", "topos_overview", "topos_entity", "topos_history")
...
def format_tui_smoke_text(result: TuiSmokeResult) -> str:
    ...
    lines: list[str] = []
    lines.append(f"topos acceptance tui-smoke  v{result.version}")
...
def _tool_call_failure(tool_result: Any) -> str | None:
    ...
    sets it when a tool *raises*, while topos's tools *return* their typed
    failures as an ordinary ``{"error": {"code": ...}}`` payload
    (``mcp/server.py`` ``_tool_error``).  Checking ``isError`` alone therefore
    reports success for every typed topos error -- an assertion that cannot
...
def run_mcp_smoke(
    ...
    *,
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="topos-mcp-smoke-"))
    daemon_socket: Path = socket_path if socket_path is not None else tmpdir / "topos.sock"
...
async def _run_mcp_client_session(
    ...
    server_params = StdioServerParameters(
        ...
        args=["-m", "topos.cli", "mcp", "serve", "--socket", str(daemon_socket)],
        env=env,
    )
    ...
    async with stdio_client(server_params) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            # Deterministic order: topos_overview must run before the two tools
            ...
            for tool_name in _MCP_SMOKE_CALL_ORDER:
                try:
                    if tool_name == "topos_health":
                        result = await session.call_tool(tool_name, {})

                    elif tool_name == "topos_overview":
                        result = await session.call_tool(tool_name, {"sort_by": "ram", "limit": 5})

                    elif tool_name == "topos_entity":
            ...
            try:
                bogus_result = await session.call_tool("topos_entity", {"selector": "__nonexistent__"})
            ...
            try:
                loss_result = await session.call_tool("topos_overview", {"sort_by": "ram", "limit": 1})
...
def format_mcp_smoke_text(result: McpSmokeResult) -> str:
    ...
    lines: list[str] = []
    lines.append(f"topos acceptance mcp-smoke  v{result.version}")
    lines.append(f"python: {result.python.split()[0]}  platform: {result.platform}")
    lines.append("")

    if not result.extra_installed:
        lines.append("  SKIPPED: topos[mcp] extra not installed")
        lines.append("")
...
def format_steady_text(result: SteadyResult) -> str:
    ...
    lines: list[str] = []
    lines.append(f"topos acceptance steady  v{result.version}")
    lines.append(f"python: {result.python.split()[0]}  platform: {result.platform}")
    lines.append("")
...
def format_text(result: SmokeResult) -> str:
    ...
    lines: list[str] = []
    lines.append(f"topos acceptance smoke  v{result.version}")
    lines.append(f"python: {result.python.split()[0]}  platform: {result.platform}")
    lines.append("")
    for check in result.checks:
        symbol = _OK_SYMBOL if check.ok else _FAIL_SYMBOL
...
def main() -> None:
    """Convenience entry point.  Called via ``python -m topos.acceptance``."""
