...
:class:`topos.providers.net_bpf.BpfProvider`.
...
class BpfSnapshotBridge:
    ...
    Args:
        bpf_root: Absolute path to the topos BPF pin root
            (e.g. ``/sys/fs/bpf/topos``). All map pin paths must reside
    ...
    def refresh(self, map_pin_rel_path: str) -> dict[str, Any]:
        ...
        Args:
            map_pin_rel_path: Path component **relative** to *bpf_root*
                identifying the pinned map, e.g. ``"topos_cgroup_skb"``.
    ...
    def _build_snapshot(
        ...
        return {
            ...
            "comment": (
                "BPF map snapshot produced by topos daemon BpfSnapshotBridge. "
                "Consumed by topos.providers.net_bpf.BpfProvider."
            ),
