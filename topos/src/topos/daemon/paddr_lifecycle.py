...
class DaemonPaddrLifecycle:
    ...
    true, ``start()`` plans and starts exactly one topos-owned whole-host paddr
    session. On graceful shutdown ``stop()`` tears down only the session
    owned by this daemon run.

    Idempotent restart: if a topos-owned paddr marker exists for the same
    ...
    def start(self) -> None:
        """Start (or adopt) a topos-owned paddr session.
        ...
        # Check for an existing topos-owned paddr marker first (idempotent).
        existing = self._find_existing_topos_paddr(root)
    ...
    def stop(self) -> int:
        ...
        Only stops sessions owned by the topos marker that this lifecycle
    ...
    def _find_existing_topos_paddr(self, state_dir: Path) -> DamonSession | None:
        """Find an existing topos-owned paddr marker for this damon_root.
        ...
        for marker in owned_markers(state_dir):
            ...
            # Found a topos-owned paddr marker for our damon_root.
    ...
    def _read_marker_payload(self, marker: Path) -> dict[str, object]:
        ...
        Ownership is uncertain when a marker in topos's marker directory is
