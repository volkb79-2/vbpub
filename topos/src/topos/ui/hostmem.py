...
from topos.damon.control import stop_owned_sessions
...
def render_host_memory_text(frame: Frame, *, config: ToposConfig, damon_root: Path) -> str:
...
def _session_lines(session: dict[str, object]) -> list[str]:
    ...
    if session.get("owner") == "topos":
        lines.append("  stop: topos damon stop --all-mine")
    else:
...
def _fmt_bytes(value: int) -> str:
    ...
    scaled = float(value)
    unit = units[0]
    for unit in units:  # pragma: no branch; final non-empty tuple element always breaks
