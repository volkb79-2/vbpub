...
from topos.ui.aliases import resolve_column
from topos.ui.sparkline import sparkline_from_history
...
def resolve_columns(config: ToposConfig, *, width: int, profile: str) -> tuple[str, ...]:
    return resolve_profile(config, width=width, profile=profile).columns


def resolve_profile(config: ToposConfig, *, width: int, profile: str) -> ProfileLayout:
...
def available_profiles(config: ToposConfig) -> tuple[str, ...]:
...
def normalize_profile_name(config: ToposConfig, profile: str | None) -> str:
...
def _profile_from_config(config: ToposConfig, profile: str) -> tuple[str, ...] | None:
