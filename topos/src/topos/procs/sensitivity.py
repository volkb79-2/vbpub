...
Reuses P81's ``Sensitivity`` enum and marker dialect (``topos.daemon.redaction``)
...
Docker/systemd unit identity elsewhere in topos) is ``operational``.
"""

from __future__ import annotations

from topos.daemon.api import Sensitivity
from topos.daemon.redaction import redaction_marker
...
def redact_process_row(row: dict[str, object], ceiling: Sensitivity | None) -> dict[str, object]:
    ...
    ``topos.daemon.redaction.redact_payload``'s convention.
    """
    if ceiling is None:
        return row
