...
(``topos.procs.candidates``), enriches only that retained set with the
...
history is exposed through :class:`ProcessFrameSource`, a ``topos.query``
...
from topos.procs.candidates import CandidateReasons, select_candidates
from topos.procs.identity import ProcessKey, read_boot_id
from topos.procs.owners import join_owner
from topos.query.source import FrameSource, SourceFrame, SourceProvenance
...
class ProcessFrameSource(FrameSource):
    """``topos.query`` ``FrameSource`` over a :class:`ProcessSampler`'s in-memory
