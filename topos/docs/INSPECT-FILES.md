...
The `topos inspect-files plan` command provides a disabled-by-default, read-only
planning interface for file/log inspection. The `topos inspect-files read`
...
topos inspect-files read --kind systemd-journal --target ssh.service --inspect-files --admin
```
...
The `topos inspect-files read` command provides bounded content reads for
