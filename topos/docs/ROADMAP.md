...
10. Re-carve the React web surface to D-003/D-011's accepted same-origin,
   ...
   outside Topos. Implement D-018's Overview/Explore/Entity/Incidents routes,
...
flowchart TD
    ...
    P46 --> P56[P56 topos squeeze :done:]
...
share `src/topos/cli.py` argument parsing, so they carry `Serialize-with:`
each other.
...
**Standing decisions (closed 2026-07-15): React, same-origin Topos hosting,
capability-token loopback auth and PWMCP browser gates.** Topos now owns a pinned
PWMCP 1.61.0-r6 consumer deployment in `topos/pwmcp`; CIU 4.6.0 starts it on a
dedicated internal network plus the workspace consumer network with no published
ports. Production Topos remains loopback-only. A browser fixture may bind to the
...
- **P75 - MCP live-daemon acceptance leg** (*review-derived*, from P58's pass #2, **done**).
  ...
  injected fake client, so nothing had ever run `topos mcp serve` against a real
  daemon. P75 now adds an `mcp-smoke` leg to `topos.acceptance` on the P33/P35/P38
...
payload byte/line limits. CLI: ``topos inspect-files read --kind --target --inspect-files --admin [--json] [--max-bytes] [--max-lines]``.
...
protocol operation, and ``topos daemon health [--json]`` CLI. Models truthful,
...
Status: done. `topos mcp serve` supplies the first separate frontend over the
...
history. The optional `topos[mcp]` extra is isolated from ordinary CLI paths;
...
P53 adds `topos --record FILE --headless [--interval N] [--duration S |
...
externally looped `topos --once`, where every frame is cold and consumers
must derive rates themselves from the embedded raw counters.

P54 adds `topos report FILE [--window last:Ns|all] [--group-by
...
P56 is **done**. ``topos squeeze --target CGROUP_PATH --admin --confirm SQUEEZE``
...
topos natively, gated through the existing P21/P46 admin action posture
...
metadata join (`src/topos/collect/dockerjoin.py`) that the collector already
runs every sweep, wherever topos currently takes a raw cgroup-path/entity
...
`handoff/P56-topos-squeeze.md`, and
...
for vaddr and paddr, topos-owned cleanup controls, fixture safety tests, and
...
planning module (`topos inspect-files plan`) with explicit --inspect-files
...
(`/run/topos/topos.sock`) when no explicit path is given, and adds
`topos daemon current --socket PATH [--pretty-json]` as a read-only one-frame
...
Status: done. P32 adds a read-only `topos daemon status` command that combines
...
Status: done. P33 adds a rootless `python -m topos.acceptance smoke` module for
...
Status: done. P38 adds a rootless `python -m topos.acceptance tui-smoke`
...
Status: done. P42 adds ``topos/src/topos/daemon/bpf_snapshot.py`` containing
...
``topos daemon serve`` via ``--bpf-root``/``--bpf-interval`` (disabled by
...
Status: done. `topos --attach <socket>` now consumes daemon frames over the
...
Status: done. `topos daemon preflight`, packaged systemd/tmpfiles templates,
...
`handoff/TOPOS-OBSERVABILITY-DISCUSSION.md`; agents must not restart it or infer
