...
| B-046 | P93 port-forward session | `test_record.py::test_cli_version_reports_package_version`, `test_acceptance.py::test_run_smoke_json_fixture_root`/`test_subprocess_smoke_json`/`test_run_steady_json_small_samples` | 4 tests hardcode the package version as `"0.1.0"`, but `pyproject.toml`'s `setuptools_scm` (`tag_regex = "^topos-v(?P<version>[0-9].*)$"`, `fallback_version = "0.1.0"`) now resolves a live dev version off the newest reachable `topos-v*` tag (currently `topos-v0.2.1`, e.g. `0.2.2.dev992+g<hash>.<date>`) — the tests were never updated after the first real `topos-v` tag was cut. Confirmed pre-existing and independent of any in-flight package: reproduces identically running ONLY `main`'s tip (`3dd08b12`, zero worktree/branch changes) through the exact `topos-suite` gate container (`tester-unified:local`). Currently makes the `topos-suite` gate lane exit 1 on a clean `main` checkout. | High; blocks a green `topos-suite` gate for every in-flight package | Open — fix by asserting a version *pattern* (e.g. `re.match(r"^\d+\.\d+\.\d+", ...)`) or a `SETUPTOOLS_SCM_PRETEND_VERSION` fixture pin instead of the literal `"0.1.0"`. |
