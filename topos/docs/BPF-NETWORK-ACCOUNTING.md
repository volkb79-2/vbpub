...
This document captures the v2 BPF network design for `topos` without changing
...
- Pin all topos-owned programs, maps, and ownership markers under
  `/sys/fs/bpf/topos/`.
...
- On startup, the helper should audit `/sys/fs/bpf/topos/` for stale topos-owned
...
``topos daemon serve`` behind an explicit ``--bpf-root`` CLI option or
