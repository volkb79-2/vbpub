# topos CONTRACTS — shared interfaces (frozen; changes need maintainer sign-off)
...
```
topos/
  pyproject.toml            # package: topos; console script: topos
  src/topos/
...
- Line 1 header: `{"type":"header","schema_version":1,"topos_version":...,`
...
TOML at `$XDG_CONFIG_HOME/topos/config.toml`; parse with `tomllib`; every key
optional with shipped defaults. Packages read config ONLY through a shared
`topos.config.load()` (P1 provides it). Thresholds live under `[thresholds]`,
...
`topos/src/topos/daemon/api.py` implements a versioned, bounded, peer-aware
...
- An authorization hook (`Callable[[PeerCredentials, str], tuple[ErrorCode,
  ...
  access enforced by the OS (mode 0660 root:topos). The hook receives
...
`topos mcp serve` is an optional `topos[mcp]`, stdio-only, read-only frontend.
...
|---|---|---|
| `topos_health` | fixed maximum 16 component summaries; 4 MiB aggregate cap | daemon health summary |
| `topos_overview` | closed sort keys; `limit` integer 1..50; 4 MiB aggregate cap | ranked compact metric rows |
| `topos_entity` | exact EntityKey or P57 docker name/prefix; max 128 metrics and 64 findings; 4 MiB aggregate cap | one entity detail |
| `topos_history` | exact EntityKey or P57 docker name/prefix; registry-validated `metric`; `last:Ns` (max 7 days) or `since:TS`; `limit` integer 1..100; 4 MiB aggregate cap | one metric's timestamp/value pairs |
