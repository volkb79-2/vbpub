"""Tests for topos.compare — the P64 informational baseline delta helper.
...
class TestDeterminism:
    ...
    def test_cli_stdout_byte_identical(self, tmp_path):
        ...
        cmd = [sys.executable, "-m", "topos.cli", "compare", str(cpath), str(bpath), "--json"]
...
class TestNeverReadsFrames:
    def test_compare_module_has_no_frame_or_recording_dependency(self):
        ...
        import topos.compare as compare_module
        ...
        assert "topos.model" not in src
        assert "topos.record" not in src
...
class TestCompareCLI:
    def _run(self, *args: str, cwd=None):
        cmd = [sys.executable, "-m", "topos.cli", "compare", *args]
        env = _merged_env({"PYTHONPATH": str(SRC_ROOT)})
...
class TestAgainstRealQueryEngine:
    def test_compares_genuine_p88_summary_results(self):
        ...
        from topos.query import DaemonHistoryFrameSource, MetricRef, Query, run_query
