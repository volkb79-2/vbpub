...
from topos.snapshot.bundle import _extract_archive, create, inspect_bundle
...
def test_create_snapshot_bundle_contains_bounded_frames_and_manifest(tmp_path: Path) -> None:
    config = ToposConfig(snapshots=SnapshotConfig(dir=tmp_path, frames=3))
...
def test_snapshot_handles_empty_previous_history(tmp_path: Path) -> None:
    config = ToposConfig(snapshots=SnapshotConfig(dir=tmp_path, frames=10))
    bundle = create(
        "missing.scope",
        HistoryRing.from_config(config),
        fixture_frame(),
...
def test_snapshot_inspect_cli(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "topos.cli", "snapshot", "inspect", str(bundle)],
        check=True,
