...
def test_cli_once_json_works_without_textual_import() -> None:
    ...
    proc = subprocess.run([sys.executable, "-m", "topos.cli", "--once", "--json", "--cgroup-root", str(fixture_root() / "cgroupfs" / "gstammtisch")], check=True, cwd=fixture_root().parents[1], env=env, text=True, stdout=subprocess.PIPE)
