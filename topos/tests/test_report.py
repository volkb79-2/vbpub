"""Tests for topos.report — steady-state profile computation.
...
class TestPercentile:
    ...
    def test_max_is_last(self):
        ...
        s = [1.0, 2.0, 3.0]
        from topos.report import _max_value
...
class TestReportAutoCLI:
    ...
    def _invoke(self, recording: Path, *args: str) -> subprocess.CompletedProcess:
        ...
        return subprocess.run(
            [sys.executable, "-m", "topos.cli", "report", str(recording), "--json", *args],
            capture_output=True,
            text=True,
            cwd=str(src_root),
...
class TestDeriveRate:
    ...
    def test_live_v_used_as_is(self):
        ...
        from topos.report import _group_frames
...
class TestReportCLI:
    ...
    def test_no_json_flag_renders_table_by_default(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "topos.cli", "report", str(fixture)],
    ...
    def test_json_and_table_conflict_exits_2(self):
        ...
        fixture = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "frames" / "gstammtisch-once.jsonl"
        result = subprocess.run(
            [sys.executable, "-m", "topos.cli", "report", str(fixture), "--json", "--table"],
            capture_output=True, text=True,
            cwd=str(src_root),
            env={"PYTHONPATH": str(src_root)},
        )
    ...
    def test_bad_window_spec_exits_2(self, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "topos.cli", "report", str(recording), "--json", "--window", "bad"],
            capture_output=True, text=True,
            cwd=str(src_root),
            env={"PYTHONPATH": str(src_root)},
        )
    ...
    def test_missing_file_exits_2(self):
        src_root = Path(__file__).resolve().parents[1] / "src"
        result = subprocess.run(
            [sys.executable, "-m", "topos.cli", "report", "nonexistent.jsonl", "--json"],
            capture_output=True, text=True,
            cwd=str(src_root),
            env={"PYTHONPATH": str(src_root)},
        )
    ...
    def test_report_on_fixture_via_cli(self, tmp_path):
        """Run topos report on the existing gstammtisch-once fixture via CLI."""
        src_root = Path(__file__).resolve().parents[1] / "src"
        fixture = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "frames" / "gstammtisch-once.jsonl"
        result = subprocess.run(
            [sys.executable, "-m", "topos.cli", "report", str(fixture), "--json"],
            capture_output=True, text=True,
            cwd=str(src_root),
            env={"PYTHONPATH": str(src_root)},
        )
    ...
    def test_zstd_magic_garbage_exits_2_no_traceback(self, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "topos.cli", "report", str(fpath), "--json"],
            capture_output=True, text=True,
            cwd=str(src_root),
            env={"PYTHONPATH": str(src_root)},
        )
...
class TestCorruptRecordingCLI:
    ...
    def test_oracle_3_corrupt_jsonl_body(self, tmp_path):
        ...
        with open(fpath, "w") as f:
            f.write('{"type":"header","schema_version":1,"topos_version":"1.0",')
...
class TestEvaluateAssertions:
    ...
    def test_deterministic_output(self):
        ...
        from topos.report import assertion_result_to_jsonable
...
class TestReportAssertionCLI:
    ...
    def _invoke(self, *args: str) -> subprocess.CompletedProcess:
        src_root = Path(__file__).resolve().parents[1] / "src"
        cmd = [sys.executable, "-m", "topos.cli", "report", str(self.FIXTURE), "--json", *args]
        return subprocess.run(
            cmd,
            capture_output=True, text=True,
            cwd=str(src_root),
