...
- CLI topos daemon health --json / --pretty-json
...
from topos.daemon.broker import FrameBroker, FrameProducerError, serve_unix_socket
...
def test_daemon_client_request_health(tmp_path: Path) -> None:
    ...
    from topos.daemon.client import DaemonClient, DaemonResponseError
...
def test_daemon_serve_health_tracks_collector_lifecycle(
    ...
    config = ToposConfig(interval=0.0, cgroup_root=tmp_path / "cgroup")
    observed: list[dict] = []
...
def test_cli_health_via_main_daemon(tmp_path: Path) -> None:
    """CLI topos daemon health --json returns valid JSON."""
...
def test_cli_health_pretty_json(tmp_path: Path) -> None:
    """CLI topos daemon health --pretty-json produces indented JSON."""
    from topos.cli import _main_daemon
