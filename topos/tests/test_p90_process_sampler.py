...
from topos.config import ProcessConfig, ProcessConfigError
from topos.model import CiuMeta, DockerMeta, Entity, frame_from_jsonable, frame_to_jsonable
from topos.procs import candidates as candidates_mod
from topos.procs import procfs
from topos.procs.candidates import ProcessKey, select_candidates
from topos.procs.owners import join_owner
from topos.procs.sampler import ProcessSampler
from topos.procs.sensitivity import redact_process_row
from topos.daemon.api import Sensitivity
from topos.query.engine import MetricRef, Query, run_query
...
def test_o6_permission_denied_is_typed_not_zero(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("topos.procs.sampler.procfs.read_io", denied_read_io)
    tick = sampler.sample({})
    key = ProcessKey(pid=pid, start_ticks=5, boot_id=sampler._boot_id)
