...
from topos.actions.execute import ExecuteResult, execute_kill, execute_plan, execute_update
...
class TestDetectOwner:
    def test_compose(self) -> None:
        from topos.actions.owner_safety import detect_owner
...
class TestEvaluateTypedReasons:
    ...
    def _eval(self, kind, target, *, labels=None, protected=(), inspect=None):
        from topos.actions.owner_safety import evaluate
        if inspect is None:
...
class TestResolveIdentity:
    def test_valid(self) -> None:
        from topos.actions.owner_safety import resolve_identity
...
class TestCliProductionWiring:
    ...
    def _capture(monkeypatch, name: str):
        from topos.actions import execute as execute_module
