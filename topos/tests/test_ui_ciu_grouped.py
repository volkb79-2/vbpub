...
These tests build a real ``ToposApp`` over frames containing ciu-managed
...
def _make_ciu_app(entity_frames: list[EntityFrame]) -> ToposApp:
    """A ``ToposApp`` over a single synthetic frame, ``triage`` profile so the
